...
CvCity::CvCity()
{
	...
	m_aiCityBonusYieldRateModifier = new int[NUM_YIELD_TYPES];
	...
	m_aiCityBonusCommerceRateModifier = new int[NUM_COMMERCE_TYPES];
...
/*************************************************************************************************/
	...
	m_paiYieldIncome = new int[NUM_YIELD_TYPES];
	m_paiCommercePerPopulation = new int[GC.getNUM_COMMERCE_TYPES()];
	m_paiCommercePerCultureLevel = new int[GC.getNUM_COMMERCE_TYPES()];
	m_paiYieldPerCultureLevel = new int[GC.getNUM_YIELD_TYPES()];
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
...
CvCity::~CvCity()
{
	...
	SAFE_DELETE_ARRAY(m_aiCityBonusYieldRateModifier);
	...
	SAFE_DELETE_ARRAY(m_aiCityBonusCommerceRateModifier);
...
/*************************************************************************************************/
	...
	SAFE_DELETE_ARRAY(m_paiYieldIncome);
	...
	SAFE_DELETE_ARRAY(m_paiYieldFromBonus);
	SAFE_DELETE_ARRAY(m_paiCommerceFromBonus);
	SAFE_DELETE_ARRAY(m_paiCommerceFromUnits);
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
}
...
*/
		...
		if ((pPlot->getFeatureType() != NO_FEATURE) && (pPlot->getFeatureType() != (FeatureTypes)GC.getDefineINT("FLOOD_PLAINS_FEATURE")))
/************************************************************************************************/
...
//FfH: End Add
	...
	changeGreatPeopleThreshold(500);
...
void CvCity::reset(int iID, PlayerTypes eOwner, int iX, int iY, bool bConstructorCall)
{
	...
	m_bPlundered = false;
	m_bDisableGrowth = false;
	m_bDisableProduction = false;
...
/*************************************************************************************************/
	...
	m_iImprovementInfrastructureCost = 0;
	m_iTotalSpecializationLevel = 0;
	m_iInfrastructureLevel = 0;
	m_iCommercePercentTypeForced = NO_COMMERCE;
	m_iUnitSupport = 0;
	m_iCultureFromImprovements = 0;
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
	...
	for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		...
		m_aiCityBonusYieldRateModifier[iI] = 0;
		m_aiTradeYield[iI] = 0;
	...
	for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
	{
		...
		m_aiCityBonusCommerceRateModifier[iI] = 0;
...
/*************************************************************************************************/
		...
		m_paiYieldIncome = new int[NUM_YIELD_TYPES];
		m_paiYieldPerCultureLevel = new int[NUM_YIELD_TYPES];
        for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
		{
			m_paiYieldIncome[iI] = 0;
			m_paiYieldPerCultureLevel[iI] = 0;
		}

		m_paiCommercePerPopulation = new int[NUM_COMMERCE_TYPES];
        for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
		{
			m_paiCommercePerPopulation[iI] = 0;
		}

		m_paiCommercePerCultureLevel = new int[NUM_COMMERCE_TYPES];
        for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
		{
			m_paiCommercePerCultureLevel[iI] = 0;
		}

		m_paiYieldFromBonus = new int[GC.getNumBonusInfos()];
		for (int iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
		{
			m_paiYieldFromBonus[iJ] = 0;
		}

		m_paiCommerceFromBonus = new int[GC.getNumBonusInfos()];
		for (int iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
		{
			m_paiCommerceFromBonus[iJ] = 0;
		}

		m_paiCommerceFromUnits = new int[NUM_COMMERCE_TYPES];
		for (int iJ = 0; iJ < NUM_COMMERCE_TYPES; iJ++)
		{
			m_paiCommerceFromUnits[iJ] = 0;
		}

		m_paiBuildingClassBlocked = new int[GC.getNumBuildingClassInfos()];
		for (int iJ = 0; iJ < GC.getNumBuildingClassInfos(); iJ++)
		{
			m_paiBuildingClassBlocked[iJ] = 0;
		}

		m_paiCitySpecialization = new int[NUM_CITYSPECIALIZATION_TYPES];
		for (int iJ = 0; iJ < NUM_CITYSPECIALIZATION_TYPES; iJ++)
		{
			m_paiCitySpecialization[iJ] = 0;
		}

		m_paiCitySpecializationLevel = new int[NUM_CITYSPECIALIZATION_TYPES];
		for (int iJ = 0; iJ < NUM_CITYSPECIALIZATION_TYPES; iJ++)
		{
			m_paiCitySpecializationLevel[iJ] = 0;
		}
    ...
    for(int iAdvLoop=0;iAdvLoop<GC.getNumAdventureInfos();iAdvLoop++)
    {
        if(GET_PLAYER(getOwnerINLINE()).isAdventureEnabled(iAdvLoop) && !GET_PLAYER(getOwnerINLINE()).isAdventureFinished(iAdvLoop))
        {
            ...
            for(int iJ=0;iJ<GC.getMAX_ADVENTURESTEPS();iJ++)
            {
                ...
                {
                    if(GC.getAdventureStepInfo((AdventureStepTypes)kAdv.getAdventureStep(iJ)).isNeverLooseCity())
                    {
...
//FfH: Added by Kael 08/08/2007
        if (getNumBuilding((BuildingTypes)iI) > 0)
        {
            ...
            {
                GC.getGameINLINE().changeGlobalCounter(-1 * GC.getBuildingInfo((BuildingTypes)iI).getModifyGlobalCounter());
            }
        }
//FfH: End Add
...
/*************************************************************************************************/
	if (getAIGroup_Defense()!=NULL)
	{
		getAIGroup_Defense()->kill();
	}
/*************************************************************************************************/
/**	    									END													**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Start("City::doTurn");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Start("City::doTurn::zero");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Stop("City::doTurn::zero");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
/*************************************************************************************************/
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Start("City::doTurn::AI_doTurn");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Stop("City::doTurn::AI_doTurn");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Start("City::doTurn::Stuff1");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Stop("City::doTurn::Stuff1");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
/*************************************************************************************************/
...
//	doGrowth();
    if (!(GET_PLAYER(getOwnerINLINE()).isIgnoreFood()) && !isDisableGrowth())
    {
        doGrowth();
    }
        ...
        if (getNumBuilding((BuildingTypes)iI) > 0)
        {
            if (GC.getBuildingInfo((BuildingTypes)iI).getRemovePromotion() != NO_PROMOTION)
            {
                ...
                while (pUnitNode != NULL)
                {
                    ...
                    if (pLoopUnit->isHasPromotion((PromotionTypes)GC.getBuildingInfo((BuildingTypes)iI).getRemovePromotion()))
                    {
                        pLoopUnit->setHasPromotion((PromotionTypes)GC.getBuildingInfo((BuildingTypes)iI).getRemovePromotion(), false);
                    }
                }
            }
            if (GC.getBuildingInfo((BuildingTypes)iI).isApplyFreePromotionOnMove())
            {
                if (GC.getBuildingInfo((BuildingTypes)iI).getFreePromotion() != NO_PROMOTION)
                {
                    ...
                    while (pUnitNode != NULL)
                    {
                        pLoopUnit = ::getUnit(pUnitNode->m_data);
                        pUnitNode = plot()->nextUnitNode(pUnitNode);
                        if ((pLoopUnit->getUnitCombatType() != NO_UNITCOMBAT) && GC.getPromotionInfo((PromotionTypes)GC.getBuildingInfo((BuildingTypes)iI).getFreePromotion()).getUnitCombat(pLoopUnit->getUnitCombatType()))
                        {
                            pLoopUnit->setHasPromotion((PromotionTypes)GC.getBuildingInfo((BuildingTypes)iI).getFreePromotion(), true);
                        }
                    }
                }
            }
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Start("City::doTurn::doProduction");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
	if (!isDisableProduction())
	{
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Stop("City::doTurn::doProduction");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
/*************************************************************************************************/
...
//		for (iI = 0; iI < NUM_CITY_PLOTS; iI++)
		for (iI = 0; iI < getNumCityPlots(); iI++)
		{
			...
			if (pLoopPlot != NULL)
			{
				...
				{
					//only checks if a resource should spawn
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Start("City::doTurn::Python");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Stop("City::doTurn::Python");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
/*************************************************************************************************/
	...
	if (!isHuman())
	{
		iDiffMod=GC.getHandicapInfo(GC.getGameINLINE().getHandicapType()).getAIFreeXP();
	}
	for (int iLoopCombat = 0; iLoopCombat < GC.getNumUnitCombatInfos(); iLoopCombat++)
	{
		// City can train the UnitCombat?
		int iTrainCap=getTrainXPCap(iLoopCombat)+GET_PLAYER(getOwnerINLINE()).getTrainXPCap(iLoopCombat);
		if(iTrainCap>0)
		{
			iTrainCap+=iDiffMod; //Cap Modified for AI Players (free XP)
			for (int iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
			{
				...
				if (pLoopPlot != NULL)
				{
					//Plot in our Land with a possible Unit to Train?
					if(pLoopPlot->getArea()==getArea() && pLoopPlot->getTeam()==getTeam() && pLoopPlot->getNumUnits()>0)
					{
						...
						while (pUnitNode != NULL)
						{
							...
							//Friendly Unit with the correct UnitCombat?
							if(pLoopUnit!=NULL && pLoopUnit->getTeam()==getTeam() && pLoopUnit->getUnitCombatType()==iLoopCombat)
							{
								int iXP=pLoopUnit->getExperience();
								if (iXP<iTrainCap+getProductionExperience(pLoopUnit->getUnitType())+getFreeExperience())
								{
									iChance=getTrainXPRate(iLoopCombat)+GET_PLAYER(getOwnerINLINE()).getTrainXPRate(iLoopCombat);
									iChance*=(iTrainCap-iXP);
									iChance/=std::max(1,iTrainCap);
									if(GC.getGameINLINE().getSorenRandNum(10000, "Training")<iChance)
									{
										pLoopUnit->changeExperience(1);
									}
								}
							}
						}
...
/** ImprovementInfrastructureCosts Sephi	**/     
//	updateImprovementInfrastructureCost();

	updateCultureFromImprovements();

	calculateCityBonusYieldRateModifier();
	calculateCityBonusCommerceRateModifier();

	for(int iI = YIELD_COMMERCE + 1; iI < NUM_YIELD_TYPES; iI++)
	{
		updateYieldIncome(iI);
	}

	//Global Enchantments /make sure all effects via buildings are valid
	for(int iI = 0; iI < GC.getNumProjectInfos(); iI++)
	{
		if(GC.getProjectInfo((ProjectTypes)iI).isGlobalEnchantment())
		{
			BuildingTypes eBuilding=(BuildingTypes)GC.getProjectInfo((ProjectTypes)iI).getBuildingType();
			if(eBuilding!=NO_BUILDING)
			{
				...
				else
				{
					...
					{
						if(iJ!=getTeam() && GET_TEAM((TeamTypes)iJ).isAtWar(getTeam()))
						{
	...
	doCitySpecialization();
	updateCommerceFromBonus();
	updateYieldFromBonus();
	updateCommerceFromUnits();

/*************************************************************************************************/
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Stop("City::doTurn");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
//			for (iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
		...
		for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
		{
//			FAssert(getBuildingCommerce((CommerceTypes)iI) >= 0);	//some buildings like markets have negative Commerce, Sephi
...
/*************************************************************************************************/
	case TASK_NEW_TRAIT:
		{
			if(iData1 != NO_TRAIT) {
				kPlayer.changeGlobalCulture(-kPlayer.getTraitCost((TraitTypes)iData1));
				kPlayer.changeTotalTraitsFromCulture(1);
				kPlayer.setHasTrait((TraitTypes)iData1, true);
			}
		}
		break;
	...
	case TASK_SUMMON:
		{
			for(int iI=0;iI<GC.getNumProjectInfos();iI++)
			{
				if(GC.getProjectInfo((ProjectTypes)iI).getSummonUnitType()==iData1)
				{
					if(GET_PLAYER((PlayerTypes)getOwnerINLINE()).canCreateSummon((ProjectTypes)iI))
					{
						GET_PLAYER((PlayerTypes)getOwnerINLINE()).setCurrentMagicRitual(iI);
					}
					break;
				}
			}
		...
		break;
	case TASK_TERRAFORM:
		if(GET_PLAYER((PlayerTypes)getOwnerINLINE()).canDoTerraformRitual((ProjectTypes)iData1))
		{
			GET_PLAYER((PlayerTypes)getOwnerINLINE()).setCurrentMagicRitual(iData1);
		}
		break;
	case TASK_GLOBAL_ENCHANTMENT:
		if(GET_PLAYER((PlayerTypes)getOwnerINLINE()).canDoGlobalEnchantment((ProjectTypes)iData1))
		{
			GET_PLAYER((PlayerTypes)getOwnerINLINE()).setCurrentMagicRitual(iData1);
		}
		break;
	case TASK_REMOVE_GLOBAL_ENCHANTMENT:
		GET_PLAYER((PlayerTypes)getOwnerINLINE()).RemoveGlobalEnchantment((ProjectTypes)iData1);
		break;
	case TASK_CITY_INFRASTRUCTURE_INCREASE:
		changeInfrastructureLevel(1);
		GET_PLAYER((PlayerTypes)getOwnerINLINE()).changeGlobalCulture(-GET_PLAYER((PlayerTypes)getOwnerINLINE()).calculateCultureNeededForCityInfrastructure());
		GET_PLAYER((PlayerTypes)getOwnerINLINE()).changeTotalCityInfastructureLevel(1);		
		break;
	case TASK_CULTURAL_ACHIEVEMENT:
		GET_PLAYER((PlayerTypes)getOwnerINLINE()).changeGlobalCulture(-GET_TEAM(getTeam()).getResearchCost((TechTypes)iData1));
		GET_TEAM(getTeam()).setHasTech((TechTypes)iData1,true,getOwnerINLINE(),true,true);
		break;
	case TASK_CITY_DUNGEON_EVENT:
		GC.getMapINLINE().getDungeon(iData1)->ResolveDeal(getOwnerINLINE(), (DungeonEventTypes)iData2);
		break;
/*************************************************************************************************/
/**	END                                                                                         **/
/*************************************************************************************************/
...
/*************************************************************************************************/
	if(!bTestVisible)
	{
		if(GC.getUnitInfo(eUnit).isCitySupportNeeded())
		{
			UnitTypes eCurrentUnit=getProductionUnit();
			if(!GET_PLAYER(getOwnerINLINE()).canSupportMoreUnits((eCurrentUnit!=NO_UNIT && GC.getUnitInfo(eCurrentUnit).isCitySupportNeeded()) ? -1 : 0))
			{
				return false;
			}
		}
	...
	if(isHuman() && !bTestVisible) {
		if(GC.getUnitInfo(eUnit).getGlobalYieldCost() > 0 &&
			GC.getUnitInfo(eUnit).getGlobalYieldTypeCost() != NO_YIELD) {
			if(!bContinue) {
//			if(getProductionTurnsLeft(eBuilding, 0) <= 1) {
				if(GET_PLAYER(getOwnerINLINE()).getGlobalYield(GC.getUnitInfo(eUnit).getGlobalYieldTypeCost()) < GC.getUnitInfo(eUnit).getGlobalYieldCost()) {
					return false;
				}
			}
		}
...
/*************************************************************************************************/
/**	Sephi Tweak							                                                        **/
/**	ADDON (allow tolerant to build prereqCiv units)                                             **/
/*************************************************************************************************/
/**								---- Start Original Code ----									**
	    if (GC.getUnitInfo(eUnit).getPrereqCiv() != GET_PLAYER(getOwnerINLINE()).getCivilizationType())

/**								----  End Original Code  ----									**/
	    if (GC.getUnitInfo(eUnit).getPrereqCiv() != GET_PLAYER(getOwnerINLINE()).getCivilizationType() &&
            GC.getUnitInfo(eUnit).getPrereqCiv() != getCivilizationType())
/*************************************************************************************************/
/**	Tweak									END													**/
/*************************************************************************************************/
	    {
...
/**
	if(GC.getUnitInfo(eUnit).isScaleWithTech()) {
		if(!GC.getCivilizationInfo(getCivilizationType()).isUnitRoster(eUnit)) {
			return false;
		}
	}
**/
    ...
    {
		if(GET_PLAYER(getOwnerINLINE()).isCityState())
		{
			return false;
		}
	}
...
/**	ADDON (block foreign heroes for tolerant cities) merged Sephi								**/
...
/**								----  End Original Code  ----									**/
        if (GC.getCivilizationInfo(GET_PLAYER(getOwnerINLINE()).getCivilizationType()).getCivilizationUnits(eUnitClass) != eUnit
            || (GC.getUnitInfo(eUnit).getPrereqCiv()!=NO_CIVILIZATION && GC.getUnitInfo(eUnit).getPrereqCiv() != GET_PLAYER(getOwnerINLINE()).getCivilizationType()))
/*************************************************************************************************/
/**	Tweak									END													**/
/*************************************************************************************************/
        {
...
/*************************************************************************************************/
/**	Xienwolf Tweak							09/06/08											**/
/**	ADDON (upgrades for Tolerant units) merged Sephi											**/
...
/**	ADDON (New Function Definitions) Sephi                                                      **/
/*************************************************************************************************/
	if (GC.getUnitInfo(eUnit).getPopulationCost()>0)
	{
		if(getPopulation()-GC.getUnitInfo(eUnit).getPopulationCost()<1)
		{
			return false;
		}
	}
...
//FfH: End Modify
	...
	if (!(kPlayer.canConstruct(eBuilding, bContinue, bTestVisible, bIgnoreCost)))
	{
		return false;
	}
...
/*************************************************************************************************/
	GameOptionTypes eGameOption = (GameOptionTypes)GC.getBuildingInfo(eBuilding).getRequiredGameOption();
	if (eGameOption!=NO_GAMEOPTION)
	{
		if (!GC.getGameINLINE().isOption(eGameOption))
		{
...
/*************************************************************************************************/
	if(isHuman() && !bTestVisible) {
		if(GC.getBuildingInfo(eBuilding).getGlobalYieldCost() > 0 &&
			GC.getBuildingInfo(eBuilding).getGlobalYieldTypeCost() != NO_YIELD) {
			if(!bContinue) {
//			if(getProductionTurnsLeft(eBuilding, 0) <= 1) {
				if(GET_PLAYER(getOwnerINLINE()).getGlobalYield(GC.getBuildingInfo(eBuilding).getGlobalYieldTypeCost()) < GC.getBuildingInfo(eBuilding).getGlobalYieldCost()) {
					return false;
				}
			}
		}
	...
	if (isBuildingClassBlocked(GC.getBuildingInfo(eBuilding).getBuildingClassType()))
	{
		return false;
	}
	if (GC.getBuildingInfo(eBuilding).getPopulationCost()>0)
	{
		if(getPopulation()-GC.getBuildingInfo(eBuilding).getPopulationCost()<1)
		{
			return false;
		}
	}
	...
	for(iI=0;iI<GC.getNumBonusInfos();iI++)
	{
		if(GC.getBuildingInfo(eBuilding).isBonusConsumed(iI))
		{
			bValid=false;
			bFound=false;
			...
			if(kPlayer.getNumAvailableBonuses((BonusTypes)iI)-kPlayer.getBonusImport((BonusTypes)iI)-kPlayer.getNumBonusesConsumedByProduction((BonusTypes)iI)+iCurrent<=0)
			{
				return false;
			}
		}
	...
	for(iI=0;iI<GC.getNumBonusInfos();iI++)
	{
		if(GC.getBuildingInfo(eBuilding).isBonusNearbyNeeded(iI))
		{
			bFound=true;
	...
	// LS: multiple buildings of the same class are not allowed
	if (isHasBuildingClass(GC.getBuildingInfo(eBuilding).getBuildingClassType()))
	{
		return false;
	}
...
/**						                                            							**/
/*************************************************************************************************/
	if(GC.getDefineINT("USE_CANCREATE_CALLBACK")==1)
	{
        CyCity* pyCity = new CyCity((CvCity*)this);
        CyArgsList argsList;
        argsList.add(gDLL->getPythonIFace()->makePythonObject(pyCity));	// pass in city class
...
/**						                                            							**/
/*************************************************************************************************/
	if(GC.getDefineINT("USE_CANMAINTAIN_CALLBACK")==1)
	{
        CyCity* pyCity = new CyCity((CvCity*)this);
        CyArgsList argsList;
        argsList.add(gDLL->getPythonIFace()->makePythonObject(pyCity));	// pass in city class
...
/**						                                            							**/
/*************************************************************************************************/
	if(GC.getDefineINT("USE_CANNOTMAINTAIN_CALLBACK")==1)
	{
        CyCity* pyCity = new CyCity((CvCity*)this);
        CyArgsList argsList2; // XXX
        argsList2.add(gDLL->getPythonIFace()->makePythonObject(pyCity));	// pass in city class
...
int CvCity::getProductionModifier(UnitTypes eUnit) const
{
	...
	//new Yields added Sephi
	YieldTypes eYield=(YieldTypes)GC.getUnitInfo(eUnit).getReducedCostByYield();
	if(eYield!=NO_YIELD)
	{
		iMultiplier+=GET_PLAYER(getOwnerINLINE()).getProductionIncreaseByYield(eYield,this,NO_BUILDING,eUnit);
	}
	//end added
...
int CvCity::getProductionModifier(BuildingTypes eBuilding) const
{
	...
	//new Yields added Sephi
	YieldTypes eYield=(YieldTypes)GC.getBuildingInfo(eBuilding).getReducedCostByYield();
	if(eYield!=NO_YIELD)
	{
		iMultiplier+=GET_PLAYER(getOwnerINLINE()).getProductionIncreaseByYield(eYield,this,eBuilding,NO_UNIT);
	}
	//end added
...
/*************************************************************************************************/
/** ADDON	(Multiple Production Mod) merged Sephi		 08/23/09 Denev							 */
/**                                                                                              */
/**                                                                                              */
/*************************************************************************************************/
...
/*************************************************************************************************/
    ...
    {
        iUnhappyProd += unhappyLevel(0)/2;

        if (iUnhappyProd>5)
        {
             iUnhappyProd=std::max(5,unhappyLevel(0)/3);
        }
        iUnhappyProd +=angryPopulation(0);
    }
/*************************************************************************************************/
/**	END                                                                  						**/
/*************************************************************************************************/
	// modfied for Multiple Production Mod
	//added +50 to make sure 1 hammer * 0.8 modifier isn't 0, but 1 * 1.2 is 1
	return ((((iYield + iOverflow + iUnhappyProd) * getBaseYieldRateModifier(YIELD_PRODUCTION, iProductionModifier) +50)/ 100) + iFoodProduction);
//FfH: End Modify
...
bool CvCity::canHurry(HurryTypes eHurry, bool bTestVisible) const
{
	...
	if(GC.getHurryInfo(eHurry).getStonePerProduction() > 0) {
		if(isProductionUnit()) {
			return false;
		}
	}
	...
	if (!bTestVisible)
	{
		...
		if (GET_PLAYER(getOwnerINLINE()).getGlobalYield(YIELD_STONE) < hurryStone(eHurry))
		{
			return false;
		}
...
bool CvCity::canHurryBuilding(HurryTypes eHurry, BuildingTypes eBuilding, bool bIgnoreNew) const
{
	...
	if (GET_PLAYER(getOwnerINLINE()).getGlobalYield(YIELD_STONE) < getHurryStone(eHurry, getHurryCost(false, eBuilding, bIgnoreNew)))
	{
		return false;
	}

	//can only rush if half the building is already worked on
	BuildingClassTypes eClass=(BuildingClassTypes)GC.getBuildingInfo(eBuilding).getBuildingClassType();
	if (GC.getBuildingClassInfo(eClass).getMaxGlobalInstances()!=-1 || GC.getBuildingClassInfo(eClass).getMaxPlayerInstances()!=-1 || GC.getBuildingClassInfo(eClass).getMaxTeamInstances()!=-1)
	{
		if(getBuildingProduction(eBuilding)*2<GC.getBuildingInfo(eBuilding).getProductionCost())
		{
			return false;
		}
	}
...
void CvCity::hurry(HurryTypes eHurry)
{
	...
	GET_PLAYER(getOwnerINLINE()).changeGlobalYield(YIELD_STONE,-(iHurryStone));
...
UnitTypes CvCity::getConscriptUnit() const
{
	...
	if(GC.getCivilizationInfo(getCivilizationType()).getDefaultUnit() != NO_UNIT)
		return (UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getDefaultUnit();
	...
	for (iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
	{
		...
		if (eLoopUnit != NO_UNIT)
		{
			if (canTrain(eLoopUnit))
			{
				...
				if(GC.getUnitClassInfo(eUnitClass).getMaxPlayerInstances() == -1 
					&& GC.getUnitClassInfo(eUnitClass).getMaxGlobalInstances() == -1) {
...
/**						                                            							**/
/*************************************************************************************************/
	if(GC.getDefineINT("USE_CONSCRIPTUNIT_CALLBACK")==1)
	{
...
CvUnit* CvCity::initConscriptedUnit()
{
	...
	CvUnit* pUnit = PlaceUnit(eConscriptUnit,getOwnerINLINE(),eCityAI);		
...
//FfH: Added by Kael 08/04/2007
        ...
        GC.getGameINLINE().changeGlobalCounter(GC.getBuildingInfo(eBuilding).getModifyGlobalCounter()  * iChange);
...
/*************************************************************************************************/
        ...
        if(GC.getBuildingInfo(eBuilding).getTaxesModifier() !=0 )
        {
            changeMistDensity(GC.getBuildingInfo(eBuilding).getTaxesModifier() * iChange);
        }
        if(GC.getBuildingInfo(eBuilding).getMistDensity() !=0 )
        {
            changeMistDensity(GC.getBuildingInfo(eBuilding).getMistDensity() * iChange);
        }

        if(GC.getBuildingInfo(eBuilding).getGPPThresholdReductionPercent() !=0 )
        {
            changeGPPThresholdReductionPercent(GC.getBuildingInfo(eBuilding).getGPPThresholdReductionPercent() * iChange);
        }

        if(GC.getBuildingInfo(eBuilding).getPopulationLimitChange() !=0 )
        {
            changePopulationLimit(GC.getBuildingInfo(eBuilding).getPopulationLimitChange() * iChange);
        }
		if(GC.getBuildingInfo(eBuilding).getUnitSupportChange() !=0)
		{
			changeUnitSupport(GC.getBuildingInfo(eBuilding).getUnitSupportChange() * iChange);
		}

		if(GC.getBuildingInfo(eBuilding).getMaxManaChange() !=0)
		{
			GET_PLAYER(getOwnerINLINE()).changeMaxMana(GC.getBuildingInfo(eBuilding).getMaxManaChange() * iChange);
		}

		if(GC.getBuildingInfo(eBuilding).getCommercePercentTypeForced()!=NO_COMMERCE)
		{
			setCommercePercentTypeForced((CommerceTypes)GC.getBuildingInfo(eBuilding).getCommercePercentTypeForced());
		}

		if(GC.getBuildingInfo(eBuilding).isDisableGrowth())
		{
			m_bDisableGrowth=true;
		}

		if(GC.getBuildingInfo(eBuilding).isDisableProduction())
		{
			m_bDisableProduction=true;
		}

		for(iI=0;iI<GC.getNumBuildingClassInfos();iI++)
		{
			if(GC.getBuildingInfo(eBuilding).isBuildingClassBlocked(iI))
			{
				changeBuildingClassBlocked(iI,iChange);
			}
		}

		updateBonusConsumed(eBuilding,iChange);
...
/*************************************************************************************************/
		...
		{
			if(GC.getBuildingInfo(eBuilding).getYieldPerCultureLevel(iI)!=0)
			{
				changeYieldPerCultureLevel(GC.getBuildingInfo(eBuilding).getYieldPerCultureLevel(iI) * iChange,iI);
			}
		}

		for(iI = 0; iI < GC.getNUM_COMMERCE_TYPES();iI++)
		{
			if(GC.getBuildingInfo(eBuilding).getCommercePerCultureLevel(iI) != 0)
			{
				changeCommercePerCultureLevel(GC.getBuildingInfo(eBuilding).getCommercePerCultureLevel(iI) * iChange, iI);
			}

			if(GC.getBuildingInfo(eBuilding).getCommercePerPopulation(iI) != 0)
			{
				changeCommercePerPopulation(GC.getBuildingInfo(eBuilding).getCommercePerPopulation(iI) * iChange, iI);
			}
		}
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
		...
		for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
		{
			...
			changeBaseYieldRate(((YieldTypes)iI), ((GC.getBuildingInfo(eBuilding).getYieldChange(iI) + getBuildingYieldChange((BuildingClassTypes)GC.getBuildingInfo(eBuilding).getBuildingClassType(), (YieldTypes)iI))* iChange), true);
...
/**						                                            							**/
/*************************************************************************************************/
				calculateBuildingBonus((BuildingClassTypes)GC.getBuildingInfo(eBuilding).getBuildingClassType(), (BonusTypes)iI, iChange);
/*************************************************************************************************/
/**	END Change Building Bonuses	                                        						**/
/*************************************************************************************************/
...
/*************************************************************************************************/
	...
	{
		if (GC.getBuildingInfo(eBuilding).getCorporationSupport((CorporationTypes)i)!=0)
		{
			GET_PLAYER(getOwnerINLINE()).changeCorporationSupport((CorporationTypes)i,GC.getBuildingInfo(eBuilding).getCorporationSupport((CorporationTypes)i));
		}

		if (GC.getBuildingInfo(eBuilding).getCorporationSupportMultiplier((CorporationTypes)i)!=0)
		{
			GET_PLAYER(getOwnerINLINE()).changeCorporationSupportMultiplier((CorporationTypes)i,GC.getBuildingInfo(eBuilding).getCorporationSupportMultiplier((CorporationTypes)i));
		}
	}
/*************************************************************************************************/
/**	END	                                        												**/
...
void CvCity::processSpecialist(SpecialistTypes eSpecialist, int iChange)
{
	...
	for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		changeBaseYieldRate(((YieldTypes)iI), (GC.getSpecialistInfo(eSpecialist).getYieldChange(iI) * iChange), true);
	}
...
/**	ADDON (City ArtStyle for tolerant cities) merged Sephi								        **/
...
{
	float fBonus=0.0f;
	if(GC.getBuildingClassInfo((BuildingClassTypes)GC.getBuildingInfo(eBuilding).getBuildingClassType()).getMaxGlobalInstances()==1)
	{
		fBonus=std::numeric_limits<float>::max()/2;
	}
	else if(GC.getBuildingClassInfo((BuildingClassTypes)GC.getBuildingInfo(eBuilding).getBuildingClassType()).getMaxPlayerInstances()==1)
	{
		fBonus=std::numeric_limits<float>::max()/4;
	}

	return fBonus+GC.getBuildingInfo(eBuilding).getVisibilityPriority();
}
...
void CvCity::setHeadquarters(CorporationTypes eIndex)
{
	...
	if (GC.getCorporationInfo(eIndex).getFreeUnitClass() != NO_UNITCLASS)
	{
		...
		{
			CvUnit* pUnit = PlaceUnit(eFreeUnit,getOwnerINLINE(),NO_UNITAI);		
		}
	}
}
...
int CvCity::goodHealth() const
{
	...
	if(!isHuman()) {
		...
		if(iInterval > 0) {
			...
			iAIBonus /= 100;
			iAIBonus /=	iInterval;
			iTotalHealth += std::max(0, iAIBonus);
		}
	}

	//Health from Culture
	iHealth = (GET_PLAYER(getOwnerINLINE()).getHealthRatePerCulture()*getCultureLevel())/100;
	if (iHealth > 0)
	{
		iTotalHealth += iHealth;
	}
...
int CvCity::badHealth(bool bNoAngry, int iExtra) const
{
	...
	iHealth = getImprovementHealthChange();
	if (iHealth < 0)
	{
		iTotalHealth += iHealth;
	}
...
//	return ((((getPopulation() + iExtra) - ((bNoAngry) ? angryPopulation(iExtra) : 0)) * GC.getFOOD_CONSUMPTION_PER_POPULATION()) - healthRate(bNoAngry, iExtra));
	...
	iFood += GET_PLAYER(getOwner()).getFoodPerPopulationModifier();

	return ((((getPopulation() + iExtra) - ((bNoAngry) ? angryPopulation(iExtra) : 0)) * iFood) - 2*healthRate(bNoAngry, iExtra));
//FfH: End Modify
...
int CvCity::hurryStone(HurryTypes eHurry) const
{
	return getHurryStone(eHurry, hurryCost(false));
}

int CvCity::getHurryStone(HurryTypes eHurry, int iHurryCost) const
{
	int iStone;
	...
	iStone = (iHurryCost * GC.getHurryInfo(eHurry).getStonePerProduction());

	iStone *= std::max(25, 100 - GET_PLAYER(getOwnerINLINE()).getReducedHurryCostBuilding());
	iStone /= 100;

	return std::max(1, iStone);
}
...
int CvCity::getGreatPeopleThreshold() const
{
	return m_iGreatPeopleThreshold;
}


void CvCity::changeGreatPeopleThreshold(int iChange)
{
	m_iGreatPeopleThreshold = (m_iGreatPeopleThreshold + iChange);
}
...
int CvCity::getTotalGreatPeopleRateModifier() const
{
	...
	iModifier += GET_PLAYER(getOwnerINLINE()).getGreatPersonRatePerCulture() * getCultureLevel();
...
//FfH: End Add
	iTempMaintenance = std::max(0, (getPopulation() - 10) * 300);
...
int CvCity::calculateNumCitiesMaintenanceTimes100() const
{
	...
	iNumCitiesPercent *= (getPopulation() + 7);
...
//FfH: End Modify
	...
	//new Calculation
	iNumCitiesMaintenance = ((GET_PLAYER(getOwnerINLINE()).getNumCities() - GET_PLAYER(getOwnerINLINE()).getNumSettlements())) * 100;
	iNumCitiesMaintenance = std::min(2000, iNumCitiesMaintenance);

	iNumCitiesMaintenance *= 100 + GET_PLAYER(getOwnerINLINE()).getNumCitiesMaintenanceModifier();
	iNumCitiesMaintenance /= 100;

	if(getPopulation() > 10) {
		iNumCitiesMaintenance *= 100 + (getPopulation() -10) * 20;
		iNumCitiesMaintenance /= 100;
	}
...
/**	ADDON (Houses of Erebus) Sephi			                                 					**/
/*************************************************************************************************/
    return getYieldRate(YIELD_COMMERCE)*15;
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
...
int CvCity::calculateBuildingMaintenanceTimes100() const
{
	...
	for(int iI=0;iI<GC.getNumBuildingInfos();iI++)
	{
		...
		{
			iMaintenance+=GC.getBuildingInfo((BuildingTypes)iI).getBuildingMaintenance();
		}
	}
...
{
	return (calculateDistanceMaintenanceTimes100() + calculateNumCitiesMaintenanceTimes100() + calculateColonyMaintenanceTimes100() + calculateCorporationMaintenanceTimes100() + calculateBuildingMaintenanceTimes100());
}
...
int CvCity::calculateSpellResearchFromFeaturesTimes100() const
{
	int iBase = 0;
	CvPlot* pLoopPlot;
				...
				if (pLoopPlot->getWorkingCity() == this)
				{
					if(!pLoopPlot->isCity() && pLoopPlot->getImprovementType() == NO_IMPROVEMENT) {
						if(pLoopPlot->getFeatureType() != NO_FEATURE &&
							kCiv.isMaintainFeatures(pLoopPlot->getFeatureType())) {
								++iBase;
						}
					}
				}
	...
	//Manashrines and Witchhuts
	ImprovementTypes iManaShrine=(ImprovementTypes)GC.getIMPROVEMENT_MANA_SHRINE();
	ImprovementTypes iWitchHut=(ImprovementTypes)GC.getInfoTypeForString("IMPROVEMENT_WITCH_HUT");
	ImprovementTypes iTower = (ImprovementTypes)GC.getInfoTypeForString("IMPROVEMENT_TOWER");
	...
	for (int iI = 0; iI < getNumCityPlots(); iI++)
	{
		...
		{
			if(pLoopPlot->getImprovementType() == iWitchHut)
			{
				iBase += 7;
			}

			if(pLoopPlot->getImprovementType() == iTower)
			{
				iBase += 10;
			}
...
int CvCity::calculateBaseTaxes() const
{
	int iBase = getPopulation() * 100;

	iBase *= 100 + GET_PLAYER(getOwnerINLINE()).getTaxesModifier();
	iBase /= 100;

	return iBase;
}

int CvCity::calculateTaxesTimes100() const
{
	int iTaxes = calculateBaseTaxes();

	iTaxes*=100+calculateTaxesModifier();
	iTaxes/=100;
	return iTaxes;
}
	
int CvCity::calculateTaxes() const
{
	return calculateTaxesTimes100() / 100;
}

int CvCity::calculateTaxesModifier() const
{
	int iTaxesMod=getTaxesModifier();
...
int CvCity::getImprovementHealthChange() const
{
	return m_iImprovementHealthChange;
}
...
void CvCity::updateImprovementHealth()
{
	...
	for (iI = 0; iI < getNumCityPlots(); iI++)
	{
		...
		if (pLoopPlot != NULL && pLoopPlot->getWorkingCity()==this)
		{
			...
			if (eImprovement != NO_IMPROVEMENT)
			{
				iChange = GC.getImprovementInfo(eImprovement).getHealth();
				if(iChange < 0 && 
	...
	m_iImprovementHealthChange = iNewValue/100;
}
...
int CvCity::getBuildingBadHealth(BuildingTypes eBuilding) const
{
	...
	/*if (eBuilding == GC.getInfoTypeForString("BUILDING_DIVINE_JUDGEMENT"))
	{
		iHealth -= GET_TEAM(GET_PLAYER(getOwnerINLINE()).getTeam()).getDivineJudgementCounter() / 3;
	}*/
...
int CvCity::getBuildingHappiness(BuildingTypes eBuilding) const
{
	...
	/*if (eBuilding == GC.getInfoTypeForString("BUILDING_DIVINE_JUDGEMENT"))
	{
		iHappiness -= GET_TEAM(GET_PLAYER(getOwnerINLINE()).getTeam()).getDivineJudgementCounter() / 6;
	}*/

	for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
	{
		if(getCommercePercentTypeForced()==NO_COMMERCE)
		{
		...
		else if(getCommercePercentTypeForced()==iI)
		{
			iHappiness += GC.getBuildingInfo(eBuilding).getCommerceHappiness(iI);
		}
	}
...
{
/*************************************************************************************************/
/**	ADDON (Allow Float values for FeatureHappiness) Sephi                     					**/
/*************************************************************************************************/
//	return m_iFeatureGoodHappiness;
	return m_iFeatureGoodHappiness/100;
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
}
...
//	return m_iFeatureBadHappiness;
	return m_iFeatureBadHappiness/100;
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
}
...
//	return ((getTotalDefense(bIgnoreBuilding) * (GC.getMAX_CITY_DEFENSE_DAMAGE() - getDefenseDamage())) / GC.getMAX_CITY_DEFENSE_DAMAGE());
	return (std::max(0,getTotalDefense(bIgnoreBuilding) - getDefenseDamage()));
}
...
void CvCity::setCultureLevel(CultureLevelTypes eNewValue, bool bUpdatePlotGroups)
{
	...
	if (eOldValue != eNewValue)
	{
		...
		{
		/**Sephi - Slower Culture Expansion
			for (iDX = -eOldValue; iDX <= eOldValue; iDX++)
			{
				for (iDY = -eOldValue; iDY <= eOldValue; iDY++)
				{
					...
					if (iCultureRange > getCultureLevel())
					{
						...
						{
					**/
			for (iDX = -eOldValue-1; iDX <= eOldValue+1; iDX++)
			{
				for (iDY = -eOldValue-1; iDY <= eOldValue+1; iDY++)
				{
					iCultureRange = cultureDistance(iDX, iDY);

					if (iCultureRange > getCultureRadius(getCultureLevel()))
					{
						if (iCultureRange <= getCultureRadius(eOldValue))
						{
					/** end modified **/
			...
			for (iDX = -getCultureLevel(); iDX <= getCultureLevel(); iDX++)
			{
				for (iDY = -getCultureLevel(); iDY <= getCultureLevel(); iDY++)
				{
					...
					if (iCultureRange > eOldValue)
					{
						...
						{
					**/
			for (iDX = -getCultureLevel()-1; iDX <= getCultureLevel()+1; iDX++)
			{
				for (iDY = -getCultureLevel()-1; iDY <= getCultureLevel()+1; iDY++)
				{
					iCultureRange = cultureDistance(iDX, iDY);
...
//			if ((getCultureLevel() > eOldValue) && (getCultureLevel() > 1))
			if ((getCultureRadius(getCultureLevel()) > getCultureRadius(eOldValue)) && (getCultureLevel() > 1))
			{
		...
		processCultureLevel(eOldValue,eNewValue);
		updateFeatureHealth();
		updateFeatureHappiness();
	}
}
...
int CvCity::getBaseYieldRateModifier(YieldTypes eIndex, int iExtra) const
{
	...
	iModifier += getCityBonusYieldRateModifier(eIndex);
...
void CvCity::changeBaseYieldRate(YieldTypes eIndex, int iChange, bool bGlobalYields)
{
	if(bGlobalYields || eIndex<=YIELD_COMMERCE)
	{
...
int CvCity::getCityBonusYieldRateModifier(YieldTypes eIndex) const
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < NUM_YIELD_TYPES, "eIndex expected to be < NUM_YIELD_TYPES");
	return m_aiCityBonusYieldRateModifier[eIndex];
}


void CvCity::calculateCityBonusYieldRateModifier()
{
	CvPlot* pLoopPlot;
	for(int i = 0; i < NUM_YIELD_TYPES; ++i) {
		int iModifier = 0;
		for(int j = 0; j < getNumCityPlots(); ++j) {
			pLoopPlot = getCityIndexPlot(j);

			if(pLoopPlot != NULL && pLoopPlot->getWorkingCity() == this) {
				if(pLoopPlot->getBonusType(getTeam()) != NO_BONUS) 
					if(GC.getBonusInfo(pLoopPlot->getBonusType(getTeam())).getCityYieldModifierType() == i)
						if(pLoopPlot == plot() || 
							(pLoopPlot->getImprovementType() != NO_IMPROVEMENT && GC.getImprovementInfo(pLoopPlot->getImprovementType()).isImprovementBonusTrade(pLoopPlot->getBonusType(getTeam()))))
							iModifier += GC.getBonusInfo(pLoopPlot->getBonusType(getTeam())).getCityBonusModifier();
			}
		}

		if(iModifier != m_aiCityBonusYieldRateModifier[i]) {
			m_aiCityBonusYieldRateModifier[i] = iModifier;

			GET_PLAYER(getOwnerINLINE()).invalidateYieldRankCache((YieldTypes)i);
...
void CvCity::updateExtraSpecialistYield(YieldTypes eYield)
{
	...
	if (iOldYield != iNewYield)
	{
		...
		changeBaseYieldRate(eYield, (iNewYield - iOldYield), true);
	}
}
...
int CvCity::getCommerceFromPercent(CommerceTypes eIndex, int iYieldRate) const
{
	...
	if(getCommercePercentTypeForced()!=NO_COMMERCE)
	{
		if(getCommercePercentTypeForced()!=eIndex)
		{
			return 0;
		}
		else
	...
	{
		iCommerce += (iYieldRate - iCommerce - getCommerceFromPercent(COMMERCE_RESEARCH, iYieldRate) - getCommerceFromPercent(COMMERCE_CULTURE, iYieldRate) - getCommerceFromPercent(COMMERCE_ESPIONAGE, iYieldRate) - getCommerceFromPercent(COMMERCE_ARCANE, iYieldRate) - getCommerceFromPercent(COMMERCE_FAITH, iYieldRate));
	}
...
//FfH: End Modify
	...
	//added Sephi
	if(eIndex == COMMERCE_CULTURE)
		iBaseCommerceRate += 100 * getCultureFromImprovements();

	iBaseCommerceRate += 100 * GET_PLAYER(getOwnerINLINE()).getCommerceChangesPerCultureLevel(eIndex)*getCultureLevel();
	iBaseCommerceRate += 100 * getCommercePerCultureLevel(eIndex) * getCultureLevel();
	iBaseCommerceRate += 100 * getCommercePerPopulation(eIndex) * getPopulation();

	iBaseCommerceRate += getCommerceFromBonus(eIndex);
	iBaseCommerceRate += getCommerceFromUnits(eIndex);
...
int CvCity::getTotalCommerceRateModifier(CommerceTypes eIndex) const
{
	return std::max(0, (getCommerceRateModifier(eIndex) 
		+ GET_PLAYER(getOwnerINLINE()).getCommerceRateModifier(eIndex) 
		+ ((isCapital()) ? GET_PLAYER(getOwnerINLINE()).getCapitalCommerceRateModifier(eIndex) : 0) + 100)
		+ getCityBonusCommerceRateModifier(eIndex));
}
...
void CvCity::updateCommerce(CommerceTypes eIndex)
{
	...
	else
	{
		...
		if(eIndex==COMMERCE_GOLD)
		{
			iNewCommerce += calculateTaxesTimes100();
		}
		if(eIndex == COMMERCE_ARCANE) {
			iNewCommerce += (calculateSpellResearchFromFeaturesTimes100() * getTotalCommerceRateModifier(eIndex)) / 100;
		}
	}
...
//		FAssert(m_aiCommerceRate[eIndex] >= 0);	some buildings can give negative commerce Sephi
...
//			FAssert(getBuildingCommerce((CommerceTypes)iI) >= -1); can be negative, modified Sephi
...
int CvCity::getCityBonusCommerceRateModifier(CommerceTypes eIndex) const
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < NUM_COMMERCE_TYPES, "eIndex expected to be < NUM_COMMERCE_TYPES");
	return m_aiCityBonusCommerceRateModifier[eIndex];
}


void CvCity::calculateCityBonusCommerceRateModifier()
{
	CvPlot* pLoopPlot;
	for(int i = 0; i < NUM_COMMERCE_TYPES; ++i) {
		...
		for(int j = 0; j < getNumCityPlots(); ++j) {
			...
			if(pLoopPlot != NULL && pLoopPlot->getWorkingCity() == this) {
				if(pLoopPlot->getBonusType(getTeam()) != NO_BONUS) 
					if(GC.getBonusInfo(pLoopPlot->getBonusType(getTeam())).getCityCommerceModifierType() == i)
						if(pLoopPlot->getImprovementType() != NO_IMPROVEMENT && GC.getImprovementInfo(pLoopPlot->getImprovementType()).isImprovementBonusTrade(pLoopPlot->getBonusType(getTeam())))
							iModifier += GC.getBonusInfo(pLoopPlot->getBonusType(getTeam())).getCityBonusModifier();
			}
		}

		if(iModifier != m_aiCityBonusCommerceRateModifier[i]) {
			m_aiCityBonusCommerceRateModifier[i] = iModifier;
...
{
/*************************************************************************************************/
/**	ADDON (Fix CommerceChangeDoubleTime) Sephi                                     					**/
/**																								**/
/**						                                            							**/
/*************************************************************************************************/
/** Orig
...
**/
	setNumRealBuildingTimed(eIndex, iNewValue, true, getOwnerINLINE(), GC.getGameINLINE().getGameTurn());
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
		...
		if (iChangeNumRealBuilding > 0)
		{
			if (bFirst)
			{
				if (GC.getBuildingInfo(eIndex).isCapital())
				{
					...
					if(getCulture(getOwnerINLINE()) < 500) {
						changeCulture(getOwnerINLINE(), 500 - getCulture(getOwnerINLINE()), true, true);
					}
				}
...
/**	ADDON (Better Traderoutes) Sephi                                                          	**/
/*************************************************************************************************/
    //scale  iTradeProfit back from 100x
    iTradeProfit/=100;
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
  ...
  switch (eOrder)
  {
	case ORDER_TRAIN:
		if (canTrain((UnitTypes)iData1) || bForce)
		{
			...
			if(GC.getUnitInfo((UnitTypes)iData1).isCitySupportNeeded())
				GET_PLAYER(getOwnerINLINE()).changeUnitSupportUsed(1);
			...
			if(getGlobalYieldCost((UnitTypes)iData1) > 0 &&
				GC.getUnitInfo((UnitTypes)iData1).getGlobalYieldTypeCost() != NO_YIELD) {
					if(isHuman()) {
						GET_PLAYER(getOwnerINLINE()).changeGlobalYield(GC.getUnitInfo((UnitTypes)iData1).getGlobalYieldTypeCost(),
							-getGlobalYieldCost((UnitTypes)iData1));
					}
					else {
						YieldTypes eYield = (YieldTypes)GC.getUnitInfo((UnitTypes)iData1).getGlobalYieldTypeCost();
						CvPlayer& pPlayer = GET_PLAYER(getOwnerINLINE());
						int iChange = std::max(pPlayer.getGlobalYield(eYield) - 500, 0);
						iChange = std::min(iChange, getGlobalYieldCost((UnitTypes)iData1));							
						GET_PLAYER(getOwnerINLINE()).changeGlobalYield(eYield, -iChange);
					}			
			}
	...
	case ORDER_CONSTRUCT:
		if (canConstruct((BuildingTypes)iData1) || bForce)
		{
			...
			for(int iI=0;iI<GC.getNumBonusInfos();iI++)
			{
				CvBuildingInfo& kBuilding=GC.getBuildingInfo((BuildingTypes)iData1);
				if(kBuilding.isBonusConsumed(iI))
				{
					GET_PLAYER(getOwnerINLINE()).changeNumBonusesConsumedByProduction((BonusTypes)iI,1);
				}
			}

			if(getGlobalYieldCost((BuildingTypes)iData1) > 0 &&
				GC.getBuildingInfo((BuildingTypes)iData1).getGlobalYieldTypeCost() != NO_YIELD) {
					if(isHuman()) {
						GET_PLAYER(getOwnerINLINE()).changeGlobalYield(GC.getBuildingInfo((BuildingTypes)iData1).getGlobalYieldTypeCost(),
							-getGlobalYieldCost((BuildingTypes)iData1));
					}
					else {
						YieldTypes eYield = (YieldTypes)GC.getBuildingInfo((BuildingTypes)iData1).getGlobalYieldTypeCost();
						CvPlayer& pPlayer = GET_PLAYER(getOwnerINLINE());
						int iChange = std::max(pPlayer.getGlobalYield(eYield) - 500, 0);
						iChange = std::min(iChange, getGlobalYieldCost((BuildingTypes)iData1));							
						GET_PLAYER(getOwnerINLINE()).changeGlobalYield(eYield, -iChange);
					}			
			}
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Start("CvCity::popOrder::finish");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
	...
	case ORDER_TRAIN:
		...
		if(GC.getUnitInfo(eTrainUnit).isCitySupportNeeded())
			GET_PLAYER(getOwnerINLINE()).changeUnitSupportUsed(-1);
		...
		if(!bFinish)
			if(getGlobalYieldCost(eTrainUnit) > 0 &&
				GC.getUnitInfo(eTrainUnit).getGlobalYieldTypeCost() != NO_YIELD)
					GET_PLAYER(getOwnerINLINE()).changeGlobalYield(GC.getUnitInfo(eTrainUnit).getGlobalYieldTypeCost(),
							+getGlobalYieldCost(eTrainUnit));			
...
/*************************************************************************************************/
			...
			pUnit = PlaceUnit(eTrainUnit,getOwnerINLINE(),eTrainAIUnit);		
...
/**	ADDON (New Function Definitions) Sephi                                                      **/
/*************************************************************************************************/
			if (GC.getUnitInfo(eTrainUnit).getPopulationCost()>0)
			{
				changePopulation(-GC.getUnitInfo(eTrainUnit).getPopulationCost());
			}
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
			GC.getTimeMeasure().Start("CvCity::popOrder::finish::python");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
			GC.getTimeMeasure().Stop("CvCity::popOrder::finish::python");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
	...
	case ORDER_CONSTRUCT:
		...
		for(int iI=0;iI<GC.getNumBonusInfos();iI++)
		{
			CvBuildingInfo& kBuilding = GC.getBuildingInfo(eConstructBuilding);
			if(kBuilding.isBonusConsumed(iI))
			{
				GET_PLAYER(getOwnerINLINE()).changeNumBonusesConsumedByProduction((BonusTypes)iI,-1);
			}
		...
		if(!bFinish)
			if(getGlobalYieldCost(eConstructBuilding) > 0 &&
				GC.getBuildingInfo(eConstructBuilding).getGlobalYieldTypeCost() != NO_YIELD)
					GET_PLAYER(getOwnerINLINE()).changeGlobalYield(GC.getBuildingInfo(eConstructBuilding).getGlobalYieldTypeCost(),
							+getGlobalYieldCost(eConstructBuilding));			
...
/*************************************************************************************************/
			...
			//Epic Destiny tag
			if(isWorldWonderClass((BuildingClassTypes)GC.getBuildingInfo(eConstructBuilding).getBuildingClassType()))
			{
				for(int iAdvLoop=0;iAdvLoop<GC.getNumAdventureInfos();iAdvLoop++)
				{
					if(GET_PLAYER(getOwnerINLINE()).isAdventureEnabled(iAdvLoop) && !GET_PLAYER(getOwnerINLINE()).isAdventureFinished(iAdvLoop))
					{
						...
						for(int iJ=0;iJ<GC.getMAX_ADVENTURESTEPS();iJ++)
						{
							if(kAdv.getAdventureStep(iJ)!=NO_ADVENTURESTEP)
							{
								if(GC.getAdventureStepInfo((AdventureStepTypes)kAdv.getAdventureStep(iJ)).isCreateWorldWonder())
								{
									GET_PLAYER(getOwnerINLINE()).changeAdventureCounter(iAdvLoop,iJ,1);
								}
							}
...
/**	ADDON (New Function Definitions) Sephi                                                      **/
/*************************************************************************************************/
			if (GC.getBuildingInfo(eConstructBuilding).getPopulationCost()>0)
			{
				changePopulation(-GC.getBuildingInfo(eConstructBuilding).getPopulationCost());
			}
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
...
/*************************************************************************************************/
            GET_PLAYER(getOwnerINLINE()).changeFaith(-GC.getProjectInfo(eCreateProject).getFaithCost());
            GET_PLAYER(getOwnerINLINE()).changeMana(-GC.getProjectInfo(eCreateProject).getManaCost());
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Stop("CvCity::popOrder::finish");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Start("CvCity::popOrder::choose");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Stop("CvCity::popOrder::choose");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
		...
		/**Sephi - Slower Culture Expansion
			...
			for (iDX = -eCultureLevel; iDX <= eCultureLevel; iDX++)
			{
				for (iDY = -eCultureLevel; iDY <= eCultureLevel; iDY++)
				{
					...
					**/
			for (iDX = -eCultureLevel-1; iDX <= eCultureLevel+1; iDX++)
			{
				for (iDY = -eCultureLevel-1; iDY <= eCultureLevel+1; iDY++)
				{
					iCultureRange = cultureDistance(iDX, iDY);

					if (iCultureRange <= getCultureRadius(eCultureLevel))
					{
					/** end modified **/
						pLoopPlot = plotXY(getX_INLINE(), getY_INLINE(), iDX, iDY);
...
//								pLoopPlot->changeCulture(ePlayer, (((eCultureLevel - iCultureRange) * iFreeCultureRate) + iCultureRate + 1), (bUpdate || !(pLoopPlot->isOwned())));
								pLoopPlot->changeCulture(ePlayer, (((getCultureRadius(eCultureLevel) - iCultureRange) * iFreeCultureRate) + iCultureRate + 1), (bUpdate || !(pLoopPlot->isOwned())));
							}
						}
					}
				}
				...
				while (pOrderNode != NULL)
				{
					if (pOrderNode->m_data.eOrderType == ORDER_TRAIN)
					{
						if (pOrderNode->m_data.iData1 == iI)
						{
							...
							if(GC.getUnitInfo((UnitTypes)pOrderNode->m_data.iData1).isCitySupportNeeded())
								GET_PLAYER(getOwnerINLINE()).changeUnitSupportUsed(-1);
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Start("CvCity::popOrder");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
			popOrder(0, true, true);
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Stop("CvCity::popOrder");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/

			//to eliminate pre-build exploits for all Wonders and all Projects
			if (isProductionWonderOrWorldUnit() || isProductionProject())
			{
				break;
			}
            ...
            // Sephi modified to prevent unlimited construction of item under very rare conditions
            // iOverflowProductionModified = getOverflowProductionDifference();

			if(iOverflowProductionModified!=0)
			{
				iOverflowProductionModified = std::min(getOverflowProductionDifference(),iOverflowProductionModified-20);
			}
			else
			{
				iOverflowProductionModified = getOverflowProductionDifference();
...
void CvCity::doReligion()
{
	...
	//Darksavant
	//Letting religions spread even if there is already one in the city, just at a reduced rate
	//if (getReligionCount() == 0)
	//{
		for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
		{
...
//FfH: End Add
										...
										//Darksavant
										//Dramatically reducing the spread chance based on how many religions are already present
										if (getReligionCount() != 0)
										{
											iSpread /= ((getReligionCount() + 1) * (getReligionCount() + 1));
										}
...
//	changeGreatPeopleProgress(getGreatPeopleRate());
	int iThreshold = getGreatPeopleThreshold() + GET_PLAYER(getOwnerINLINE()).greatPeopleThreshold(false);
	int iModifier = getTotalGreatPeopleRateModifier();
	int iRate;
	for (int iI = 0; iI < GC.getNumUnitInfos(); iI++)
	{
		iRate = getGreatPeopleUnitRate((UnitTypes)iI);
		if(iRate > 0) {
			changeGreatPeopleUnitProgress((UnitTypes)iI, (iRate * iModifier) / 100);
	
			if (getGreatPeopleUnitProgress((UnitTypes)iI) >= iThreshold)
			{
				//changeGreatPeopleProgress(-(GET_PLAYER(getOwnerINLINE()).greatPeopleThreshold(false)));

				setGreatPeopleUnitProgress(((UnitTypes)iI), 0);
				createGreatPeople((UnitTypes)iI, true, false);
			}
		}
	}
}
...
void CvCity::read(FDataStreamBase* pStream)
{
	...
	pStream->Read(&m_iGreatPeopleThreshold);
	...
	pStream->Read(&m_iImprovementHealthChange);
	...
	pStream->Read(&m_bDisableGrowth);
	pStream->Read(&m_bDisableProduction);
...
/**	ADDON (New Function Definitions) Sephi                                                      **/
/*************************************************************************************************/
	pStream->Read(&m_bDistrict);
	pStream->Read(&m_iImprovementInfrastructureCost);
	pStream->Read(&m_iMistDensity);
	pStream->Read(&m_iTaxesModifier);
	pStream->Read(&m_iGPPThresholdReductionPercent);
	pStream->Read(&m_iPopulationLimit);
	pStream->Read(&m_iTotalSpecializationLevel);
	pStream->Read(&m_iInfrastructureLevel);
	pStream->Read(&m_iCommercePercentTypeForced);
	pStream->Read(&m_iUnitSupport);
	pStream->Read(&m_iCultureFromImprovements);
	pStream->Read(GC.getNumBuildingClassInfos(), m_paiBuildingClassBlocked);
	pStream->Read(NUM_CITYSPECIALIZATION_TYPES, m_paiCitySpecialization);
	pStream->Read(NUM_CITYSPECIALIZATION_TYPES, m_paiCitySpecializationLevel);
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
/*************************************************************************************************/
...
/*************************************************************************************************/
	...
	pStream->Read(NUM_COMMERCE_TYPES, m_paiCommercePerPopulation);
	pStream->Read(NUM_COMMERCE_TYPES, m_paiCommercePerCultureLevel);
	pStream->Read(NUM_YIELD_TYPES, m_paiYieldPerCultureLevel);
	pStream->Read(NUM_YIELD_TYPES, m_paiYieldIncome);
	pStream->Read(GC.getNumBonusInfos(), m_paiYieldFromBonus);
	pStream->Read(GC.getNumBonusInfos(), m_paiCommerceFromBonus);
	pStream->Read(NUM_COMMERCE_TYPES, m_paiCommerceFromUnits);
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
	...
	pStream->Read(NUM_YIELD_TYPES, m_aiCityBonusYieldRateModifier);
	...
	pStream->Read(NUM_COMMERCE_TYPES, m_aiCityBonusCommerceRateModifier);
...
/*************************************************************************************************/
	...
	for (int i = 0; i < iNumElts; ++i)
	{
		...
		kChange.read(pStream);
		m_aBuildingBonusChange.push_back(kChange);
	}
/*************************************************************************************************/
/**	END Change Building Bonuses	                                        						**/
/*************************************************************************************************/
...
void CvCity::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(m_iGreatPeopleThreshold);
	...
	pStream->Write(m_iImprovementHealthChange);
	...
	pStream->Write(m_bDisableGrowth);
	pStream->Write(m_bDisableProduction);
...
/**	ADDON (New Function Definitions) Sephi                                                      **/
/*************************************************************************************************/
	pStream->Write(m_bDistrict);
	pStream->Write(m_iImprovementInfrastructureCost);
	pStream->Write(m_iMistDensity);
	pStream->Write(m_iTaxesModifier);
	pStream->Write(m_iGPPThresholdReductionPercent);
	pStream->Write(m_iPopulationLimit);
	pStream->Write(m_iTotalSpecializationLevel);
	pStream->Write(m_iInfrastructureLevel);
	pStream->Write(m_iCommercePercentTypeForced);
	pStream->Write(m_iUnitSupport);
	pStream->Write(m_iCultureFromImprovements);
	pStream->Write(GC.getNumBuildingClassInfos(), m_paiBuildingClassBlocked);
	pStream->Write(NUM_CITYSPECIALIZATION_TYPES, m_paiCitySpecialization);
	pStream->Write(NUM_CITYSPECIALIZATION_TYPES, m_paiCitySpecializationLevel);
...
/*************************************************************************************************/
	...
	pStream->Write(NUM_COMMERCE_TYPES, m_paiCommercePerPopulation);
	pStream->Write(NUM_COMMERCE_TYPES, m_paiCommercePerCultureLevel);
	pStream->Write(NUM_YIELD_TYPES, m_paiYieldPerCultureLevel);
	pStream->Write(NUM_YIELD_TYPES, m_paiYieldIncome);
	pStream->Write(GC.getNumBonusInfos(), m_paiYieldFromBonus);
	pStream->Write(GC.getNumBonusInfos(), m_paiCommerceFromBonus);
	pStream->Write(NUM_COMMERCE_TYPES, m_paiCommerceFromUnits);
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
	...
	pStream->Write(NUM_YIELD_TYPES, m_aiCityBonusYieldRateModifier);
	...
	pStream->Write(NUM_COMMERCE_TYPES, m_aiCityBonusCommerceRateModifier);
...
/**						                                            							**/
/*************************************************************************************************/
	pStream->Write(m_aBuildingBonusChange.size());
	for (std::vector<BuildingBonusChange>::iterator it = m_aBuildingBonusChange.begin(); it != m_aBuildingBonusChange.end(); ++it)
	{
		(*it).write(pStream);
	}
/*************************************************************************************************/
...
bool CvCity::isValidBuildingLocation(BuildingTypes eBuilding) const
{
	...
	int iBuildingClass=kBuilding.getPrereqBuildingClass();
	...
	iBuildingClass=kBuilding.getPrereqBuildingClassInAllCities();
	...
	if(iBuildingClass!=NO_BUILDINGCLASS)
	{
		...
		{
			bool bValid=false;
			//do not check if City cannot have Buildingclass because of Civilizationtype
			if(pLoopCity!=this && GC.getCivilizationInfo(pLoopCity->getCivilizationType()).getCivilizationBuildings(iBuildingClass)==NO_BUILDING)
			{
				continue;
			}
	...
	{
		if(!plot()->isNearUF(getPlotRadius())) {
			return false;
		}
	}
	...
	if(kBuilding.getPrereqImprovement() != NO_IMPROVEMENT) {
		bool bValid = false;
		ImprovementTypes eImprovement = (ImprovementTypes)kBuilding.getPrereqImprovement();
		for (int iI = 0; iI < getNumCityPlots(); iI++) {
			CvPlot* pLoopPlot = getCityIndexPlot(iI);
	...
	//Does the City have a Global Yield Income?
	if(kBuilding.isReqGlobalYieldIncome())
	{
		for(int iI=YIELD_COMMERCE+1;iI<GC.getNUM_YIELD_TYPES();iI++)
		{
			if(kBuilding.getYieldModifier(iI)>0)
			{
				if(getYieldIncome(iI)<=0)
				{
					return false;
				}
			}
...
//				GET_PLAYER(getOwnerINLINE()).initUnit(eUnit, getX_INLINE(), getY_INLINE());
                CvUnit* pUnit = PlaceUnit(eUnit,getOwnerINLINE(),NO_UNITAI);
...
void CvCity::setBuildingYieldChange(BuildingClassTypes eBuildingClass, YieldTypes eYield, int iChange)
{
		...
		if ((*it).eBuildingClass == eBuildingClass && (*it).eYield == eYield)
		{
			...
			if (iOldChange != iChange)
			{
				...
				if (NO_BUILDING != eBuilding)
				{
					if (getNumActiveBuilding(eBuilding) > 0)
					{
						changeBaseYieldRate(eYield, (iChange - iOldChange) * getNumActiveBuilding(eBuilding), true);
					}
				}
			}
	...
	if (0 != iChange)
	{
		...
		if (NO_BUILDING != eBuilding)
		{
			if (getNumActiveBuilding(eBuilding) > 0)
			{
				changeBaseYieldRate(eYield, iChange * getNumActiveBuilding(eBuilding), true);
			}
		}
	}
}
...
bool CvCity::isAutoRaze() const
{
	...
	//City States cannot be conquered
	if(getPreviousOwner()!=NO_PLAYER)
	{
		if(GET_PLAYER(getPreviousOwner()).isCityState())
		{
			return true;
		}
	}
	//city States cannot capture
	if(GET_PLAYER(getOwnerINLINE()).isCityState())
	{
		return true;
	}
...
*/
					if (GC.getGameINLINE().getSorenRandNum(100, "Religion Adoption") < GC.getDefineINT("RELIGION_ADOPTION_CHANCE"))
					{
						aeAdoptableReligions.push_back((ReligionTypes)iI);
...
{
	int iCrime = GC.getGameINLINE().getCrime() + m_iCrime + GET_PLAYER(getOwnerINLINE()).getCrime();
    if (iCrime < 0)
    {
        return 0;
    }
...
void CvCity::swapAustrinCities(CvPlot* pNewPlot)
{
    if(pNewPlot==NULL || !pNewPlot->isCity())
    {
        return;
    }

    CvCity* pCity2=pNewPlot->getPlotCity();
    int iPoptemp=getPopulation();
    setPopulation(pCity2->getPopulation());
    pCity2->setPopulation(iPoptemp);

    int iNumBuilding;
    for(int iI=0;iI<GC.getNumBuildingInfos();iI++)
    {
        iNumBuilding=getNumRealBuilding((BuildingTypes)iI);
        setNumRealBuilding((BuildingTypes)iI,pCity2->getNumRealBuilding((BuildingTypes)iI));
        pCity2->setNumRealBuilding((BuildingTypes)iI,iNumBuilding);
    }

}

bool CvCity::isHasDistrict() const
{
	return m_bDistrict;
}

void CvCity::setHasDistrict(bool bNewValue)
{
	m_bDistrict=bNewValue;
}

int CvCity::getMistDensity() const
{
    return m_iMistDensity;
}

void CvCity::changeMistDensity(int iChange)
{
    m_iMistDensity += iChange;
}

int CvCity::getTaxesModifier() const
{
    return m_iTaxesModifier;
}

void CvCity::changeTaxesModifier(int iChange)
{
    m_iTaxesModifier += iChange;
}

int CvCity::getGPPThresholdReductionPercent() const
{
    return m_iGPPThresholdReductionPercent;
}

void CvCity::changeGPPThresholdReductionPercent(int iChange)
{
    m_iGPPThresholdReductionPercent += iChange;
}

int CvCity::getBuildingClassBlocked(int iBuildingClass) const
{
	return m_paiBuildingClassBlocked[iBuildingClass];
}

bool CvCity::isBuildingClassBlocked(int iBuildingClass) const
{
	return (m_paiBuildingClassBlocked[iBuildingClass]>0);
}

void CvCity::changeBuildingClassBlocked(int iBuildingClass, int iChange)
{
	m_paiBuildingClassBlocked[iBuildingClass]+=iChange;
}

int CvCity::getCitySpecialization(int iType) const
{
	return m_paiCitySpecialization[iType];
}

void CvCity::changeCitySpecialization(int iType, int iChange)
{
	m_paiCitySpecialization[iType]+=iChange;
	//Disable CitySpecialization
	return;
	//Disable CitySpecialization
	while(getCitySpecialization(iType)>=getCitySpecializationThreshold(iType))
	{
		m_paiCitySpecialization[iType]-=getCitySpecializationThreshold(iType);
		changeCitySpecializationLevel(iType,1);
	}
}

int CvCity::getCitySpecializationLevel(int iType) const
{
	//Disable CitySpecialization
	return 0;
	//Disable CitySpecialization
	return m_paiCitySpecializationLevel[iType];
}

void CvCity::changeCitySpecializationLevel(int iType, int iChange)
{
	if(iChange!=0)
	{
		m_paiCitySpecializationLevel[iType]+=iChange;
		changeTotalSpecializationLevel(iChange);
		if(iType==CITYSPECIALIZATION_GOLD)
		{
			changeCommerceRateModifier(COMMERCE_GOLD, GC.getDefineINT("GOLD_PER_SPECIALIZATION_LEVEL") * iChange);
		}
		if(iType==CITYSPECIALIZATION_RESEARCH)
		{
			changeCommerceRateModifier(COMMERCE_RESEARCH, GC.getDefineINT("RESEARCH_PER_SPECIALIZATION_LEVEL") * iChange);
		}
		if(iType==CITYSPECIALIZATION_CULTURE)
		{
			changeCommerceRateModifier(COMMERCE_CULTURE, GC.getDefineINT("CULTURE_PER_SPECIALIZATION_LEVEL") * iChange);
		}
		if(iType==CITYSPECIALIZATION_ARCANE)
		{
			changeCommerceRateModifier(COMMERCE_ARCANE, GC.getDefineINT("SPELLRESEARCH_PER_SPECIALIZATION_LEVEL") * iChange);
		}
		if(iType==CITYSPECIALIZATION_FAITH)
		{
			changeCommerceRateModifier(COMMERCE_FAITH, GC.getDefineINT("FAITH_PER_SPECIALIZATION_LEVEL") * iChange);
		}
		if(iType==CITYSPECIALIZATION_MILITARY)
		{
			changeFreeExperience(GC.getDefineINT("FREE_EXPERIENCE_PER_SPECIALIZATION_LEVEL")* iChange);
		}
		if(iType==CITYSPECIALIZATION_METAL)
		{
			changeYieldRateModifier(YIELD_METAL,GC.getDefineINT("GLOBALYIELD_PER_SPECIALIZATION_LEVEL")* iChange);
		}
		if(iType==CITYSPECIALIZATION_LUMBER)
		{
			changeYieldRateModifier(YIELD_LUMBER,GC.getDefineINT("GLOBALYIELD_PER_SPECIALIZATION_LEVEL")* iChange);
		}
		if(iType==CITYSPECIALIZATION_LEATHER)
		{
			changeYieldRateModifier(YIELD_LEATHER,GC.getDefineINT("GLOBALYIELD_PER_SPECIALIZATION_LEVEL")* iChange);
		}
		if(iType==CITYSPECIALIZATION_STONE)
		{
			changeYieldRateModifier(YIELD_STONE,GC.getDefineINT("GLOBALYIELD_PER_SPECIALIZATION_LEVEL")* iChange);
		}
	}
}

int CvCity::getTotalSpecializationLevel() const
{
	return m_iTotalSpecializationLevel;
}

void CvCity::changeTotalSpecializationLevel(int iChange)
{
	m_iTotalSpecializationLevel+=iChange;
}

int CvCity::getInfrastructureLevel() const
{
	return m_iInfrastructureLevel;
}

void CvCity::changeInfrastructureLevel(int iChange)
{
	m_iInfrastructureLevel+=iChange;
}

int CvCity::getCommercePercentTypeForced() const
{
	return m_iCommercePercentTypeForced;
}

void CvCity::setCommercePercentTypeForced(CommerceTypes iNewvalue)
{
	m_iCommercePercentTypeForced=iNewvalue;
}

int CvCity::getCityCommercePercent(CommerceTypes eCommerceType) const
{
	if (getCommercePercentTypeForced()!=NO_COMMERCE)
	{
		if(getCommercePercentTypeForced()==eCommerceType)
		{
			return 100;
		}
		else
	...
	return GET_PLAYER(getOwnerINLINE()).getCommercePercent(eCommerceType);
}
int CvCity::getUnitSupport() const
{
	return m_iUnitSupport;
}

void CvCity::changeUnitSupport(int iChange)
{
	m_iUnitSupport+=iChange;
}

int CvCity::getCultureFromImprovements() const
{
	return m_iCultureFromImprovements;
}

void CvCity::setCultureFromImprovements(int iChange)
{
	m_iCultureFromImprovements = iChange;
}

int CvCity::getImprovementInfrastructureCost() const
{
    return m_iImprovementInfrastructureCost;
}

void CvCity::changeYieldFromBonus(int iYield,int iChange)
{
	if(iChange!=0)
	{
		int iOldValue=m_paiYieldFromBonus[iYield];
		int iNewValue=iOldValue+iChange;
		m_paiYieldFromBonus[iYield]+=iChange;
		changeBaseYieldRate((YieldTypes)iYield,-iOldValue/100);
		changeBaseYieldRate((YieldTypes)iYield,iNewValue/100);
	}
}

int CvCity::getYieldFromBonus(int iYield) const
{
    return m_paiYieldFromBonus[iYield];
}

void CvCity::changeCommerceFromBonus(int iCommerce,int iChange)
{
    m_paiCommerceFromBonus[iCommerce] += iChange;
	FAssertMsg(m_paiCommerceFromBonus[iCommerce] >= 0, "CommerceFromBonus expected to be >= 0");
}

int CvCity::getCommerceFromBonus(int iCommerce) const
{
    return m_paiCommerceFromBonus[iCommerce];
}

int CvCity::getCommerceFromUnits(int iCommerce) const
{
    return m_paiCommerceFromUnits[iCommerce];
}

void CvCity::updateCommerceFromUnits()
{
	for(int i = 0; i < NUM_COMMERCE_TYPES; ++i) {
		m_paiCommerceFromUnits[i] = 0;
	}
	...
	if(pPlot->getNumUnits() > 0)
	{
        ...
        while (pUnitNode != NULL)
        {
            ...
            {
				for(int i = 0; i < NUM_COMMERCE_TYPES; ++i) {
					m_paiCommerceFromUnits[i] += pLoopUnit->getUnitInfo().getCityCommerceChanges((CommerceTypes)i) * 100;
				}
			}
		}
	}
...
int CvCity::calculateImprovementInfrastructureCost(ImprovementTypes eImprovement)
{
	if(GC.getImprovementInfo(eImprovement).getImprovementClassType()!=0)
	{
		return 0;
	}
	...
	int iBaseMod=2;
	int iCivicMod=0;
	int iCivilizationMod=0;

	ImprovementTypes eStartImprovement=eImprovement;
	bool bValid=true;
	while(bValid)
	{
		bValid=false;
		...
		{
			if(GC.getImprovementInfo((ImprovementTypes)iJ).getImprovementUpgrade()==eStartImprovement)
			{
				eStartImprovement=(ImprovementTypes)iJ;
				bValid=true;
				break;
			}
		}
	...
	{
		if(GC.getCivicInfo((CivicTypes)iJ).getImpInfrastructureHalfCost()==eStartImprovement)
		{
			if(kPlayer.getCivics((CivicOptionTypes)GC.getCivicInfo((CivicTypes)iJ).getCivicOptionType())==iJ)
			{
				iCivicMod++;
			}
		}
	...
	if(GC.getCivilizationInfo(getCivilizationType()).getImpInfrastructureHalfCost()==eStartImprovement)
	{
		iCivilizationMod++;
	}

	iAmount/=(iBaseMod+iCivicMod+iCivilizationMod);

	return iAmount;
}

int CvCity::calculateImprovementInfrastructureCostFreeBase()
{
	//TEMP: DISABLE IMPROVEMENT LIMITS
	return 10000;

	if (isSettlement())
	{
		return 300 + GET_PLAYER(getOwnerINLINE()).getExtraImprovementPerCity() * 100;
	}
	return 100 * (getPlotRadius() * 3 + getInfrastructureLevel()
					+ GET_PLAYER(getOwnerINLINE()).getExtraImprovementPerCity());
}

int CvCity::calculateImprovementInfrastructureCostFreeCulture()
{
	return 0;
//	return getCultureLevel()*100;
}

int CvCity::calculateImprovementInfrastructureCostFreePopulation()
{
	return 0;
//	return std::max(0,(getPopulation()-5)*20);
}

int CvCity::calculateImprovementInfrastructureCostFreeModifier()
{
	return 100;
	//How many potential plots do we have?
	int iPotential=0;
	for (int iI = 0; iI < getNumCityPlots(); iI++)
	{
		CvPlot* pLoopPlot = getCityIndexPlot(iI);
	...
	return (100*iPotential)/21;
}

//returns number of Free Improvements times 100
int CvCity::calculateImprovementInfrastructureCostFree()
{
	int iFreeImpsPrim=calculateImprovementInfrastructureCostFreeBase();
	iFreeImpsPrim+=calculateImprovementInfrastructureCostFreePopulation();
	iFreeImpsPrim+=calculateImprovementInfrastructureCostFreeCulture();
		
	iFreeImpsPrim=(iFreeImpsPrim*calculateImprovementInfrastructureCostFreeModifier())/100;

	return iFreeImpsPrim;
}

//returns how many Free Improvements can still be build (times 100)
int CvCity::calculateImprovementInfrastructureCostFreeLeft()
{
	int iCountPrim=0;
	...
	int iFavoredImpCount=0;
	int iFreeFavored=GC.getDefineINT("FREE_FAVORED_IMPROVEMENTS_PER_CITY");
	for (int iI = 0; iI < getNumCityPlots(); iI++)
	{
		CvPlot* pLoopPlot = getCityIndexPlot(iI);
		...
		if (pLoopPlot != NULL)
		{
			if(pLoopPlot->getWorkingCity()==this)
			{
				...
				{
					if(GC.getImprovementInfo(pLoopPlot->getImprovementType()).getImprovementClassType()==0)
					{
						if(iFavoredImpCount<iFreeFavored && pLoopPlot->isImprovementOrUpgrade(iFavoredImp))
						{
							iFavoredImpCount++;
						}
						else
						{
							iCountPrim+=calculateImprovementInfrastructureCost(pLoopPlot->getImprovementType());
						}
					}
				}
			}
	...
	return std::max(0,calculateImprovementInfrastructureCostFree()-iCountPrim);
}

int CvCity::calculateImprovementInfrastructureCost(int iNextImprovementType)
{
	//Count the Improvements adjacent to the City
	int iCountPrim=0;
	int iFavoredImp=GC.getCivilizationInfo(GET_PLAYER(getOwnerINLINE()).getCivilizationType()).getImpInfrastructureHalfCost();
	int iCountFav=0;
		...
		if (pLoopPlot != NULL)
		{
			if(pLoopPlot->getWorkingCity()==this)
			{
				...
				if(eImprovement!=NO_IMPROVEMENT)
				{
					if(GC.getImprovementInfo(eImprovement).getImprovementClassType()==0)
					{
						if(pLoopPlot->isImprovementOrUpgrade(eImprovement) && iCountFav<GC.getDefineINT("FREE_FAVORED_IMPROVEMENTS_PER_CITY"))
						{
							iCountFav++;
						}
						else
	...
	int iFreeImpsPrim=calculateImprovementInfrastructureCostFree();

	if(iNextImprovementType!=NO_IMPROVEMENT)
	{
		int iTemp=-calculateImprovementInfrastructureCost(iCountPrim,iFreeImpsPrim);
		if(GC.getImprovementInfo((ImprovementTypes)iNextImprovementType).getImprovementClassType()==0)
		{
			iCountPrim+=calculateImprovementInfrastructureCost((ImprovementTypes)iNextImprovementType);
		}

		iTemp+=calculateImprovementInfrastructureCost(iCountPrim,iFreeImpsPrim);

		return std::max(0,iTemp);
	}

	return calculateImprovementInfrastructureCost(iCountPrim,iFreeImpsPrim);
}

int CvCity::calculateImprovementInfrastructureCost(int iCount, int iFreeImps)
{
	int iCost=std::max(0,(iCount-iFreeImps));
	iCost*=iCost*3;
...
void CvCity::updateImprovementInfrastructureCost()
{
	int iNewInfrastructreCost=calculateImprovementInfrastructureCost();

	//remove Improvements, if City can't support Improvements
	if(iNewInfrastructreCost > 0)
	{
		int iBestValue = 0;
		int iValue = 0;
        ...
        for (int iI = 0; iI < getNumCityPlots(); iI++)
        {
            if (iI != CITY_HOME_PLOT)
            {
                ...
                {
					if(pPlot->getImprovementType()!=NO_IMPROVEMENT && GC.getImprovementInfo(pPlot->getImprovementType()).getImprovementClassType()>=0)
					{
						iValue=calculateImprovementInfrastructureCost(pPlot->getImprovementType());
						...
						{
							if(getNumFavoredImprovements()<=GC.getDefineINT("FREE_FAVORED_IMPROVEMENTS_PER_CITY"))
							{
								iValue/=20;
							}
						}
		...
		if(pBestPlot!=NULL)
		{
			if(pBestPlot->isOwned() && pBestPlot->getImprovementType()!=NO_IMPROVEMENT)
			{
				CvWString szMessage;
				szMessage = gDLL->getText("TXT_KEY_IMPROVEMENT_AUTO_RAZE", getNameKey());
				gDLL->getInterfaceIFace()->addMessage(pBestPlot->getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szMessage, 0, MESSAGE_TYPE_MINOR_EVENT, GC.getImprovementInfo(pBestPlot->getImprovementType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pBestPlot->getX_INLINE(), pBestPlot->getY_INLINE(), false);
			}
...
void CvCity::changePopulationLimit(int iChange)
{
    m_iPopulationLimit += iChange;
}

bool CvCity::canHaveReligion(ReligionTypes eReligion)
{
...
//FfH: End Add
	...
	if(GET_PLAYER(getOwnerINLINE()).isCityState())
	{
		if(eReligion!=GC.getCityStateInfo((CityStateTypes)GET_PLAYER(getOwnerINLINE()).getCityStateType()).getReligionType())
		{
			return false;
		}
	}
	...
	argsList.add((int)getCivilizationType());

	long lResult=1;
	gDLL->getPythonIFace()->callFunction(PYGameModule, "canCityHaveReligion", argsList.makeFunctionArgs(), &lResult);
...
int CvCity::calculateTradeYieldTimes100(YieldTypes eIndex, int iTradeProfit) const
{
	if ((iTradeProfit > 0) && (GET_PLAYER(getOwnerINLINE()).getTradeYieldModifier(eIndex) > 0))
	{
		return ((iTradeProfit * GET_PLAYER(getOwnerINLINE()).getTradeYieldModifier(eIndex)));
	}
	else
	{
		return 0;
...
UnitArtStyleTypes CvCity::getUnitArtStyleType() const
{
	...
	{
		iUnitArtStyle = GC.getCivilizationInfo(GET_PLAYER(getOwnerINLINE()).getCivilizationType()).getUnitArtStyleType();
	}
	else
	{
...
int CvCity::getAdditionalHappinessByBuilding(BuildingTypes eBuilding, int& iGood, int& iBad) const
{
	...
	//Count Global Happiness in this City
...
// added Sephi
	...
	int iUnhappyProd=0;
	if (eIndex==YIELD_PRODUCTION && GC.getBuildingInfo(eBuilding).isUnhappyProduction())
    {
        iUnhappyProd += unhappyLevel(0)/2;
...
// added Sephi
	int iExtra = ((iRate + iUnhappyProd +getAdditionalBaseYieldRateByBuilding(eIndex, eBuilding)) * (iModifier + getAdditionalYieldRateModifierByBuilding(eIndex, eBuilding)) / 100) - (iRate * iModifier / 100);

	//added Sephi
	//ExtraYields from City
	if(eIndex>YIELD_COMMERCE)
	{
		iExtra+=(GC.getBuildingInfo(eBuilding).getYieldModifier(eIndex)*calculateCityYieldIncome(eIndex))/100;
	}
...
//		if (iPlayerTradeYieldModifier > 0 && (kBuilding.getTradeRouteModifier() != 0 || kBuilding.getForeignTradeRouteModifier() != 0)) modified Sephi
		if (iPlayerTradeYieldModifier > 0 && (kBuilding.getTradeRouteModifier() != 0 || kBuilding.getForeignTradeRouteModifier() != 0 || kBuilding.getTradeRoutes()!= 0))
		{
...
// BUG - Fractional Trade Routes - end
			...
			for (int iI = 0; iI < getTradeRoutes(); ++iI)
			{
				...
				if (pCity)
				{
					...
					int iNewTradeYield = iTradeProfit * iTradeModifier / iTradeProfitDivisor * iPlayerTradeYieldModifier;
					iNewTotalTradeYield += iNewTradeYield;
				}
			}
			//added Sephi
			if(kBuilding.getTradeRoutes()!=0 && iWorstTradeRouteProfit!=MAX_INT)
			{
				int iNumTradeRouteChange=kBuilding.getTradeRoutes();
...
// BUG - Fractional Trade Routes - end
		...
		//Yield from Bonuses
		if(kBuilding.getBonusYieldType()==eIndex)
		{
			int iCountBonuses=0;
			for(int iI=0;iI<GC.getNumBonusInfos();iI++)
			{
				...
				{
					iCountBonuses+=getNumBonuses((BonusTypes)iI);
				}
			}
			if(iCountBonuses>0)
			{
				iExtraRate+=(kBuilding.getInitialValue()+kBuilding.getPerBonusValue()*iCountBonuses)/100;
			}
		}
		//Yield from Culture
		iExtraRate+=kBuilding.getYieldPerCultureLevel(eIndex)*getCultureLevel();		
	}
...
int CvCity::getAdditionalYieldRateModifierByBuilding(YieldTypes eIndex, BuildingTypes eBuilding) const
{
	...
	if (!bObsolete)
	{
		...
		//Sephi, only count general Bonuses
		/**
...
int CvCity::getAdditionalBaseCommerceRateByBuildingImpl(CommerceTypes eIndex, BuildingTypes eBuilding) const
{
	...
	if (!bObsolete)
	{
		...
		//Commerce from Bonuses
		if(kBuilding.getBonusCommerceType()==eIndex)
		{
			int iCountBonuses=0;
			for(int iI=0;iI<GC.getNumBonusInfos();iI++)
			{
		...
		//Commerce from Culture
		iExtraRate += kBuilding.getCommercePerCultureLevel(eIndex)*getCultureLevel();		
		iExtraRate += kBuilding.getCommercePerPopulation(eIndex) * getPopulation();		
		// Commerce from YIELD_COMMERCE
		if(GET_PLAYER(getOwnerINLINE()).getCommercePercent(eIndex)>0
		|| getCommercePercentTypeForced()==eIndex)
		{
			int iRate = getBaseYieldRate(YIELD_COMMERCE);
			int iModifier = getBaseYieldRateModifier(YIELD_COMMERCE);
			int iCommercechange = ((iRate + getAdditionalBaseYieldRateByBuilding(YIELD_COMMERCE, eBuilding)) * (iModifier + getAdditionalYieldRateModifierByBuilding(YIELD_COMMERCE, eBuilding)) / 100) - (iRate * iModifier / 100);
			
			if(getCommercePercentTypeForced()!=NO_COMMERCE)
			{
				if(getCommercePercentTypeForced()==eIndex)
				{
					iExtraRate+=iCommercechange;
				}
			}
			else
			{
				iExtraRate+=(iCommercechange * GET_PLAYER(getOwnerINLINE()).getCommercePercent(eIndex))/100;
			}
		}
		//Commerce Change because of CommercePercentTypeForced Property
		if(kBuilding.getCommercePercentTypeForced()!=NO_COMMERCE)
		{
			int iRate = getBaseYieldRate(YIELD_COMMERCE);
			int iModifier = getBaseYieldRateModifier(YIELD_COMMERCE);
			int iCityCommerceRate = ((iRate + getAdditionalBaseYieldRateByBuilding(YIELD_COMMERCE, eBuilding)) * (iModifier + getAdditionalYieldRateModifierByBuilding(YIELD_COMMERCE, eBuilding)) / 100);

			if(kBuilding.getCommercePercentTypeForced()==eIndex)
			{
				iExtraRate+=((100-getCityCommercePercent(eIndex))*iCityCommerceRate)/100;
			}
			else
			{
				iExtraRate-=(getCityCommercePercent(eIndex)*iCityCommerceRate)/100;
			}
		}
	}
...
bool CvCity::isDisableGrowth() const
{
	return m_bDisableGrowth;
}

bool CvCity::isDisableProduction() const
{
	return m_bDisableProduction;
}
...
int CvCity::getSavedMaintenanceTimes100ByBuilding(BuildingTypes eBuilding) const
{
	...
	int iBuildingMaintenance=(kBuilding.getBuildingMaintenance() * (100 + getMaintenanceModifier())) / 100;

	iValue-=iBuildingMaintenance;
	...
	if (iModifier != 0 && !isDisorder() && !isWeLoveTheKingDay() && (getPopulation() > 0))
	{
		...
		iValue+= std::max(0, getMaintenanceTimes100() - iNewMaintenance);
	}
...
/**                                                                                              */
/*************************************************************************************************/
bool CvCity::isProductionWonderOrWorldUnit() const
{
	CLLNode<OrderData>* pOrderNode = headOrderQueueNode();
...
//added Sephi
		if (pOrderNode->m_data.eOrderType == ORDER_TRAIN)
		{
			UnitTypes eUnit = (UnitTypes)(pOrderNode->m_data.iData1);
			UnitClassTypes eUnitClass = (UnitClassTypes)(GC.getUnitInfo(eUnit).getUnitClassType());
			return (isWorldUnitClass(eUnitClass));
		}
...
int CvCity::getTrainXPCap(int iI)
{
    ...
    return std::max(0, m_paiTrainXPCap[iI]);
}
...
int CvCity::getTrainXPRate(int iI)
{
	if (iI < 0 || iI > GC.getNumUnitCombatInfos())
	{
		return 0;
...
void CvCity::changeTrainXPRate(int iChange, int iI)
{
	if (iI >= 0 && iI <= GC.getNumUnitCombatInfos())
	{
...
int CvCity::getCommercePerPopulation(int iCommerce) const
{
	return m_paiCommercePerPopulation[iCommerce];
}
	
void CvCity::changeCommercePerPopulation(int iChange, int iCommerce)
{
	m_paiCommercePerPopulation[iCommerce]+=iChange;
}

int CvCity::getCommercePerCultureLevel(int iCommerce) const
{
	return m_paiCommercePerCultureLevel[iCommerce];
}
	
void CvCity::changeCommercePerCultureLevel(int iChange, int iCommerce)
{
	m_paiCommercePerCultureLevel[iCommerce]+=iChange;
}
int CvCity::getYieldPerCultureLevel(int iYield)
{
	return m_paiYieldPerCultureLevel[iYield];
}	
void CvCity::changeYieldPerCultureLevel(int iChange, int iYield)
{
	if(iChange!=0)
	{
		m_paiYieldPerCultureLevel[iYield]+=iChange;
		changeBaseYieldRate((YieldTypes)iYield,getCultureLevel()*iChange);
	}
}


int CvCity::getYieldIncome(int iI) const
{
	if (iI < 0 || iI > GC.getNUM_YIELD_TYPES())
	{
		return 0;
	}
    return std::max(0, m_paiYieldIncome[iI]);
}

int CvCity::calculateCityYieldIncome(int iYield) const
{
	int iIncome=0;
	CvPlot* pLoopPlot;
	...
	for (int iI = 0; iI < getNumCityPlots(); iI++)
	{
		...
		if (pLoopPlot != NULL)
		{
			if (pLoopPlot->getWorkingCity() == this)
			{
				if(pLoopPlot->getImprovementType() != NO_IMPROVEMENT && pLoopPlot != plot() && isWorkingPlot(iI))
				{
					iIncome += pLoopPlot->getYield((YieldTypes)iYield);
				}
			}
		}
	}

	iIncome += getBaseYieldRate((YieldTypes)iYield);

	return iIncome;
}

//if iExtra is not equal zero returns the change that iExtra causes
int CvCity::getExtraYieldIncome(int iYield, int iExtra)
{
	//disabled
	return 0;
	//disabled
	...
	int iIncome = 0;
	int iMaxIncome = 0;
	int iExtraYieldperImprovement = 4;
	...
	for (int iI = 0; iI < getNumCityPlots(); iI++)
	{
		...
		if (pLoopPlot != NULL)
		{
			if (pLoopPlot->getWorkingCity() == this && pLoopPlot != plot())
			{
				if(pLoopPlot->getImprovementType() != NO_IMPROVEMENT)
				{
					if(GC.getImprovementInfo(pLoopPlot->getImprovementType()).getYieldChange(iYield) > 1)
					{
						iMaxIncome += iExtraYieldperImprovement;
					}
				}
				else
				{
					iIncome += pLoopPlot->getCityYield((YieldTypes)iYield);
				}
			}
		}
	}

	iIncome = iIncome / 100;	//getCityYield above returns x100

	if(iExtra > 0)
	{
		return std::max(0,std::min(iExtra*iExtraYieldperImprovement,iIncome-iMaxIncome));
	}
	if(iExtra < 0)
	{
		return std::min(0,std::max(iExtra*iExtraYieldperImprovement,iMaxIncome+iExtra*iExtraYieldperImprovement-iIncome));
	}

	return std::min(iMaxIncome,iIncome);
}

void CvCity::updateYieldIncome(int iYield)
{
	int iIncome = 0;
	bool bValid = false;
	CvPlot* pLoopPlot;
	bool bLumberFromAF = false;

	//Count Lumberincome from ancient forests only if right Civic is picked
	if(iYield == YIELD_LUMBER)
	{
		bool bValid = false;

		for(int iI = 0; iI < GC.getNumCivicOptionInfos(); iI++)
		{
			if(GC.getCivicInfo((CivicTypes)GET_PLAYER(getOwnerINLINE()).getCivics((CivicOptionTypes)iI)).isLumberFromAF())
			{
				bLumberFromAF = true;
				break;
			}
	...
	if(bLumberFromAF || iYield == YIELD_HERB) {
		ImprovementTypes eWitchHut = (ImprovementTypes)GC.getInfoTypeForString("IMPROVEMENT_WITCH_HUT");
		FeatureTypes eAF = (FeatureTypes)GC.getInfoTypeForString("FEATURE_FOREST_ANCIENT");
		for (int iI = 0; iI < getNumCityPlots(); iI++)
		{
			pLoopPlot = getCityIndexPlot(iI);
			...
			if (pLoopPlot != NULL)
			{
				if (pLoopPlot->getWorkingCity() == this)
				{
					if(iYield == YIELD_HERB)
					{
						if(pLoopPlot->getImprovementType() == eWitchHut)
						{
							iIncome+=3;
						}
					}
					...
					{
						if(pLoopPlot->getFeatureType() == eAF)
						{
							iIncome += 1;
						}
					}
	...
	iIncome += calculateCityYieldIncome(iYield);
	iIncome += getExtraYieldIncome(iYield);

	int iCityIncome = iIncome;
	iCityIncome *= getYieldRateModifier((YieldTypes)iYield);
	iIncome += iCityIncome/100;

	m_paiYieldIncome[iYield] = iIncome;
}
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
...
/**						                                            							**/
/*************************************************************************************************/
int CvCity::getBuildingBonusChange(BuildingClassTypes eBuildingClass, BonusTypes eBonus) const
{
	for (std::vector<BuildingBonusChange>::const_iterator it = m_aBuildingBonusChange.begin(); it != m_aBuildingBonusChange.end(); ++it)
	{
		if ((*it).eBuildingClass == eBuildingClass && (*it).eBonus == eBonus)
		{
			return (*it).iChange;
		}
	}
...
void CvCity::setBuildingBonusChange(BuildingClassTypes eBuildingClass, BonusTypes eBonus, int iChange)
{
	FAssertMsg(iChange >= 0, "iChange expected to be non-negative");
	for (std::vector<BuildingBonusChange>::iterator it = m_aBuildingBonusChange.begin(); it != m_aBuildingBonusChange.end(); ++it)
	{
		if ((*it).eBuildingClass == eBuildingClass && (*it).eBonus == eBonus)
		{
			int iOldChange = (*it).iChange;
			if (iChange != iOldChange)
			{
				if (iChange == 0)
				{
					m_aBuildingBonusChange.erase(it);
				}
				else
				{
					(*it).iChange = iChange;
				...
				if (NO_BUILDING != eBuilding)
				{
					if (getNumActiveBuilding(eBuilding) > 0)
					{
						changeFreeBonus(eBonus, (iChange - iOldChange) * getNumActiveBuilding(eBuilding));
					}
				}
			}
	...
	if (0 != iChange)
	{
		...
		if (NO_BUILDING != eBuilding)
		{
			if (getNumActiveBuilding(eBuilding) > 0)
			{
				changeFreeBonus(eBonus, iChange * getNumActiveBuilding(eBuilding));
			}
		}
	}
}

void CvCity::changeBuildingBonusChange(BuildingClassTypes eBuildingClass, BonusTypes eBonus, int iChange)
{
	setBuildingBonusChange(eBuildingClass, eBonus, getBuildingBonusChange(eBuildingClass, eBonus) + iChange);
}

void CvCity::calculateBuildingBonus(BuildingClassTypes eBuildingClass, BonusTypes eBonus, int iNumBuilding)
{
	for (std::vector<BuildingBonusChange>::iterator it = m_aBuildingBonusChange.begin(); it != m_aBuildingBonusChange.end(); ++it)
	{
		if ((*it).eBuildingClass == eBuildingClass && (*it).eBonus == eBonus)
		{
			changeFreeBonus(eBonus, (*it).iChange * iNumBuilding);
		}
	}
}
/*************************************************************************************************/
...
CvUnit* CvCity::PlaceUnit(UnitTypes eUnit, PlayerTypes ePlayer, UnitAITypes eUnitAI)
{
	CvPlot* pTarget=NULL;
	...
	CvUnit* pUnit;
	pUnit = GET_PLAYER(ePlayer).initUnit(eUnit,getX_INLINE(),getY_INLINE(), eUnitAI);
	return pUnit;
}


void CvCity::processCultureLevel(int iOldLevel, int iNewLevel)
{
	for(int iI=0;iI<GC.getNUM_YIELD_TYPES();iI++)
	{
		changeBaseYieldRate((YieldTypes)iI,getYieldPerCultureLevel(iI)*(iNewLevel-iOldLevel));
	}
}

void CvCity::updateCommerceFromBonus(int iCommerceType)
{
	changeCommerceFromBonus(iCommerceType,-getCommerceFromBonus(iCommerceType));
	//reset to zero

	int iNetEffect;
	//Building is added/removed
	for(int iI=0;iI<GC.getNumBuildingInfos();iI++)
	{
		CvBuildingInfo& kBuilding=GC.getBuildingInfo((BuildingTypes)iI);
		bool bFirst=true;
		if(kBuilding.getBonusCommerceType()==iCommerceType)
		{
			if(getNumRealBuilding((BuildingTypes)iI)>0)
			{
				for(int iJ=0;iJ<GC.getNumBonusInfos();iJ++)
				{
					if(kBuilding.isBonusLinked(iJ)&& getNumBonuses((BonusTypes)iJ)>0)
					{
						iNetEffect=kBuilding.getPerBonusValue()*(getNumBonuses((BonusTypes)iJ));
						if(bFirst)
						{
							iNetEffect+=kBuilding.getInitialValue();
							bFirst=false;
						}
						changeCommerceFromBonus(kBuilding.getBonusCommerceType(),iNetEffect);
					}
				}
			}
		}
...
void CvCity::updateYieldFromBonus(int iYieldType)
{
	changeBaseYieldRate((YieldTypes)iYieldType, -getYieldFromBonus(iYieldType)/100, true);
	changeYieldFromBonus(iYieldType,-getYieldFromBonus(iYieldType));
	//reset to zero
	...
	for(int iI=0;iI<GC.getNumBuildingInfos();iI++)
	{
		CvBuildingInfo& kBuilding=GC.getBuildingInfo((BuildingTypes)iI);
		bool bFirst=true;
		if(kBuilding.getBonusYieldType()==iYieldType)
		{
			if(getNumRealBuilding((BuildingTypes)iI)>0)
			{
				for(int iJ=0;iJ<GC.getNumBonusInfos();iJ++)
				{
					if(kBuilding.isBonusLinked(iJ)&& getNumBonuses((BonusTypes)iJ)>0)
					{
						...
						changeYieldFromBonus(kBuilding.getBonusYieldType(),iNetEffect);
					}
				}
			}
		}
...
void CvCity::updateBonusConsumed(BuildingTypes eBuilding,int iChange)
{
	...
	for(int iI=0;iI<GC.getNumBonusInfos();iI++)
	{
		if(GC.getBuildingInfo(eBuilding).isBonusConsumed(iI))
		{
			//Pick a Bonus that we substract
			if(iChange>0)
			{
				iValue=GET_PLAYER(getOwnerINLINE()).getNumAvailableBonuses((BonusTypes)iI);
			}
			else
			{
				iValue=GET_PLAYER(getOwnerINLINE()).getNumBonusesConsumed((BonusTypes)iI);
			}
			if(iValue>iBestValue)
			{
				iBestValue=iValue;
	...
	{
		GET_PLAYER(getOwnerINLINE()).changeNumBonusesConsumed(eBestBonus,iChange);
	}
}

int CvCity::getCitySpecializationThreshold(int iType) const
{
	int iBase=GC.getDefineINT("CITY_SPECIALIZATION_BASE");
	int iIncrease=GC.getDefineINT("CITY_SPECIALIZATION_INCREASE");
	return iBase+(getTotalSpecializationLevel()*iIncrease)+(getCitySpecializationLevel(iType)*iIncrease);
}

int CvCity::calculateCitySpecializationPointsPerTurn(ProcessTypes eProcess) const
{
	CvProcessInfo &kProcess=GC.getProcessInfo(eProcess);
	int iValue=0;
	if(kProcess.getProductionToCommerceModifier(COMMERCE_GOLD))
	{
		iValue+=getCitySpecializationThreshold(CITYSPECIALIZATION_GOLD)/25;
		iValue+=(kProcess.getProductionToCommerceModifier(COMMERCE_GOLD))*getCurrentProductionDifference(true,false)/5;
	}
	if(kProcess.getProductionToCommerceModifier(COMMERCE_RESEARCH))
	{
		iValue+=getCitySpecializationThreshold(CITYSPECIALIZATION_RESEARCH)/25;
		iValue+=(kProcess.getProductionToCommerceModifier(COMMERCE_RESEARCH))*getCurrentProductionDifference(true,false)/5;
	}
	if(kProcess.getProductionToCommerceModifier(COMMERCE_CULTURE))
	{
		iValue+=getCitySpecializationThreshold(CITYSPECIALIZATION_CULTURE)/25;
		iValue+=(kProcess.getProductionToCommerceModifier(COMMERCE_CULTURE))*getCurrentProductionDifference(true,false)/5;
	}
	if(kProcess.getProductionToCommerceModifier(COMMERCE_ARCANE))
	{
		iValue+=getCitySpecializationThreshold(CITYSPECIALIZATION_ARCANE)/25;
		iValue+=(kProcess.getProductionToCommerceModifier(COMMERCE_ARCANE))*getCurrentProductionDifference(true,false)/5;
	}

	return iValue;
}

int CvCity::getCitySpecializationIncome(int iSpecializationType) const
{
	int iIncome=0;
	switch(iSpecializationType)
	{
		case CITYSPECIALIZATION_GOLD:
			if(getCitySpecializationLevel(CITYSPECIALIZATION_GOLD)*10<getCommerceRate(COMMERCE_GOLD))
			{
				iIncome+=50*getSpecialistCount((SpecialistTypes)GC.getSPECIALIST_MERCHANT());
			}			
			if(getProductionProcess()!=NO_PROCESS && GC.getProcessInfo((ProcessTypes)getProductionProcess()).getProductionToCommerceModifier(COMMERCE_GOLD)>0)
			{
				iIncome+=calculateCitySpecializationPointsPerTurn(getProductionProcess());
			}
			break;
		case CITYSPECIALIZATION_RESEARCH:
			...
			{
				iIncome+=50*getSpecialistCount((SpecialistTypes)GC.getSPECIALIST_SCIENTIST());
			}			
			if(getProductionProcess()!=NO_PROCESS && GC.getProcessInfo((ProcessTypes)getProductionProcess()).getProductionToCommerceModifier(COMMERCE_RESEARCH)>0)
			{
				iIncome+=calculateCitySpecializationPointsPerTurn(getProductionProcess());
			}
			break;
		case CITYSPECIALIZATION_CULTURE:
			...
			{
				iIncome+=50*getSpecialistCount((SpecialistTypes)GC.getSPECIALIST_ARTIST());
			}			
			if(getProductionProcess()!=NO_PROCESS && GC.getProcessInfo((ProcessTypes)getProductionProcess()).getProductionToCommerceModifier(COMMERCE_CULTURE)>0)
			{
				iIncome+=calculateCitySpecializationPointsPerTurn(getProductionProcess());
			}
			break;
		case CITYSPECIALIZATION_ARCANE:
			...
			if(getProductionProcess()!=NO_PROCESS && GC.getProcessInfo((ProcessTypes)getProductionProcess()).getProductionToCommerceModifier(COMMERCE_ARCANE)>0)
			{
				iIncome+=calculateCitySpecializationPointsPerTurn(getProductionProcess());
			}
			break;
		case CITYSPECIALIZATION_FAITH:
			...
			{
				iIncome+=50*getSpecialistCount((SpecialistTypes)GC.getSPECIALIST_PRIEST());
			}			
			if(getProductionProcess()!=NO_PROCESS && GC.getProcessInfo((ProcessTypes)getProductionProcess()).getProductionToCommerceModifier(COMMERCE_FAITH)>0)
			{
				iIncome+=calculateCitySpecializationPointsPerTurn(getProductionProcess());
			}
			break;
		case CITYSPECIALIZATION_MILITARY:
			if(isProductionUnit() && GC.getUnitInfo(getProductionUnit()).getCombat()>0)
			{
				iIncome+=getCurrentProductionDifference(true,false)*3;
			}
			break;
		case CITYSPECIALIZATION_METAL:
			if(getCitySpecializationLevel(CITYSPECIALIZATION_METAL)*10<getYieldIncome(YIELD_METAL))
			{
				iIncome+=50*getSpecialistCount((SpecialistTypes)GC.getSPECIALIST_ENGINEER());
			}			
			break;
		case CITYSPECIALIZATION_LEATHER:
			if(getCitySpecializationLevel(CITYSPECIALIZATION_LEATHER)*10<getYieldIncome(YIELD_LEATHER))
			{
				iIncome+=50*getSpecialistCount((SpecialistTypes)GC.getSPECIALIST_ENGINEER());
			}			
			break;
		case CITYSPECIALIZATION_STONE:
			if(getCitySpecializationLevel(CITYSPECIALIZATION_STONE)*10<getYieldIncome(YIELD_STONE))
			{
				iIncome+=50*getSpecialistCount((SpecialistTypes)GC.getSPECIALIST_ENGINEER());
			}			
			break;
		case CITYSPECIALIZATION_LUMBER:
			if(getCitySpecializationLevel(CITYSPECIALIZATION_LUMBER)*10<getYieldIncome(YIELD_LUMBER))
			{
				iIncome+=50*getSpecialistCount((SpecialistTypes)GC.getSPECIALIST_ENGINEER());
			}			
			break;
...
void CvCity::doCitySpecialization()
{	
	for(int iI=0;iI<NUM_CITYSPECIALIZATION_TYPES;iI++)
	{
		changeCitySpecialization(iI,getCitySpecializationIncome(iI));
	}
}

int CvCity::getGreatPeopleUnitRateTotal(UnitTypes eIndex) const
{
	return (getGreatPeopleUnitRate(eIndex)*getTotalGreatPeopleRateModifier())/100;
}

int CvCity::getNumFavoredImprovements() const
{
	ImprovementTypes iFavoredImp=(ImprovementTypes)GC.getCivilizationInfo(GET_PLAYER(getOwnerINLINE()).getCivilizationType()).getImpInfrastructureHalfCost();
	int iCount=0;
	...
	for (int iI = 0; iI < getNumCityPlots(); iI++)
	{
		...
		if (pLoopPlot != NULL)
		{
			if(pLoopPlot->getWorkingCity()==this)
			{
				if(pLoopPlot->isImprovementOrUpgrade(iFavoredImp))
				{
					iCount++;
				}
			}
...
int CvCity::calculateGPPThresholdReductionByCulture() const
{
	int iThresholdReduction = (getCultureLevel()*(getCultureLevel()+1))/3;
	return iThresholdReduction * 2;
}

int CvCity::calculateGPPThresholdReductionByPopulation() const
{
	return std::max(0,(getPopulation()-15)*4);
}

int CvCity::getCultureRadius(int iCultureLevel) const
{
	switch(iCultureLevel)
	{
		case NO_CULTURELEVEL:
			return -1;
		default:
			break;
	}
	return 1+(iCultureLevel/2);
}

int CvCity::getNextGreatPersonInfo(bool bType) const
{
	int iReductionTotal=0;
/**
	if(getGreatPeopleThreshold()>0)
	{
		iReductionTotal+=GC.getDefineINT("CITY_GPPTHRESHOLD_REDUCTION_PER_TURN_BASE");
		iReductionTotal+=calculateGPPThresholdReductionByPopulation();
		iReductionTotal+=calculateGPPThresholdReductionByCulture();

		iReductionTotal*=100+getGPPThresholdReductionPercent();
		iReductionTotal/=100;
	}
**/
	...
	for(int iI=0;iI<GC.getNumUnitInfos();iI++)
	{
		if(getGreatPeopleUnitRateTotal((UnitTypes)iI)>0)
		{
			iValue = GET_PLAYER(getOwnerINLINE()).greatPeopleThreshold(false)+getGreatPeopleThreshold() - getGreatPeopleUnitProgress((UnitTypes)iI);
			if(iValue>0)
			{
				int iRate=getGreatPeopleUnitRateTotal((UnitTypes)iI);
				if(getGreatPeopleThreshold()>0 && iReductionTotal>0)
				{
	...
	{
		return eNextGreatPerson;
	}
}

//returns how likely the City is selected for revolt
int CvCity::getRevoltValue() const
{
	if(isCapital() || GET_PLAYER(getOwnerINLINE()).getCapitalCity()==NULL)
	{
		return -MAX_INT;
	}

	int iDistance=stepDistance(getX_INLINE(),getY_INLINE(),GET_PLAYER(getOwnerINLINE()).getCapitalCity()->getX_INLINE(),GET_PLAYER(getOwnerINLINE()).getCapitalCity()->getY_INLINE());

	return iDistance-getCommerceRate(COMMERCE_GOLD);
}

void CvCity::updateCommerceFromBonus()
{
	for(int iI=0;iI<GC.getNUM_COMMERCE_TYPES();iI++)
	{
		updateCommerceFromBonus(iI);
	}
}

void CvCity::updateYieldFromBonus()
{
	for(int iI=0;iI<GC.getNUM_COMMERCE_TYPES();iI++)
	{
		updateYieldFromBonus(iI);
	}
}

void CvCity::updateCultureFromImprovements()
{
	int iCulture = 0;
	for (int iI = 0; iI < getNumCityPlots(); iI++) {
		CvPlot* pLoopPlot = getCityIndexPlot(iI);
		...
		if (pLoopPlot != NULL) {
			if(pLoopPlot->getWorkingCity() == this 
				&& pLoopPlot->getImprovementType() != NO_IMPROVEMENT) {
					iCulture += GC.getImprovementInfo(pLoopPlot->getImprovementType()).getCultureChange();
			}
		}
	} // for CityPlots
	setCultureFromImprovements(iCulture);
}

bool CvCity::isNearPeak() const
{
	for (int iI = 0; iI < getNumCityPlots(); iI++) {
		CvPlot* pLoopPlot = getCityIndexPlot(iI);
...
void CvCity::removeBuildings(BuildingTypes eBuilding, int* iCount) {
	BuildingClassTypes eBuildingClass = (BuildingClassTypes)GC.getBuildingInfo(eBuilding).getBuildingClassType();
	if(getNumRealBuilding(eBuilding) > 0) {
		++(*iCount);
		setNumRealBuilding(eBuilding, 0);
	...
	for(int i = 0; i < GC.getNumBuildingInfos(); ++i) {
		if(GC.getBuildingInfo((BuildingTypes)i).getPrereqBuildingClass() == eBuildingClass) {
			removeBuildings((BuildingTypes)i, iCount);
		}
	}
}

void CvCity::removeBuildingsWhenConquered(bool bConquest) {
	int iCount = 0;
	int iBuildingCount = 0;

	//Count Buildings
	for(int i = 0; i < GC.getNumBuildingInfos(); ++i) {
		if(getNumRealBuilding((BuildingTypes)i) > 0) {
			if(!GC.getBuildingInfo((BuildingTypes)i).isEnchantment()) {
				++iBuildingCount;
			}
		}
	}

	//Step0 Adjust Enchantments
	for(int i = 0; i < GC.getNumProjectInfos(); ++i) {
		if(GC.getProjectInfo((ProjectTypes)i).isGlobalEnchantment()) {
			BuildingTypes eBuilding = (BuildingTypes)GC.getProjectInfo((ProjectTypes)i).getBuildingType();
			if(eBuilding != NO_BUILDING) {
				setNumRealBuilding(eBuilding, 
					(GET_TEAM(getTeam()).getProjectCount((ProjectTypes)i) > 0));
			}
		}
	}

	//Step1 convert UB to Standard Buildings for that Civ
	for(int i = 0; i < GC.getNumBuildingInfos(); ++i) {
		if(getNumRealBuilding((BuildingTypes)i) > 0) {
			BuildingClassTypes eBuildingClass = (BuildingClassTypes)GC.getBuildingInfo((BuildingTypes)i).getBuildingClassType();
			BuildingTypes CivBuilding = (BuildingTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(eBuildingClass);
			if(CivBuilding != i) {
				setNumRealBuilding((BuildingTypes)i, 0);
				if(CivBuilding != NO_BUILDING)
					setNumRealBuilding(CivBuilding, 1);
			}
		}
	}

	//Step2 remove National Wonders
	for(int i = 0; i < GC.getNumBuildingInfos(); ++i) {
		if(GC.getBuildingClassInfo((BuildingClassTypes)GC.getBuildingInfo((BuildingTypes)i).getBuildingClassType()).getMaxPlayerInstances() != -1) {
			removeBuildings((BuildingTypes)i, &iCount);
		}
	}
	//Step3 remove Districts
	for(int i = 0; i < GC.getNumBuildingInfos(); ++i) {
		if(GC.getBuildingInfo((BuildingTypes)i).isDistrict()) {
			removeBuildings((BuildingTypes)i, &iCount);
		}
	}
	//Step4 remove Civ Prereq Buildings
	for(int i = 0; i < GC.getNumBuildingInfos(); ++i) {
		if(GC.getBuildingInfo((BuildingTypes)i).getPrereqCiv() != NO_CIVILIZATION) {
			if(GC.getBuildingInfo((BuildingTypes)i).getPrereqCiv() != getCivilizationType()) {
				removeBuildings((BuildingTypes)i, &iCount);
			}
		}
	}
	...
	for(int i = 0; i < GC.getNumBuildingInfos(); ++i) {
		if(GC.getBuildingInfo((BuildingTypes)i).isNeverCapture()) {
			removeBuildings((BuildingTypes)i, &iCount);
		}
	}
	...
	//Step5 remove other Buildings if needed
	int iNumRemoveBuildings = ((75 - GET_PLAYER(getOwner()).getExtraBuildingCapturedChance()) * iBuildingCount) / 100;

	while(iNumRemoveBuildings > iCount) {
		bool bFound = false;

		std::vector<int> Buildings_to_Remove;

		for(int i = 0; i < GC.getNumBuildingInfos(); ++i) {
			if(getNumRealBuilding((BuildingTypes)i) > 0) {
				if(!GC.getBuildingInfo((BuildingTypes)i).isEnchantment()) {
					Buildings_to_Remove.push_back(i);
				}
			}
		}
		...
		if(Buildings_to_Remove.size() > 0) {
			bFound = true;
			removeBuildings((BuildingTypes)Buildings_to_Remove[GC.getGame().getSorenRandNum(Buildings_to_Remove.size(), "Remove Building")], &iCount);
		}
...
int CvCity::getGlobalYieldCost(UnitTypes eUnit) const
{
//	UnitClassTypes eUnitClass = (UnitClassTypes)GC.getUnitInfo(eUnit).getUnitClassType();
//	if(GET_PLAYER(getOwnerINLINE()).isIgnoreGlobalYieldCost(eUnitClass)) {
//		return 0;
//	}

	return GC.getUnitInfo(eUnit).getGlobalYieldCost();
}

int CvCity::getGlobalYieldCost(BuildingTypes eBuilding) const
{
	BuildingClassTypes eBuildingClass = (BuildingClassTypes)GC.getBuildingInfo(eBuilding).getBuildingClassType();
	if(GET_PLAYER(getOwnerINLINE()).isIgnoreGlobalYieldCost(eBuildingClass)) {
		return 0;
	}

	return GC.getBuildingInfo(eBuilding).getGlobalYieldCost();
}

void CvCity::spawnGarrisonUnits() const
{
	UnitTypes eUnit = (UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getDefaultUnit();
	...
	if(iCounter < iMaxGarrison)
	 {
		CvUnit* newUnit = GET_PLAYER(getOwnerINLINE()).initUnit(eUnit, getX_INLINE(), getY_INLINE());
		newUnit->setGarrision(true);
	}
}
