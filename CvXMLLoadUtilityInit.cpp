...
//------------------------------------------------------------------------------------------------------
//
//  FUNCTION:   InitTerrainDefaults(int **ppiDefaults)
//
//  PURPOSE :   allocate and initialize the planes's default terrains
//
//------------------------------------------------------------------------------------------------------
void CvXMLLoadUtility::InitTerrainDefaults(int **ppiDefaults)
{
	// SPEEDUP
	PROFILE_FUNC();
	...
	FAssertMsg(*ppiDefaults == NULL,"memory leak?");
	// allocate memory based on the number of building classes
	*ppiDefaults = new int[GC.getNumTerrainClassInfos()];
	// set the local pointer to the new memory
	piDefaults = *ppiDefaults;

	// loop through all the pointers and set their default values
	for (i=0;i<GC.getNumTerrainClassInfos();i++)
	{
		piDefaults[i] = GC.getTerrainClassInfo((TerrainClassTypes) i).getDefaultTerrainIndex();
	}
}

//------------------------------------------------------------------------------------------------------
//
//  FUNCTION:   InitFeatureDefaults(int **ppiDefaults)
//
//  PURPOSE :   allocate and initialize the planes's default features
//
//------------------------------------------------------------------------------------------------------
void CvXMLLoadUtility::InitFeatureDefaults(int **ppiDefaults)
{
	// SPEEDUP
	PROFILE_FUNC();
	...
	FAssertMsg(*ppiDefaults == NULL,"memory leak?");
	// allocate memory based on the number of building classes
	*ppiDefaults = new int[GC.getNumFeatureClassInfos()];
	// set the local pointer to the new memory
	piDefaults = *ppiDefaults;

	// loop through all the pointers and set their default values
	for (i=0;i<GC.getNumFeatureClassInfos();i++)
	{
		piDefaults[i] = GC.getFeatureClassInfo((FeatureClassTypes) i).getDefaultFeatureIndex();
	}
}
//------------------------------------------------------------------------------------------------------
//
