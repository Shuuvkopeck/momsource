...
public:
	...
	int getRawBaseCombatStr();
	int getRawBaseCombatStrDefense();
...
/*************************************************************************************************/
	...
	void exploreLair();
	void equip(int /* PromotionTypes */ Equipment, bool bTreasure);
	int getEquipment(int eCategory) const;
	bool isImmuneToCapture() const;
	bool isHeadSelectedUnit();
	void changeAIControl(int iNewValue);
	void changeDurability(int eCategory, int iChange);
	void changeCannotLeaveCulture(int iChange);
	void changeStrengthPotionsUsed(int iChange);
	void displayMessagePromotionGainedInBattle(int ePromotion) const;
/** Better Promotions **/
    int AI_getPromotionSpecialization();
