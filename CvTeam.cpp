...
void CvTeam::reset(TeamTypes eID, bool bConstructorCall)
{
	...
	//Darksavant
	//For Divine Judgement enhancement
	setDivineJudgementCounter(0);
...
/**						                                            							**/
/*************************************************************************************************/
	m_bHouseErebus = false;
	m_iSeeEverything = 0;
	m_iCulturalAchievementsCount = 0;
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
	m_eID = eID;
...
/*************************************************************************************************/
	if (!bConstructorCall)
	{
		if (eID == BARBARIAN_TEAM)
		{
			for(int iI = BARBARIAN_PLAYER+1; iI < NUM_BARBARIAN_PLAYERS; iI++)
			{
				GET_TEAM((TeamTypes)iI).reset((TeamTypes)iI, bConstructorCall);
			}
		}
	}
/*************************************************************************************************/
...
/*************************************************************************************************/
/**	ADDON (New Barbarians) Sephi			                                 					**/
/**	When joining a Team, Peace with a BarbarianFaction is kept  								**/
/*************************************************************************************************/
	for (iI = 0; iI < MAX_TEAMS; iI++)
	{
		if ((iI != getID()) && (iI != eTeam))
		{
			if (GET_TEAM((TeamTypes)iI).isAlive() && GET_TEAM((TeamTypes)iI).isBarbarian())
			{
				...
				{
					GET_TEAM(eTeam).makePeace(((TeamTypes)iI), false);
				}
			}
		}
	}
...
/*************************************************************************************************/
/**	ADDON (Global Enchantments) Sephi                                         					**/
/*************************************************************************************************/
	...
	for(iI=0;iI<GC.getNumProjectInfos();iI++)
	{
		if(getProjectCount((ProjectTypes)iI)>0)
		{
			if(GC.getProjectInfo((ProjectTypes)iI).isGlobalEnchantment())
			{
				for(int iJ=0;iJ<GC.getMAX_CIV_PLAYERS();iJ++)
				{
					...
					{
						GET_PLAYER(getLeaderID()).ApplyGlobalEnchantment((ProjectTypes)iI,NO_TEAM,(PlayerTypes)iJ);
					}
				}
			}
		}
		...
		{
			for(int iK=0;iK<GC.getMAX_CIV_TEAMS();iK++)
			{
				if(GET_TEAM((TeamTypes)iK).isAlive())
				{
					if(GET_TEAM((TeamTypes)iK).isAtWar(getID()))
					{
						if(GET_TEAM((TeamTypes)iK).getProjectCount((ProjectTypes)iI)>0)
						{
							for(int iJ=0;iJ<GC.getMAX_CIV_PLAYERS();iJ++)
							{
								if(GET_PLAYER((PlayerTypes)iJ).getTeam()==eTeam)
								{
									GET_PLAYER(GET_TEAM((TeamTypes)iK).getLeaderID()).ApplyGlobalEnchantment((ProjectTypes)iI,NO_TEAM,(PlayerTypes)iJ);
								}
							}
						}
					}
...
void CvTeam::shareCounters(TeamTypes eTeam)
{
	...
	for (iI = 0; iI < MAX_TEAMS; iI++)
	{
		...
		{
			//Darksavant
			//In the unlikely event that someone forms a team when one of them is running the Divine Judgement project this should cover it, with the one farther along getting credit.
			//I imagine that in the extremely unlikely event both team members are running different Divine Judgements there might be some unexpected repercussions.  Or rather expected repercussions that I
			//don't have the mental fortitude to deal with correctly.  Basically losing one holy city will reset the counter.
			if (GET_TEAM(eTeam).getDivineJudgementCounter() > getDivineJudgementCounter())
			{
				setDivineJudgementCounter(GET_TEAM(eTeam).getDivineJudgementCounter());
			}
...
//FfH: Added by Kael 11/25/2007
	if (GC.getBuildingInfo(eBuilding).isNoCivicAnger())
	{
		if (iChange > 0)
		{
            setNoCivicAnger(true);
		}
		else
		{
            setNoCivicAnger(false);
		}
	}
//FfH: End Add
...
{
	// sometimes a value of -1 is given when ai groups target a field with no owner
	// we return at this point because of potential ctds in further processing
	if (eTeam < 0 || eTeam == getID())
	{
		return false;
	}
...
/*************************************************************************************************/
	...
	//Special Rules for CityStates
	if(GET_PLAYER(getLeaderID()).isCityState())
	{
		return false;
	}
	CvPlayer& kLeader=GET_PLAYER(GET_TEAM(eTeam).getLeaderID());
	if(!isHuman())
	{
		if(kLeader.isCityState() && kLeader.getCityStateType()!=NO_CITYSTATE)
		{
			ReligionTypes eReligion=(ReligionTypes)GC.getCityStateInfo((CityStateTypes)kLeader.getCityStateType()).getReligionType();
			if(eReligion!=NO_RELIGION)
			{
				if(eReligion==GET_PLAYER(getLeaderID()).getStateReligion() || eReligion==GET_PLAYER(getLeaderID()).getFavoriteReligion())
				{
					return false;
				}
			}

			//don't declare war early on
			if(GC.getGameINLINE().getElapsedGameTurns()<1000 && GC.getGameINLINE().getElapsedGameTurns()*3<GC.getGame().getMaxTurns())
			{
				return false;
			}
		}
...
/*************************************************************************************************/
    if(getID()==GC.getMERCENARIES_TEAM() || eTeam ==GC.getMERCENARIES_TEAM())
    {
        if(GC.getGameINLINE().getMercenariesWarTarget()==NO_TEAM)
        {
            return false;
        }
        if(getID()!=GC.getGameINLINE().getMercenariesWarTarget() && eTeam != GC.getGameINLINE().getMercenariesWarTarget()
            && !GET_TEAM(getID()).isVassal((TeamTypes)GC.getGameINLINE().getMercenariesWarTarget())
            && !GET_TEAM(eTeam).isVassal((TeamTypes)GC.getGameINLINE().getMercenariesWarTarget()))
        {
            return false;
        }
    }
...
//FfH: Added by Kael 11/29/2007
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
			{
				GET_PLAYER((PlayerTypes)iI).setDeclaringWar(true);
			}
		}
//FfH: End Add
				...
				for(int iAdvLoop=0;iAdvLoop<GC.getNumAdventureInfos();iAdvLoop++)
				{
					if(GET_PLAYER((PlayerTypes)iK).isAdventureEnabled(iAdvLoop) && !GET_PLAYER((PlayerTypes)iK).isAdventureFinished(iAdvLoop))
					{
						...
						for(int iJ=0;iJ<GC.getMAX_ADVENTURESTEPS();iJ++)
						{
							...
							{
								if(GC.getAdventureStepInfo((AdventureStepTypes)kAdv.getAdventureStep(iJ)).isDeclareWarAtFriendly())
								{
									bool bValid=false;

									for(int iL=0;iL<GC.getMAX_CIV_PLAYERS();iL++)
									{
										if(GET_PLAYER((PlayerTypes)iL).isAlive() && GET_PLAYER((PlayerTypes)iL).getTeam()==eTeam)
										{
											if(GET_PLAYER((PlayerTypes)iL).AI_getAttitude((PlayerTypes)iK)==ATTITUDE_FRIENDLY)
											{
												bValid=true;
												break;
											}
								...
								if(GC.getAdventureStepInfo((AdventureStepTypes)kAdv.getAdventureStep(iJ)).isDeclareWar())
								{
									bool bValid=false;
									for(int iL=0;iL<GC.getMAX_CIV_PLAYERS();iL++)
									{
										if(GET_PLAYER((PlayerTypes)iL).isAlive() && GET_PLAYER((PlayerTypes)iL).getTeam()==eTeam)
										{
											if(GET_PLAYER((PlayerTypes)iK).VerifyAdventureProgress(kAdv.getAdventureStep(iJ),NULL,iL,NULL))
											{
												bValid=true;
												break;
											}
...
/**	ADDON (Houses of Erebus) Sephi			                                 					**/
/**																								**/
/**	Declaring War on Houses of Erebus doesn't cancel defensive Pacts 					                                            							**/
/*************************************************************************************************/
/* original code
...
*/
		if (!isHouseErebus() && !(GET_TEAM(eTeam).isHouseErebus()))
		{
			cancelDefensivePacts();
...
//FfH: Added by Kael 11/29/2007
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
			{
				GET_PLAYER((PlayerTypes)iI).setDeclaringWar(false);
			}
		}
//FfH: End Add
/*************************************************************************************************/
...
/*************************************************************************************************/
		...
		for(iI=0;iI<GC.getNumProjectInfos();iI++)
		{
			if(getProjectCount((ProjectTypes)iI)>0)
			{
				if(GC.getProjectInfo((ProjectTypes)iI).isGlobalEnchantment() && GC.getProjectInfo((ProjectTypes)iI).isHostile())
				{
					GET_PLAYER(getLeaderID()).ApplyGlobalEnchantment((ProjectTypes)iI,eTeam);
				}
			}
			...
			if(GET_TEAM(eTeam).getProjectCount((ProjectTypes)iI)>0)
			{
				if(GC.getProjectInfo((ProjectTypes)iI).isGlobalEnchantment() && GC.getProjectInfo((ProjectTypes)iI).isHostile())
				{
					GET_PLAYER(GET_TEAM(eTeam).getLeaderID()).ApplyGlobalEnchantment((ProjectTypes)iI,getID());
				}
			}
		}
...
/**	ADDON (Mercenaries) Sephi	        								                    	**/
/*************************************************************************************************/
    if(GC.getGameINLINE().getMercenariesSupportTarget()==NO_TEAM && GET_PLAYER((PlayerTypes)GC.getMERCENARIES_PLAYER()).getNumUnits()>0)
    {
        if(!isBarbarian() && !GET_TEAM(eTeam).isBarbarian())
        {
            int iVoters=0;
            ...
            if (iVoters>0)
            {
                int iLucky=GC.getGameINLINE().getSorenRandNum(iVoters,"Pick Hippus to decide on Mercenaries Support");
                iVoters=0;
                for(int iI=0; iI<MAX_CIV_PLAYERS; iI++)
                {
                    if(GET_PLAYER((PlayerTypes)iI).isAlive())
                    {
                        if(GET_PLAYER((PlayerTypes)iI).getCivilizationType()==GC.getInfoTypeForString("CIVILIZATION_HIPPUS"))
                        {
                            if (iVoters==iLucky)
                            {
                                TeamTypes eTarget=getID();
                                TeamTypes eSupport=eTeam;
                                if (GET_TEAM(eTeam).getPower(true)>getPower(true))
                                {
                                    eTarget=eTeam;
                                    eSupport=getID();
                                }

                                GC.getGameINLINE().setMercenariesSupportTarget(eSupport);
                                GC.getGameINLINE().setMercenariesWarTarget(eTarget);

                                //triggerEventHere
                                int iEventTrigger = GC.getInfoTypeForString("EVENTTRIGGER_MERCENARIES_DECIDE");
                                if (iEventTrigger!=NO_EVENTTRIGGER)
                                {
                                    GET_PLAYER((PlayerTypes)iI).trigger((EventTriggerTypes)iEventTrigger);
                                }
                                break;

                                if(!GET_TEAM((TeamTypes)GC.getMERCENARIES_TEAM()).isAtWar(eTarget))
                                {
                                    GC.getGameINLINE().setMercenariesSupportTarget(NO_TEAM);
                                    GC.getGameINLINE().setMercenariesWarTarget(NO_TEAM);
                                }
                            }
                            iVoters++;
                        }
...
/*************************************************************************************************/
		...
		for(iI=0;iI<GC.getNumProjectInfos();iI++)
		{
			if(getProjectCount((ProjectTypes)iI)>0)
			{
				if(GC.getProjectInfo((ProjectTypes)iI).isGlobalEnchantment() && GC.getProjectInfo((ProjectTypes)iI).isHostile())
				{
					GET_PLAYER(getLeaderID()).RemoveGlobalEnchantment((ProjectTypes)iI,eTeam);
				}
			}
			...
			if(GET_TEAM(eTeam).getProjectCount((ProjectTypes)iI)>0)
			{
				if(GC.getProjectInfo((ProjectTypes)iI).isGlobalEnchantment() && GC.getProjectInfo((ProjectTypes)iI).isHostile())
				{
					GET_PLAYER(GET_TEAM(eTeam).getLeaderID()).RemoveGlobalEnchantment((ProjectTypes)iI,getID());
				}
			}
		}
/*************************************************************************************************/
...
/**	ADDON (Mercenaries) Sephi	        								                    	**/
/*************************************************************************************************/
    if(GC.getGameINLINE().getMercenariesSupportTarget()!=NO_TEAM && GC.getGameINLINE().getMercenariesWarTarget()!=NO_TEAM)
    {
        if(!GET_TEAM((TeamTypes)GC.getGameINLINE().getMercenariesSupportTarget()).isAtWar((TeamTypes)GC.getGameINLINE().getMercenariesSupportTarget()))
        {
            TeamTypes eMemory=(TeamTypes)GC.getGameINLINE().getMercenariesWarTarget();
            GC.getGameINLINE().setMercenariesWarTarget(NO_TEAM);
            GC.getGameINLINE().setMercenariesSupportTarget(NO_TEAM);
            GET_TEAM((TeamTypes)GC.getMERCENARIES_PLAYER()).makePeace(eMemory);
        }
    }
/*************************************************************************************************/
/**			        						END													**/
...
/*************************************************************************************************/
/**	BUGFIX (Votes for the Councils) Sephi														**/
/*************************************************************************************************/
/**
...
int CvTeam::getResearchCost(TechTypes eTech) const
{
	...
	if(GC.getTechInfo(eTech).isCulturalAchievement()) {
		iCost *= 100 + 20 * getCulturalAchievementsCount();
		iCost /= 100;
	}
...
**/
	if(!isHuman()) {
		iCost *= 100;
		iCost /= std::max(1, GC.getHandicapInfo(GC.getGame().getHandicapType()).getResearchPercent());
	}
	//end modified
...
{
/*************************************************************************************************/
/**	ADDON (Multibarb)  merged Sephi			12/22/08							Xienwolf		**/
/**	adjusted for new barb teams                 												**/
...
//FfH: Added by Kael 11/17/2008
	if (GC.getGameINLINE().isOption(GAMEOPTION_ALWAYS_OPEN_BORDERS))
	{
	    return true;
	}
//FfH: End Add
		...
		if (iChange > 0)
		{
			...
			if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
			{
				...
				for (iI = 0; iI < MAX_PLAYERS; iI++)
				{
					...
					{
						//added Sephi
						if(GET_PLAYER((PlayerTypes)iI).getTeam()==getID() || kProject.getTechPrereq()==NO_TECH || GC.getTechInfo((TechTypes)kProject.getTechPrereq()).getSpellSchool()==NO_MANASCHOOL)
						{
							if(GET_PLAYER((PlayerTypes)iI).getTeam()==getID() || eIndex!=GC.getInfoTypeForString("PROJECT_JOIN_THE_CULT_OF_ESUS"))
							{
						//end Added
		...
		if(kProject.isGlobalEnchantment())
		{
			for(iI=0;iI<GC.getMAX_CIV_PLAYERS();iI++)
			{
				...
				if(kPlayer.isAlive())
				{
					...
					{
						kPlayer.changeFaithUpkeep(kProject.getFaithUpkeep()*iChange);
					}
				}
			}
		}
...
//		FAssert(getResearchProgress(eIndex) >= 0);	can be negative thanks to markets modified Sephi
...
/*************************************************************************************************/
/**	ADDON (Houses of Erebus) Sephi			                                 					**/
/**	Disable Founding of Religions and Corporations															**/
/*************************************************************************************************/
/**
...
//FfH: Added by Kael 10/10/2008
                              && !GC.getGameINLINE().isReligionFounded((ReligionTypes)iI)
//FfH: End Add
...
/*************************************************************************************************/
			...
			//new Tag FreeUnitClass
			if(GC.getTechInfo(eIndex).getFreeUnitClass() != NO_UNITCLASS)
			{
				eFreeUnit = (UnitTypes)GC.getCivilizationInfo(GET_PLAYER(ePlayer).getCivilizationType()).getCivilizationUnits(GC.getTechInfo(eIndex).getFreeUnitClass());
...
//FfH: Added by Kael 11/17/2007
                                              && GET_PLAYER((PlayerTypes)iI).canDoCivics(eCivicType)
//FfH: End Add
...
void CvTeam::processTech(TechTypes eTech, int iChange)
{
	...
	if(GC.getTechInfo(eTech).isCulturalAchievement())
	{
		changeCulturalAchievementsCount(iChange);
	}
...
/**	ADDON (Houses of Erebus) Sephi			                                 					**/
            for (int i=0;i<GC.getNumCorporationInfos();i++)
            {
                ...
                {
                    kPlayer.changeCorporationSupport((CorporationTypes)i,GC.getTechInfo(eTech).getCorporationSupport((CorporationTypes)i));
                }
            }
            ...
            for (int i=0;i<GC.getNumCorporationInfos();i++)
            {
                ...
                {
                    kPlayer.changeCorporationSupportMultiplier((CorporationTypes)i,GC.getTechInfo(eTech).getCorporationSupportMultiplier((CorporationTypes)i));
                }
            }
			...
			if(GC.getTechInfo(eTech).getArcaneAuraPromotion()!=NO_PROMOTION)
			{
				...
				{
					if(pLoopUnit->getUnitCombatType()==GC.getUNITCOMBAT_ADEPT())
					{
						pLoopUnit->setHasPromotion((PromotionTypes)GC.getTechInfo(eTech).getArcaneAuraPromotion(),(iChange>0) ? true : false);
					}
				}
			}
			...
			{
				kPlayer.changeWillpowerChange(GC.getTechInfo(eTech).getWillpowerChange()*iChange);
			}

			if(GC.getTechInfo(eTech).getReducedEnchantmentCost()!=0)
			{
				kPlayer.changeReducedEnchantmentCost(GC.getTechInfo(eTech).getReducedEnchantmentCost()*iChange);
			}

			if(GC.getTechInfo(eTech).getReducedSummonCost()!=0)
			{
				kPlayer.changeReducedSummonCost(GC.getTechInfo(eTech).getReducedSummonCost()*iChange);
			}

			if(GC.getTechInfo(eTech).getReducedTerraformCost()!=0)
			{
				kPlayer.changeReducedTerraformCost(GC.getTechInfo(eTech).getReducedTerraformCost()*iChange);
			}
	   
			if(GC.getTechInfo(eTech).isIncreaseTerraformTiles())
			{
				kPlayer.changeIncreaseTerraformTiles(iChange);
			}
		   
			if(GC.getTechInfo(eTech).isIncreaseHostileTerraformTiles())
			{
				kPlayer.changeIncreaseHostileTerraformTiles(iChange);
			}

			if(GC.getTechInfo(eTech).getIncreaseSummonXP())
			{
				kPlayer.changeIncreaseSummonXP(GC.getTechInfo(eTech).getIncreaseSummonXP()*iChange);
			}

			if(GC.getTechInfo(eTech).getIncreaseMaxMana())
			{
				kPlayer.changeMaxMana(GC.getTechInfo(eTech).getIncreaseMaxMana()*iChange);
			}
			//all Spellresearch increases Max Mana, Mana Income
			if(GC.getTechInfo(eTech).getSpellSchool()!=NO_MANASCHOOL)
			{
				kPlayer.changeMaxMana(10*iChange);
				kPlayer.changeManaFromSpellResearch(1*iChange);
			}

			if(GC.getTechInfo(eTech).getIncreaseMana())
			{
				kPlayer.changeManaFromSpellResearch(GC.getTechInfo(eTech).getIncreaseMana()*iChange);
			}

			if(GC.getTechInfo(eTech).getIncreaseManaFromNodes())
			{
				kPlayer.changeManaFromNodes(GC.getTechInfo(eTech).getIncreaseManaFromNodes() * iChange);
			}

			if(GC.getTechInfo(eTech).getIncreaseStrengthOfRegularUnits())
			{
				kPlayer.changeIncreaseStrengthOfRegularUnits(GC.getTechInfo(eTech).getIncreaseStrengthOfRegularUnits() * iChange);
			}			

			for(iJ=0;iJ<GC.getNumDamageTypeInfos();iJ++)
			{
				if(GC.getTechInfo(eTech).getSpellDamageChange(iJ)!=0)
				{
					kPlayer.changeSpellDamageModify(iJ,GC.getTechInfo(eTech).getSpellDamageChange(iJ));
				}
			}
/*************************************************************************************************/
/**	END	                                        												**/
...
int CvTeam::getDivineJudgementCounter() const
{
	return m_iDivineJudgementCounter;
}

void CvTeam::setDivineJudgementCounter(int iCount)
{
	m_iDivineJudgementCounter = iCount;
}
...
void CvTeam::read(FDataStreamBase* pStream)
{
	...
	pStream->Read(&m_iDivineJudgementCounter);
...
/**						                                            							**/
/*************************************************************************************************/
    pStream->Read(&m_iSeeEverything);
    pStream->Read(&m_iCulturalAchievementsCount);
	pStream->Read(&m_bHouseErebus);
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
//FfH: End Add
...
void CvTeam::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(m_iDivineJudgementCounter);
...
/**						                                            							**/
/*************************************************************************************************/
    pStream->Write(m_iSeeEverything);
    pStream->Write(m_iCulturalAchievementsCount);
	pStream->Write(m_bHouseErebus);
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
	pStream->Write(m_eID);
...
//FfH Traits: Added by Kael 08/02/2007
bool CvTeam::isBarbarianAlly() const
{
    return m_bBarbarianAlly;
}

void CvTeam::setBarbarianAlly(bool bNewValue)
{
	m_bBarbarianAlly = bNewValue;
}

bool CvTeam::isNoCivicAnger() const
{
    return m_bNoCivicAnger;
}

void CvTeam::setNoCivicAnger(bool bNewValue)
{
...
/**						                                            							**/
/*************************************************************************************************/
bool CvTeam::isHasCorporationSupport(CorporationTypes eCorporation) const
{
	for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; ++iPlayer)
	{
		...
		if (kPlayer.isAlive() && kPlayer.getTeam() == getID())
		{
			if (kPlayer.isHasCorporationSupport(eCorporation))
			{
				return true;
			}
		}
...
void CvTeam::setHouseErebus(bool bNewValue)
{
	m_bHouseErebus=bNewValue;
}

bool CvTeam::isHouseErebus() const
{
	return m_bHouseErebus;
}

void CvTeam::changeSeeEverything(int iChange)
{
	if(iChange!=0)
	{
		int iI;

		if(!isSeeEverything() && m_iSeeEverything+iChange>0)
		{
			for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
			{
				GC.getMapINLINE().plotByIndexINLINE(iI)->changeVisibilityCount(getID(),1,NO_INVISIBLE,false);
			}
		}

		m_iSeeEverything+=iChange;
...
bool CvTeam::isSeeEverything() const { return (m_iSeeEverything > 0); }

void CvTeam::changeCulturalAchievementsCount(int iChange)
{
	m_iCulturalAchievementsCount += iChange;
}

int CvTeam::getCulturalAchievementsCount() const { return m_iCulturalAchievementsCount; }

bool CvTeam::isCityState() const { return GET_PLAYER(getLeaderID()).isCityState(); }

int CvTeam::getNumAvailableBonuses(BonusTypes eBonus) const
{
	int iCount=0;
	for (int iI = 0; iI < MAX_PLAYERS; ++iI)
	{
		if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
		{
			iCount+=GET_PLAYER((PlayerTypes)iI).getNumAvailableBonuses(eBonus);
		}
	}
...
//returns true if Team has researched all techs that have eTech as Prereq
bool CvTeam::hasMasteredTech(TechTypes eTech) const
{
	if(eTech == NO_TECH)
		return false;
	...
	for (int j = 0; j < GC.getNumTechInfos(); ++j) {
		CvTechInfo& kTech = GC.getTechInfo((TechTypes)j);

		for (int iI = 0; iI < GC.getNUM_OR_TECH_PREREQS(); iI++)
		{
			if (kTech.getPrereqOrTechs(iI) == eTech)
			{
				if(!isHasTech((TechTypes)j)) {
					return false;
				}
			}
		}
		...
		{
			if (kTech.getPrereqAndTechs(iJ) == eTech)
			{
				if(!isHasTech((TechTypes)j)) {
					return false;
				}
