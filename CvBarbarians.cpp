...
void CvAIGroup::update_barb_plunder()
{
	...
	CvPlot* pMission = getMissionPlot();
	CvPlot* pLair = getHeadUnit()->getOriginPlot();
	CvPlot* pLoopPlot;
	int iDX, iDY;
	...
	if(pMission == NULL && m_iBarbPlunder == 0) {
		//pick new Mission Plot
		for(int iPass = 1; iPass < 4; ++iPass) {
			int iSearchRange = iPass * 10;
			int iBestValue = MAX_INT;
			...
			for (iDX = -(iSearchRange); iDX <= iSearchRange; iDX++)	{
				for (iDY = -(iSearchRange); iDY <= iSearchRange; iDY++) {
					pLoopPlot	= plotXY(getMissionTarget()->getX_INLINE(), getMissionTarget()->getY_INLINE(), iDX, iDY);
	...
	if(pMission != NULL) {
		...
		if(iCounter > 0) {
			m_iBarbPlunder += 500 * iCounter;
		}
	}
	else if(m_iBarbPlunder > 0) {
		pMission = pLair;
	}

	if(pMission == pLair && pMission != NULL) {
		Move(pMission);
		if(atPlot(pMission)) {
			if(pMission->getDungeon() != NULL) {
				pMission->getDungeon()->changePowerTimes1000(m_iBarbPlunder);
				m_iBarbPlunder = 0;
				pMission = NULL;
			}
		}
