...
CvCityAI::CvCityAI()
{
	...
	m_aiCitySpecializationValue = new int[NUM_CITYSPECIALIZATION_TYPES];
    m_aiCitySpecializationValueShortTerm = new int[NUM_CITYSPECIALIZATION_TYPES];

	m_pbEmphasize = NULL;
	m_abWorkerImprovement = NULL;
	m_aiAI_valueYield = new int[NUM_YIELD_TYPES];
	AI_reset();
}
...
CvCityAI::~CvCityAI()
{
	...
	SAFE_DELETE_ARRAY(m_aiCitySpecializationValue);
	SAFE_DELETE_ARRAY(m_aiCitySpecializationValueShortTerm);
	SAFE_DELETE_ARRAY(m_aiAI_valueYield);
}
...
void CvCityAI::AI_init()
{
	...
	CvAIGroup* pNewWorkerGroup = GET_PLAYER(getOwnerINLINE()).initAIGroup(AIGROUP_WORKER);
	setAIGroup_Worker(pNewWorkerGroup);
	pNewWorkerGroup->setMissionCity(this);
	AI_updateBestBuild();

/*************************************************************************************************/
/**	ADDON (AIGroups) Sephi																		**/
/*************************************************************************************************/
	CvAIGroup* pNewGroup = GET_PLAYER(getOwnerINLINE()).initAIGroup(AIGROUP_CITY_DEFENSE);
	setAIGroup_Defense(pNewGroup);
	pNewGroup->setMissionCity(this);
	...
	if(isCoastal(GC.getMIN_WATER_SIZE_FOR_OCEAN()))
	{
		for (int iI = 0; iI < NUM_DIRECTION_TYPES; iI++)
		{
			...
			if(pLoopPlot!=NULL && pLoopPlot->area()->isWater())
			{
				...
				{
					if(pAIGroup->getGroupType()==AIGROUP_NAVAL_EXPLORER && pAIGroup->getMissionArea()==pLoopPlot->getArea())
					{
						bExistsalready=true;
						break;
					}
				...
				if(!bExistsalready)
				{
					CvAIGroup* pNewGroup=GET_PLAYER(getOwnerINLINE()).initAIGroup(AIGROUP_NAVAL_EXPLORER);
	...
	//Which Improvements can be build by Workers?

	UnitClassTypes eWorkerClass = (UnitClassTypes)GC.getUNITCLASS_WORKER();
	UnitTypes eWorker = NO_UNIT;
	if(eWorkerClass != NO_UNITCLASS)
		eWorker = (UnitTypes)GC.getCivilizationInfo((CivilizationTypes)GET_PLAYER((PlayerTypes)getOwnerINLINE()).getCivilizationType()).getCivilizationUnits(eWorkerClass);
	if(eWorker != NO_UNIT) {
		for (int iI = 0; iI < GC.getNumBuildInfos(); ++iI) {
			if(GC.getUnitInfo(eWorker).getBuilds(iI))
				m_abWorkerImprovement[iI] = true;
		}
	}
/*************************************************************************************************/
/**	    									END													**/
...
void CvCityAI::AI_uninit()
{
	...
	SAFE_DELETE_ARRAY(m_abWorkerImprovement);	
}
...
/*************************************************************************************************/
	...
	m_abWorkerImprovement = new bool[GC.getNumBuildInfos()];
	for (iI = 0; iI < GC.getNumBuildInfos(); ++iI) {
		m_abWorkerImprovement[iI] = false;
	}

	for (iI = 0; iI < NUM_CITYSPECIALIZATION_TYPES; iI++)
	{
		m_aiCitySpecializationValue[iI] = 0;
		m_aiCitySpecializationValueShortTerm[iI] = 0;
	}

	for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		m_aiAI_valueYield[iI] = 0;
	}
	
	m_iCitySpecialization = NO_CITYSPECIALIZATION;

	m_iAI_WorstPlotIndex = FFreeList::INVALID_INDEX;

	m_iAIGroup_Defense = FFreeList::INVALID_INDEX;
	m_iAIGroup_Defense_Replace = FFreeList::INVALID_INDEX;
	m_iAIGroup_Worker = FFreeList::INVALID_INDEX;
/*************************************************************************************************/
/**	END                                                                  						**/
/*************************************************************************************************/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Start("CvCity::AI_doTurn::zero");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Stop("CvCity::AI_doTurn::zero");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
/*************************************************************************************************/
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Start("AI_updateBestBuild -- CvCity::AI_doTurn");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/

	//do this only rarely to increase performance
//	if((getImprovementInfrastructureCost()==0)
//		|| (GC.getGame().getElapsedGameTurns() % 10 == 0))
//	{		
		AI_updateBestBuild();
//		AI_updateWorkersNeededHere();
//	}
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Stop("AI_updateBestBuild -- CvCity::AI_doTurn");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
/*************************************************************************************************/
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Start("UpdateTradeRoutes -- CvCity::AI_doTurn");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Stop("UpdateTradeRoutes -- CvCity::AI_doTurn");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
/*************************************************************************************************/
...
bool CvCityAI::AI_stopGrowth()
{
	...
	//added for PopulationLimit Sephi
	if(GET_PLAYER(getOwnerINLINE()).getPopulationLimit()>-1)
	{
	    if(GET_PLAYER(getOwnerINLINE()).getPopulationLimit()+getPopulationLimit()<=getPopulation())
	    {
...
int CvCityAI::AI_specialistValue(SpecialistTypes eSpecialist, bool bAvoidGrowth, bool bRemove)
{
	...
	//use new method
	return AI_specialistValueNew(eSpecialist);
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Start("CvCity::AI_choose_Production::isProduction");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Stop("CvCity::AI_choose_Production::isProduction");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
//FfH: Modified by Kael 01/30/2009
//				if (kPlayer.AI_totalAreaUnitAIs(pWaterArea, UNITAI_ATTACK_SEA) < iNumCitiesInArea)
				if (kPlayer.AI_totalAreaUnitAIs(pWaterArea, UNITAI_ATTACK_SEA) < (iNumCitiesInArea / 3))
//FfH: End Modify
...
//		if (kPlayer.AI_getNumTrainAIUnits(UNITAI_ATTACK_SEA) + kPlayer.AI_getNumTrainAIUnits(UNITAI_PIRATE_SEA) + kPlayer.AI_getNumTrainAIUnits(UNITAI_RESERVE_SEA) < 3)
		if (kPlayer.AI_getNumTrainAIUnits(UNITAI_ATTACK_SEA) + kPlayer.AI_getNumTrainAIUnits(UNITAI_PIRATE_SEA) + kPlayer.AI_getNumTrainAIUnits(UNITAI_RESERVE_SEA) == 0)
//FfH: End Modify
...
/**						                                            							**/
/*************************************************************************************************/
            if (((iFocusFlags & BUILDINGFOCUS_HEALTHY) || (iPass > 0)) && !isNoUnhealthyPopulation() && !GET_PLAYER(getOwnerINLINE()).isIgnoreFood())
            {
/*************************************************************************************************/
/**	END                                                                  						**/
/*************************************************************************************************/
	...
	for (iI = 0; iI < GC.getNumHurryInfos(); iI++)
	{
		if (canHurry((HurryTypes)iI))
		{
			...
			if ((iHurryAngerLength == 0) && (iHurryPopulation == 0))
			{
				...
				{
/*************************************************************************************************/
/** BETTER AI (use stone to hurry buildings) Sephi                                              **/
/**						                                            							**/
...
/** orig
					...
					return;
**/
					if (GET_PLAYER(getOwnerINLINE()).AI_getGoldTreasury(false,true,false,false)>iHurryGold)
					{
						hurry((HurryTypes)iI);
						return;
					}
...
*/
				...
				if( (iHurryPopulation > 0) && (getPopulation() > iHurryPopulation) )
				{
					if (AI_countGoodTiles((healthRate(0) == 0), false, 100) <= (getPopulation() - iHurryPopulation))
					{
						//** Rolled back by Denev **//
						// Here is whipping process, then it doesn't require the condition about treasury.
						hurry((HurryTypes)iI);
						return;
					}
				}
...
void CvCityAI::AI_doEmphasize()
{
	...
	//TODO: write decent logic for AI
	for (iI = 0; iI < GC.getNumEmphasizeInfos(); iI++)
	{
...
//							int iValue = AI_plotValue(pLoopPlot, bAvoidGrowth, /*bRemove*/ false, /*bIgnoreFood*/ false, bIgnoreGrowth);
							int iValue = AI_plotValueNew(pLoopPlot);
	...
	if (iBestPlot != -1)
	{
		...
		//never work a Specialist if the City could still grow
		/** Disable this Stuff, Sephi
		if(!GET_PLAYER(getOwnerINLINE()).isIgnoreFood() && getCityIndexPlot(iBestPlot)->calculateNatureYield(YIELD_FOOD,getTeam())>0)
		{
			if(happyLevel()>unhappyLevel(0) && goodHealth()>badHealth())
			{
				if(foodDifference()<=4 )
				{
					eBestSpecialist = NO_SPECIALIST;
...
**/
	...
	if (eBestSpecialist != NO_SPECIALIST)
	{
		...
		//DEBUG
		TCHAR szOut[1024];
		sprintf(szOut, "%d Selected was a %S Specialist, Value:  %d\n",extraPopulation(), GC.getSpecialistInfo(eBestSpecialist).getDescription(),iBestSpecialistValue);
	...
	else if (iBestPlot != -1)
	{
		...
		sprintf(szOut, "%d Selected was a (%d,%d,%d) Plott, Value:  %d\n",extraPopulation(),pTarget->getYield(YIELD_FOOD),pTarget->getYield(YIELD_PRODUCTION),pTarget->getYield(YIELD_COMMERCE),iBestPlotValue);
		gDLL->logMsg("AIGoverneur.log",szOut, false, false);
		//DEBUG
...
int CvCityAI::AI_neededPermDefense()
{
	return GC.getDefineINT("NUM_GARRISION_UNITS_PER_CITY");
}

int CvCityAI::AI_CityDangerValue()
{
	...
	for (iDX = -(iRange); iDX <= iRange; iDX++)
	{
		for (iDY = -(iRange); iDY <= iRange; iDY++)
		{
			...
			if (pLoopPlot != NULL)
			{
				if ((pLoopPlot->area() == pPlotArea) || pLoopPlot->isWater())
				{
				    iDistance = stepDistance(getX_INLINE(), getY_INLINE(), pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE());
					...
					while (pUnitNode != NULL)
					{
						...
						if (pLoopUnit->isEnemy(getTeam()))
						{
							if (pLoopUnit->canAttack())
							{
								...
								{
								    if (pLoopUnit->canMoveOrAttackInto(plot()))
								    {
										//use the same scale as everywhere
								        Dangervalue+=pLoopUnit->currEffectiveStr(NULL, NULL);
								    }
								}
							}
						}
...
void CvCityAI::setAI_WorstPlotIndex(int iNewIndex)
{
	m_iAI_WorstPlotIndex=iNewIndex;
}

CvPlot* CvCityAI::getAI_WorstPlot() const
{
	if(m_iAI_WorstPlotIndex==FFreeList::INVALID_INDEX)
	{
		return NULL;
	}
	return getCityIndexPlot(m_iAI_WorstPlotIndex);	
}

//returns how Much the City is in Danger by Barbarians/other teams
int CvCityAI::AI_dangerValue()
{
	int iDanger=0;
	int iEnemyDanger;
	int iPowerRatio;
	...
	bool bCoastal=isCoastal(GC.getMIN_WATER_SIZE_FOR_OCEAN());
	for(int iDX=-iRange;iDX<=iRange;iDX++)
	{
		for(int iDY=-iRange;iDY<=iRange;iDY++)
		{
			...
			if(pLoopPlot)
			{
				...
				else if(pLoopPlot->getTeam()!=getTeam() && GET_TEAM(getTeam()).AI_getAttitude(pLoopPlot->getTeam())!=ATTITUDE_FRIENDLY)
				{
					iEnemyDanger=100;
					iPowerRatio=GET_TEAM(pLoopPlot->getTeam()).getPower(true);
					iPowerRatio/=std::max(1,GET_TEAM(getTeam()).getPower(true));
					iEnemyDanger*=iPowerRatio;
					iDanger+=iEnemyDanger;
				}
				else if(bCoastal)
				{
					iDanger+=50;
...
CvAIGroup* CvCityAI::getAIGroup_Defense()
{
	if(m_iAIGroup_Defense==FFreeList::INVALID_INDEX)
	{
		return NULL;
	}
	return GET_PLAYER(getOwnerINLINE()).getAIGroup(m_iAIGroup_Defense);
}

void CvCityAI::setAIGroup_Defense(CvAIGroup* pNewGroup)
{
	if(pNewGroup==NULL)
	{
		m_iAIGroup_Defense=FFreeList::INVALID_INDEX;
	...
	{
		m_iAIGroup_Defense=pNewGroup->getID();
	}
}

CvAIGroup* CvCityAI::getAIGroup_Defense_Replace()
{
	if(m_iAIGroup_Defense_Replace==FFreeList::INVALID_INDEX)
	{
		return NULL;
	}
	return GET_PLAYER(getOwnerINLINE()).getAIGroup(m_iAIGroup_Defense_Replace);
}

void CvCityAI::setAIGroup_Defense_Replace(CvAIGroup* pNewGroup)
{
	if(pNewGroup==NULL)
	{
		m_iAIGroup_Defense_Replace=FFreeList::INVALID_INDEX;
	...
	{
		m_iAIGroup_Defense_Replace=pNewGroup->getID();
	}
}

CvAIGroup* CvCityAI::getAIGroup_Worker()
{
	if(m_iAIGroup_Worker==FFreeList::INVALID_INDEX)
	{
		return NULL;
	}
	return GET_PLAYER(getOwnerINLINE()).getAIGroup(m_iAIGroup_Worker);
}

void CvCityAI::setAIGroup_Worker(CvAIGroup* pNewGroup)
{
	if(pNewGroup==NULL)
	{
		m_iAIGroup_Worker=FFreeList::INVALID_INDEX;
	...
	{
		m_iAIGroup_Worker=pNewGroup->getID();
	}
}

void CvCityAI::AI_updateCitySpecializationValues()
{
	...
	for(int iI=0;iI<NUM_CITYSPECIALIZATION_TYPES;iI++)
	{
		...
		for(int iJ=0;iJ<getNumCityPlots();iJ++)
		{
			CvPlot* pLoopPlot=getCityIndexPlot(iJ);
			{
				if(pLoopPlot!=NULL && pLoopPlot!=plot())
				{
					...
					if(pLoopPlot->getBonusType(getTeam())!=NO_BONUS)
					{
						if(GC.getBonusInfo(pLoopPlot->getBonusType(getTeam())).getYieldChange(YIELD_FOOD)>0)
						{
							if(iI==CITYSPECIALIZATION_GOLD)
							{
								iValue+=100;
							}
						}

						if(GC.getBonusInfo(pLoopPlot->getBonusType(getTeam())).getYieldChange(YIELD_COMMERCE)>0)
						{
							if(iI==CITYSPECIALIZATION_RESEARCH)
							{
								iValue+=100;
							}
						}
					...
					//Food Yield
					if(pLoopPlot->calculateNatureYield(YIELD_FOOD,getTeam())>=2)
					{
						if(iI==CITYSPECIALIZATION_GOLD)
						{
							iValue+=100;
					...
					{
						if(iI==CITYSPECIALIZATION_METAL)
						{
							iValue+=100;
						}
					}

					if(iI==CITYSPECIALIZATION_LUMBER || iI==CITYSPECIALIZATION_LEATHER)
					{
						if(pLoopPlot->getFeatureType()==eForest || pLoopPlot->getFeatureType()==eForestAncient)
						{
							iValue+=100;
						}
					}
		...
		{	
			...
			//if(getCommerceRateTimes100(COMMERCE_GOLD)>50)
			if(GC.getBUILDINGCLASS_MERCHANT_DISTRICT() != NO_BUILDINGCLASS) {
				BuildingTypes eSpecial=(BuildingTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(GC.getBUILDINGCLASS_MERCHANT_DISTRICT());
				if(eSpecial!=NO_BUILDING && getNumRealBuilding(eSpecial)>0)
				{
					iValue+=100000;
				}
			}
...
//			if(getCommerceRateTimes100(COMMERCE_RESEARCH)>50)
			if(GC.getBUILDINGCLASS_SAGE_DISTRICT() != NO_BUILDINGCLASS) {
				BuildingTypes eSpecial=(BuildingTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(GC.getBUILDINGCLASS_SAGE_DISTRICT());
				if(eSpecial!=NO_BUILDING && getNumRealBuilding(eSpecial)>0)
				{
					iValue+=100000;
				}
		...
		if(iI==CITYSPECIALIZATION_MILITARY)
		{
			iValue+=getYieldRate(YIELD_PRODUCTION)*100;
			iValue+=200*getFreeExperience();

			for(int iJ=0;iJ<GC.getNumBuildingInfos();iJ++)
			{
				if(getNumRealBuilding((BuildingTypes)iJ)>0)
				{
					if(::isTrainingBuilding((BuildingTypes)iJ,getOwnerINLINE(),true,true))
					{
						iValue+=400;
					}
				}
			...
			{
				iValue+=100*getUnitCombatFreeExperience((UnitCombatTypes)iJ);				
			}

			if(isCapital() && GC.getGame().getElapsedGameTurns() < GC.getGameSpeedInfo(GC.getGame().getGameSpeedType()).getTrainPercent()) {
				iValue += 10000;
			}
		}
		...
		if(iI==CITYSPECIALIZATION_METAL)
		{
			iValue+=getYieldIncome(YIELD_METAL)*100;

			if(GET_PLAYER(getOwnerINLINE()).AI_getYieldValue(YIELD_METAL)==0)
				iValue=0;
		}

		if(iI==CITYSPECIALIZATION_LUMBER)
		{
			iValue+=getYieldIncome(YIELD_LUMBER)*100;
			if(GET_PLAYER(getOwnerINLINE()).AI_getYieldValue(YIELD_LUMBER)==0)
				iValue=0;
		}

		if(iI==CITYSPECIALIZATION_LEATHER)
		{
			iValue+=getYieldIncome(YIELD_LEATHER)*100;
			if(GET_PLAYER(getOwnerINLINE()).AI_getYieldValue(YIELD_LEATHER)==0)
				iValue=0;
		}

		m_aiCitySpecializationValue[iI]=iValue;
	}
}

bool CvCityAI::AI_isWorkerImprovement(BuildTypes eBuild) const
{
	return m_abWorkerImprovement[eBuild];
}

int CvCityAI::AI_getCitySpecializationValue(CitySpecializationTypes eSpecialization) const
{
	return m_aiCitySpecializationValue[eSpecialization];
}

void CvCityAI::AI_updateCitySpecializationValuesShortTerm()
{
	CvPlayer& kPlayer=GET_PLAYER(getOwnerINLINE());
	int paiSpecialisedCities[NUM_CITYSPECIALIZATION_TYPES];
	int iLoop;

	for(int iI=0;iI<NUM_CITYSPECIALIZATION_TYPES;iI++)
	{
		paiSpecialisedCities[iI]=0;
	}
	//TODO: Cache the Number of Specialised Cities
	for(CvCity* pLoopCity=kPlayer.firstCity(&iLoop); pLoopCity!=NULL; pLoopCity=kPlayer.nextCity(&iLoop))
	{
		CvCityAI* pAICity=static_cast<CvCityAI*>(pLoopCity);
		if(pAICity->AI_getCitySpecialization()!=NO_CITYSPECIALIZATION)
		{
			paiSpecialisedCities[(int)pAICity->AI_getCitySpecialization()]++;
		}
	}
	...
	for(int iI=0;iI<NUM_CITYSPECIALIZATION_TYPES;iI++)
	{
		...
		//Rule number one: If we have more than two Cities, always pick one to specialize in Gold
		if(iI==CITYSPECIALIZATION_GOLD)
		{			
			if(paiSpecialisedCities[CITYSPECIALIZATION_GOLD]==0	&& kPlayer.getNumCities()>1)
			{
				if(!isCapital())
				{
					iValue=MAX_INT/4;
			...
			//never Specialise too many Cities into gold, not good longterm
			if(paiSpecialisedCities[CITYSPECIALIZATION_GOLD]>=2)
			{
				if(kPlayer.getNumCities()/3>paiSpecialisedCities[CITYSPECIALIZATION_GOLD])
				{
					iValue=-MAX_INT/4;
				}
			}
		...
		m_aiCitySpecializationValueShortTerm[iI]=iValue;
	}
}

int CvCityAI::AI_getCitySpecializationValueShortTerm(CitySpecializationTypes eSpecialization) const
{
	return m_aiCitySpecializationValueShortTerm[eSpecialization];
}

CitySpecializationTypes CvCityAI::AI_getCitySpecialization() const
{
	return (CitySpecializationTypes)m_iCitySpecialization;	
}

void CvCityAI::AI_setCitySpecialization(CitySpecializationTypes eSpecialization)
{
	m_iCitySpecialization=(int)eSpecialization;
}
...
/**						                                            							**/
/*************************************************************************************************/
	pStream->Read(NUM_CITYSPECIALIZATION_TYPES, m_aiCitySpecializationValue);
	pStream->Read(NUM_CITYSPECIALIZATION_TYPES, m_aiCitySpecializationValueShortTerm);
    pStream->Read(&m_iCitySpecialization);
	pStream->Read(NUM_YIELD_TYPES, m_aiAI_valueYield);

    pStream->Read(&m_iAI_WorstPlotIndex);

    pStream->Read(&m_iAIGroup_Defense);
    pStream->Read(&m_iAIGroup_Defense_Replace);
    pStream->Read(&m_iAIGroup_Worker);
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
/*************************************************************************************************/
...
/**	New Tag Defs							END													**/
/*************************************************************************************************/
	pStream->Read(GC.getNumBuildInfos(), m_abWorkerImprovement);
}
...
/**						                                            							**/
/*************************************************************************************************/
	pStream->Write(NUM_CITYSPECIALIZATION_TYPES, m_aiCitySpecializationValue);
	pStream->Write(NUM_CITYSPECIALIZATION_TYPES, m_aiCitySpecializationValueShortTerm);
    pStream->Write(m_iCitySpecialization);
	pStream->Write(NUM_YIELD_TYPES, m_aiAI_valueYield);

    pStream->Write(m_iAI_WorstPlotIndex);

    pStream->Write(m_iAIGroup_Defense);
    pStream->Write(m_iAIGroup_Defense_Replace);
    pStream->Write(m_iAIGroup_Worker);
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
/*************************************************************************************************/
...
/**	New Tag Defs							END													**/
/*************************************************************************************************/
	pStream->Write(GC.getNumBuildInfos(), m_abWorkerImprovement);
}
