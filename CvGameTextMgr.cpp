...
//FfH: End Modify
			...
			for (iI = 0; iI < GC.getNumBonusInfos(); ++iI)
			{
				int iChance = pPlot->getBonusSpawnChance((BonusTypes)iI, GC.getGameINLINE().getActivePlayer());
				...
				if(iChance > 0) {
					...
					szString.append(szTempBuffer);
					szTempBuffer.Format(L"(%d.%02d%%)", iChance / 100, iChance % 100);
					szString.append(szTempBuffer);
				}
			}
...
void CvGameTextMgr::parsePromotionHelp(CvWStringBuffer &szBuffer, PromotionTypes ePromotion, const wchar* pcNewline)
{
    ...
    if (kPromotion.getGoldCost() > 0)
    {
		...
		CvWString szTempBuffer;
		int iGoldCost = GC.getPromotionInfo(ePromotion).getGoldCost();
		iGoldCost *= std::max(20, 100 - GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getReducedEquipmentCost());
		iGoldCost /= 100;
        szTempBuffer.append(gDLL->getText("TXT_KEY_PROMOTION_GOLD_COST", iGoldCost));
		if(GC.getGameINLINE().getActivePlayer()!=NO_PLAYER && GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getGold()<iGoldCost && gDLL->getInterfaceIFace()->getHeadSelectedUnit()!=NULL)
    ...
    {
		szBuffer.append(pcNewline);
        szBuffer.append(gDLL->getText("TXT_KEY_UNIT_GOLD_GAIN", GC.getPromotionInfo(ePromotion).getGoldCost()));
	}
    ...
    if (kPromotion.getManaCost() > 0)
    {
		szBuffer.append(pcNewline);
		CvWString szTempBuffer;
		int iManaCost = GC.getPromotionInfo(ePromotion).getManaCost();
		iManaCost *= std::max(20, 100 - GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getReducedEquipmentCost());
		iManaCost /= 100;
        szTempBuffer.append(gDLL->getText("TXT_KEY_PROMOTION_MANA_COST", iManaCost));
		if(GC.getGameINLINE().getActivePlayer()!=NO_PLAYER && GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getMana()<iManaCost && gDLL->getInterfaceIFace()->getHeadSelectedUnit()!=NULL)
			szTempBuffer.Format( SETCOLR L"%s" ENDCOLR , TEXT_COLOR("COLOR_NEGATIVE_TEXT"), szTempBuffer.c_str());
		szBuffer.append(szTempBuffer);
	}
    ...
    if (kPromotion.getFaithCost() > 0)
    {
		szBuffer.append(pcNewline);
		CvWString szTempBuffer;
		int iFaithCost = GC.getPromotionInfo(ePromotion).getFaithCost();
		iFaithCost *= std::max(20, 100 - GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getReducedEquipmentCost());
		iFaithCost /= 100;
        szTempBuffer.append(gDLL->getText("TXT_KEY_PROMOTION_FAITH_COST", iFaithCost));
		if(GC.getGameINLINE().getActivePlayer()!=NO_PLAYER && GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getFaith()<iFaithCost && gDLL->getInterfaceIFace()->getHeadSelectedUnit()!=NULL)
			szTempBuffer.Format( SETCOLR L"%s" ENDCOLR , TEXT_COLOR("COLOR_NEGATIVE_TEXT"), szTempBuffer.c_str());
		szBuffer.append(szTempBuffer);
	}
    ...
    if (kPromotion.getYieldCostType()!=NO_YIELD && kPromotion.getYieldCost(1)>0)
    {
		...
		if(bPopup)
		{
			...
			{
				int iTier=GC.getUnitInfo(gDLL->getInterfaceIFace()->getHeadSelectedUnit()->getUnitType()).getTier();
				iYieldCost=GC.getPromotionInfo(ePromotion).getYieldCost(iTier);
			}
			iYieldCost*=std::max(50,100-GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getReducedEquipmentCost());
			iYieldCost/=100;
		}
		szTempBuffer.clear();
		...
		if(bPopup && iYieldCost>GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getGlobalYield(GC.getPromotionInfo(ePromotion).getYieldCostType()))
		{
...
/** Equipment **/
	...
	{
		if(kPromotion.isMakeEquipmentDurabilityValid(iI))
		{
			if(!bFirst)
			{
				szBuffer.append(L", ");
...
//FfH: Added by Kael 07/30/2007
	...
	for (iI = 0; iI < GC.getNumDamageTypeInfos(); iI++)
	{
        ...
        if (GC.getPromotionInfo(ePromotion).getCombatAuraDamageTypeResist((DamageTypes)iI) != 0)
		{
            ...
            {
                szBuffer.append(gDLL->getText("TXT_KEY_PROMOTION_DAMAGE_TYPE_RESIST_AURA", GC.getPromotionInfo(ePromotion).getCombatAuraDamageTypeResist((DamageTypes)iI), GC.getDamageTypeInfo((DamageTypes)iI).getDescription()));
            }
		}
	}
	for (iI = 0; iI < GC.getNumBonusInfos(); iI++)
...
/*************************************************************************************************/
	...
	if (GC.getPromotionInfo(ePromotion).getCombatAuraCaptureChanceChange() > 0)
	{
		szBuffer.append(pcNewline);
		szBuffer.append(gDLL->getText("TXT_KEY_PROMOTION_AURA_CAPTURE_CHANCE_EFFECT"));
	}

	if (GC.getPromotionInfo(ePromotion).getCombatAuraEnslaveChanceChange() > 0)
	{
		szBuffer.append(pcNewline);
		szBuffer.append(gDLL->getText("TXT_KEY_PROMOTION_AURA_ENSLAVE_CHANCE_EFFECT"));
	}
...
/** New Definitions **/
	...
	if (kSpell.isManaCostReductionBonus() && kSpell.getTechPrereq()!=NO_TECH)
	{
		...
		{
			szBuffer.append(pcNewline);
			szBuffer.append(gDLL->getText("TXT_KEY_PROJECT_GLOBAL_ENCHANTMENT_BONUS_REDUCED_COST",getLinkedText(eBonus).c_str()));
		}
	}
	...
	if(kSpell.isTargetedHostile())
	{
		if(kSpell.getDamage()!=0 && kSpell.getDamageType()!=NO_DAMAGE)
		{
			...
			if(pHeadSelectedUnit != NULL) {
				...
				iPromotionBonus = (iBaseDamage * pHeadSelectedUnit->getSpellDamageModify()) / 100;
			}
			if(pHeadSelectedUnit==NULL || (iManaBonus == 0 && iPromotionBonus == 0))
			{
				szTempBuffer.Format(L"%c%d%% %s %s %d%%", gDLL->getSymbolID(BULLET_CHAR), GC.getSpellInfo(eSpell).getDamage(), GC.getDamageTypeInfo((DamageTypes)GC.getSpellInfo(eSpell).getDamageType()).getDescription(), gDLL->getText("TXT_KEY_SPELL_TARGETED_HOSTILE_DAMAGE").c_str(),GC.getSpellInfo(eSpell).getDamageLimit());
				szBuffer.append(szTempBuffer);
			...
			{
				szBuffer.append(gDLL->getText("TXT_KEY_SPELL_TARGETED_HOSTILE_DAMAGE_DETAIL",iBaseDamage + iManaBonus + iPromotionBonus, iBaseDamage, GC.getDamageTypeInfo((DamageTypes)GC.getSpellInfo(eSpell).getDamageType()).getDescription()));
				if(iManaBonus != 0) 
		...
		{
			szTempBuffer.Format(L"%d", kSpell.getNumTargets());
		}
		else
		{
			szTempBuffer.Format(L"all");
		}
		szBuffer.append(gDLL->getText("TXT_KEY_SPELL_TARGETED_HOSTILE",szTempBuffer.c_str()));
...
/*************************************************************************************************/
	...
	if(kUnit.getGlobalYieldTypeCost() != NO_YIELD && kUnit.getGlobalYieldCost() > 0)
    {
		...
		szBuffer.append(NEWLINE);
        szBuffer.append(gDLL->getText("TXT_KEY_BUILDING_GLOBAL_YIELD_COST", kUnit.getGlobalYieldCost(), GC.getYieldInfo((YieldTypes)kUnit.getGlobalYieldTypeCost()).getChar()));
    }
...
/*************************************************************************************************/
	...
	if(kBuilding.getGlobalYieldTypeCost() != NO_YIELD && kBuilding.getGlobalYieldCost() > 0)
    {
        ...
        szBuffer.append(NEWLINE);
        szBuffer.append(gDLL->getText("TXT_KEY_BUILDING_GLOBAL_YIELD_COST", kBuilding.getGlobalYieldCost(), GC.getYieldInfo((YieldTypes)kBuilding.getGlobalYieldTypeCost()).getChar()));
    }
...
/*************************************************************************************************/
	...
	if (kProject.getFaithCost() != 0 || iProjectFaithUpkeep!=0)
	{
		if(iProjectFaithUpkeep>=0)
		{
			szBuffer.append(NEWLINE);
			szBuffer.append(gDLL->getText("TXT_KEY_PROJECT_FAITH_COST", kProject.getFaithCost(), iProjectFaithUpkeep));
		}

		if(iProjectFaithUpkeep<0)
		{
			szBuffer.append(NEWLINE);
			szBuffer.append(gDLL->getText("TXT_KEY_PROJECT_FAITH_INCOME", kProject.getFaithCost(), -iProjectFaithUpkeep));
		}
	}
	...
	{
		szBuffer.append(NEWLINE);
		szBuffer.append(gDLL->getText("TXT_KEY_PROJECT_GLOBAL_ENCHANTMENT_BONUS_REDUCED_COST",getLinkedText((BonusTypes)GC.getTechInfo((TechTypes)kProject.getTechPrereq()).getPrereqBonus()).c_str()));
	}
...
//<<<<	BUGFfH: End Modify
	...
	for (int iI = 0; iI < GC.getNumTerrainInfos(); iI++)
	{
		for (int iJ = 0; iJ < NUM_YIELD_TYPES; iJ++)
		{
			int iReligionTerrainYield = religion.getReligionTerrainYields(iI, iJ);
			if (iReligionTerrainYield != 0)
			{
				szBuffer.append(gDLL->getText("TXT_KEY_CIVILIZATION_YIELD_CHANGE", iReligionTerrainYield, GC.getYieldInfo((YieldTypes)iJ).getChar(), GC.getTerrainInfo((TerrainTypes)iI).getDescription()));
			}
		}
	}
...
//>>>>	BUGFfH: Added by Denev 2009/09/29
	...
	for (int iBonus = 0; iBonus < GC.getNumBonusInfos(); iBonus++)
	{
		...
		{
			//Resources have only research modifier.
...
**/
	...
	{
		if(city.calculateTaxes() != 0)
		{
			szBuffer.append(gDLL->getText("TXT_KEY_MISC_TAXES_FROM_POPULATION", city.calculateTaxes()));
			szBuffer.append(NEWLINE);
...
void CvGameTextMgr::setFaithStr(CvWString& szString, PlayerTypes ePlayer)
{
	...
	int iFaithRate = GET_PLAYER(ePlayer).getFaithIncome() - (GET_PLAYER(ePlayer).getFaithIncome() / 2);
...
void CvGameTextMgr::setCombatAuraStr(CvWString& szString, CvCombatAura* pAura, const CvUnit* pUnit, int iType)
{
	...
	if (pAura->getCaptureChance() != 0)
	{
		szString.append(NEWLINE);
		szString.append(gDLL->getText("TXT_KEY_AURA_CAPTURE_CHANCE", (pAura->getCaptureChance()) * (pAura->isBlessing() ? 1 : -1)));
	}

	if (pAura->getEnslaveChance() != 0)
	{
		szString.append(NEWLINE);
		szString.append(gDLL->getText("TXT_KEY_AURA_ENSLAVE_CHANCE", (pAura->getEnslaveChance()) * (pAura->isBlessing() ? 1 : -1)));
	}

	if (pAura->getChanceMiscast() != 0)
	{
		szString.append(NEWLINE);
		szString.append(gDLL->getText("TXT_KEY_AURA_CHANCE_MISCAST", (pAura->getChanceMiscast()) * (pAura->isBlessing() ? 1 : -1)));
	}
