...
/*************************************************************************************************/
	...
	DllExport void setImprovementType(ImprovementTypes eNewValue, bool bWorker = false);																									// Exposed to Python
...
/**						                                            							**/
/*************************************************************************************************/
    bool isPirateCove() const;
    bool isPirateCoveValid(PlayerTypes ePlayer) const;
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
/*************************************************************************************************/
...
/*************************************************************************************************/
	bool isAIGroupClose(PlayerTypes ePlayer, int iRange, int iAIGroupType, int iFlag);
    int AI_neededBetterDefender(int type);
	int ActualImprovementChanges(PlayerTypes ePlayer, BuildTypes eBuild, YieldTypes eYield, CommerceTypes eCommerce, bool bSecondary, bool bCheckPlotOnly = false);
	int ActualImprovementValues(PlayerTypes ePlayer, BuildTypes eBuild, YieldTypes eYield);
	int ActualImprovementYield(PlayerTypes ePlayer, ImprovementTypes eImprovement, YieldTypes eYield);
	int ActualSecondaryImprovementChanges(PlayerTypes ePlayer, ImprovementTypes eNewImprovement, YieldTypes eYield, CommerceTypes eCommerce);
	int calculateExtraYieldfromAdjacentPlot(CvPlot* pPlot, YieldTypes eYield, ImprovementTypes eImprovement, ImprovementTypes eImprovementAdjacent) const;
	bool isExtraYieldfromAdjacentImprovementValid(const CvPlot* pPlot, ImprovementTypes eImprovement) const;
	bool canHaveImprovementClassType(ImprovementTypes eImprovement, int iClass=-1) const;
	int getCityYield(YieldTypes eYield);

	CvPlot* getcloseSecurePlot(TeamTypes eTeam) const;

	bool isLair() const;
	bool isWildmanaLair() const;
	bool isValidCityStateLocation() const;
	bool isAdjacentToPeak() const;
	bool isImprovementOrUpgrade(ImprovementTypes eImprovement) const;
	bool isImprovementToExpensive(PlayerTypes eOwner, ImprovementTypes eNewImprovement) const;

	bool isPlundered() const;
	void setPlundered(int iNewValue);
	int getPlundered() const;

	bool canBePillaged() const; 
	bool isDefensiveBonuses(const CvUnit* pUnit, int iModifier) const;


	int StartingPlotValue(CivilizationTypes eCiv, int iPass) const;

	void createDungeon(DungeonTypes eDungeon);
	void destroyDungeon();
	CvDungeon* getDungeon() const;

	//Pathfinding
	// int getRegion() const;
	// void setRegion(int iNewRegion);
	// int getSector() const;
	// void setSector(int iNewSector);
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
/*************************************************************************************************/
...
/*************************************************************************************************/
	void doLairSpawnGroup();
	void doLairDefenderSpawn();
	void doLairTurn();
    int getAnimalDensity(bool bPlusLairs);
	int getDungeonType() const;
	void setDungeonType(int iNewValue);
	int getWilderness() const;
	void setWilderness(int iNewValue);
	PlaneTypes getPlane() const;
	void setPlane(PlaneTypes iNewValue);
	bool isWildmanaImmune() const;
	void setWildmanaImmune(bool bNewValue);
	bool isNonPlanar() const;
	void setNonPlanar(bool bNewValue);
    void calculateAnimalDensity();
    int getBarBarianDensity(bool bPlusLairs);
    void calculateBarBarianDensity();
	void CreateLair(CivilizationTypes eCiv = NO_CIVILIZATION);
	bool isInRangeLair(int iRange,CivilizationTypes eOwnerCiv = NO_CIVILIZATION);
	bool isValidLairLocation();
	int getBarBarianActivity();
	void doDevilGateSpawn();
	bool isValidforBarbSpecialAction();
	void SpawnBarbarianUnit(UnitTypes eUnit, PlayerTypes ePlayer, bool bLeashed = true, UnitAITypes eUnitAI = UNITAI_ATTACK, bool bLoyalty = false);
	bool doTerraformRitual(ProjectTypes eRitual, PlayerTypes eCaster, int& iValue, bool bCountOnly);

	void createPortal(int iTargetArea = -1);
	void destroyPortal();
	void setPortalExit(int iPlotIndex); 
	CvPlot* getPortalExit() const;
	bool isPortal() const;
	bool canCreatePortal() const;
	bool isInRangePortal(int iRange) const;
	bool isNearUF(int iRange) const;
	int getID() const;
...
/*************************************************************************************************/
	CvUnit* getBestRangedDefender(PlayerTypes eOwner, PlayerTypes eAttackingPlayer = NO_PLAYER, const CvUnit* pAttacker = NULL, bool bTestAtWar = false, bool bTestPotentialEnemy = false, bool bTestCanMove = false) const;
/*************************************************************************************************/
/**	Tweak									END													**/
/*************************************************************************************************/
...
/**							Adds extra Barbarian Civilizations									**/
/*************************************************************************************************/
    int m_iAnimalDensity;
    int m_iAnimalDensityPlusLairs;
    int m_iBarBarianDensity;
    int m_iBarBarianDensityPlusLairs;
	int m_iDungeonType;
	int m_iWilderness;
	int m_iPlane;
	int m_iPlundered;
	bool m_bWildmanaImmune;
	bool m_bNonPlanar;
	int m_iDungeon;
	int m_iPortalExit;
	bool m_bGraphicsPageIndex;
/*************************************************************************************************/
/**	MultiBarb								END													**/
/*************************************************************************************************/

	/* Fourth Yield (Asaf) - Start */
	enum RenderOffsetMode
	{
		en_RenderOffsetNormal,
		en_RenderOffsetLeft,
		en_RenderOffsetRight
	};
	RenderOffsetMode m_eCurSymbolRenderOffsetMode;
	std::vector<RenderOffsetMode> m_symbolsOffsetModes;

	void updateSymbolsForRange(int iYieldStart, int iYieldEnd, int* arrYieldAmounts);
...
/**						                                            							**/
/*************************************************************************************************/
    static bool c_bPirateCoveTypesLoaded;
    static std::set<ImprovementTypes> c_aePirateCoveTypes;
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
	//Pathfinding
