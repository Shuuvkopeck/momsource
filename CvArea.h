...
public:
  ...
  void init(int iID, bool bWater);
	...
	int AI_getPotentialCounterUnits(PlayerTypes ePlayer) const;
	CvAIGroup* getReserveGroup(PlayerTypes ePlayer) const;
    /**
    *   CAPITULATION_EDIT - Till - 20.04.2010
    **/
    int getCitiesPerTeam(PlayerTypes eIndex) const;
    /**
    *   END
    **/
	int getPlane() const;
	...
	bool canAreaBeReachedThroughPortal(int iArea);
	bool isConnectedByPortalToArea(int iArea, int iRecursionLevel);
	int getNumPortals() const;
	void changeNumPortals(int iChange);
