...
//FfH: Added by Kael 11/04/2007
	case BUTTONPOPUP_CONFIRMCASTGLOBAL:
		if (pPopupReturn->getButtonClicked() == 0)
		{
			int iAction = info.getData1();
			GC.getGameINLINE().selectionListGameNetMessage(GAMEMESSAGE_DO_COMMAND, GC.getActionInfo(iAction).getCommandType(), GC.getActionInfo(iAction).getCommandData(), -1, 0, info.getOption1());
		...
		break;
	case BUTTONPOPUP_CONFIRMCASTWAR:
		if (pPopupReturn->getButtonClicked() == 0)
		{
			int iAction = info.getData1();
			GC.getGameINLINE().selectionListGameNetMessage(GAMEMESSAGE_DO_COMMAND, GC.getActionInfo(iAction).getCommandType(), GC.getActionInfo(iAction).getCommandData(), -1, 0, info.getOption1());
...
//FfH: End Add
	...
	case BUTTONPOPUP_CHOOSETECH:
		if (pPopupReturn->getButtonClicked() == GC.getNumTechInfos())
		{
			CyArgsList argsList;
			argsList.add(0);
			gDLL->getPythonIFace()->callFunction("CvScreensInterface", "showTechChooser", argsList.makeFunctionArgs());
...
/*************************************************************************************************/
	case BUTTONPOPUP_NEW_TRAIT:
		{
			...
			if (0 == pPopupReturn->getButtonClicked()) {
				CvMessageControl::getInstance().sendDoTask(info.getData2(), TASK_NEW_TRAIT, info.getData1(), -1, false, false, false, false);
				gDLL->getInterfaceIFace()->setScreenDying(30, true);
			}
			break;
		}

	case BUTTONPOPUP_SUMMON:
		{
			CyArgsList argsList;
			argsList.add(0);
			gDLL->getPythonIFace()->callFunction(PYGameModule, "Trigger_MagicScreenPopupEnd", argsList.makeFunctionArgs());

			if (0 == pPopupReturn->getButtonClicked())
			{
				CvMessageControl::getInstance().sendDoTask(info.getData2(),TASK_SUMMON,info.getData1(),-1,false,false,false,false);
				gDLL->getInterfaceIFace()->setScreenDying(31,true);
			}
			break;
		}

	case BUTTONPOPUP_TERRAFORM:
		{
			CyArgsList argsList;
			argsList.add(0);
			gDLL->getPythonIFace()->callFunction(PYGameModule, "Trigger_MagicScreenPopupEnd", argsList.makeFunctionArgs());
			...
			{
				gDLL->getInterfaceIFace()->setScreenDying(31,true);
				CvMessageControl::getInstance().sendDoTask(info.getData2(),TASK_TERRAFORM,info.getData1(),-1,false,false,false,false);
			}				
			if (1 == pPopupReturn->getButtonClicked())
			{
				...
				argsList1.add(GC.getGame().getActivePlayer());
				argsList1.add(info.getData1());
				gDLL->getPythonIFace()->callFunction(PYGameModule, "NetMessage_SendTerraformPlan", argsList1.makeFunctionArgs());
				gDLL->getInterfaceIFace()->selectUnit(GET_PLAYER(GC.getGame().getActivePlayer()).firstUnit(&iLoop), false, false, true);
				gDLL->getInterfaceIFace()->setInterfaceMode(INTERFACEMODE_TERRAFORM);
			}

			break;
		}

	case BUTTONPOPUP_GLOBALENCHANTMENT:
		{
			CyArgsList argsList;
			argsList.add(0);
			gDLL->getPythonIFace()->callFunction(PYGameModule, "Trigger_MagicScreenPopupEnd", argsList.makeFunctionArgs());
			...
			if (0 == pPopupReturn->getButtonClicked())
			{
				CvMessageControl::getInstance().sendDoTask(info.getData2(),TASK_GLOBAL_ENCHANTMENT,info.getData1(),-1,false,false,false,false);
				gDLL->getInterfaceIFace()->setScreenDying(31,true);
			}				
			break;
		}

	case BUTTONPOPUP_GLOBALENCHANTMENT_CANCEL:
		{
			CyArgsList argsList;
			argsList.add(0);
			gDLL->getPythonIFace()->callFunction(PYGameModule, "Trigger_MagicScreenPopupEnd", argsList.makeFunctionArgs());
			...
			if (0 == pPopupReturn->getButtonClicked())
			{
				CvMessageControl::getInstance().sendDoTask(info.getData2(),TASK_REMOVE_GLOBAL_ENCHANTMENT,info.getData1(),-1,false,false,false,false);
				gDLL->getInterfaceIFace()->setScreenDying(31,true);
			}				
			break;
		}

	case BUTTONPOPUP_PICK_EQUIPMENT:
		{
			int ePromotion=pPopupReturn->getButtonClicked();
			bool bGroup=false;
			if(ePromotion>=EQUIPMENT_POPUP_SHIFT)
			{
				bGroup=true;
				ePromotion-=EQUIPMENT_POPUP_SHIFT;
			}
			if(ePromotion!=NO_PROMOTION && gDLL->getInterfaceIFace()->getHeadSelectedUnit()!=NULL)
			{
				CvMessageControl::getInstance().sendDoCommand(gDLL->getInterfaceIFace()->getHeadSelectedUnit()->getID(), COMMAND_PICK_EQUIPMENT, ePromotion, -1, false);	
				if(bGroup)
				{
					CvSelectionGroup* pGroup=gDLL->getInterfaceIFace()->getHeadSelectedUnit()->getGroup();
					...
					while (pUnitNode != NULL)
					{
						...
						if (pLoopUnit != NULL)
						{
							if(GET_PLAYER(pGroup->getOwnerINLINE()).canEquipUnit(pLoopUnit, (PromotionTypes)ePromotion))
							{
								CvMessageControl::getInstance().sendDoCommand(pLoopUnit->getID(), COMMAND_PICK_EQUIPMENT, ePromotion, -1, false);	
							}
						}
					}
				}
	...
	case BUTTONPOPUP_CHOOSESPELLRESEARCH:
		if (pPopupReturn->getButtonClicked() == GC.getNumTechInfos())
		{
			gDLL->getPythonIFace()->callFunction("CvScreensInterface", "showArcaneScreen");
		}
		break;

	case BUTTONPOPUP_PICK_SACREDKNOWLEDGE:
		//broadcast that we picked == bought the tech
		CvMessageControl::getInstance().sendResearch(((TechTypes)pPopupReturn->getButtonClicked()), GC.getGameINLINE().getActivePlayer(), false);
		break;

	case BUTTONPOPUP_CITY_INFRASTRUCTURE_INCREASE:
		if (0 == pPopupReturn->getButtonClicked())
		{
			CvMessageControl::getInstance().sendDoTask(info.getData1(),TASK_CITY_INFRASTRUCTURE_INCREASE,-1,-1,false,false,false,false);
		}				
		break;

	case BUTTONPOPUP_CULTURAL_ACHIEVEMENT:
		{
			/**
			CyArgsList argsList;
			argsList.add(0);
			gDLL->getPythonIFace()->callFunction(PYGameModule, "Trigger_GuildScreenPopupEnd", argsList.makeFunctionArgs());
			**/
			if (0 == pPopupReturn->getButtonClicked())
			{
				CvMessageControl::getInstance().sendDoTask(info.getData2(),TASK_CULTURAL_ACHIEVEMENT,info.getData1(),-1,false,false,false,false);
			}
			break;
		}
	case BUTTONPOPUP_CHOOSE_GUILD:
		//broadcast that we picked == bought the tech
		CvMessageControl::getInstance().sendResearch(((TechTypes)pPopupReturn->getButtonClicked()), GC.getGameINLINE().getActivePlayer(), false);
		break;
	case BUTTONPOPUP_CHOOSE_RELIGION:
		//broadcast that we picked == bought the tech
		CvMessageControl::getInstance().sendResearch(((TechTypes)pPopupReturn->getButtonClicked()), GC.getGameINLINE().getActivePlayer(), false);
		break;
	case BUTTONPOPUP_DUNGEON_EVENT:
		{
			int iResult = pPopupReturn->getButtonClicked();
			CvMessageControl::getInstance().sendDoTask(info.getData2(),TASK_CITY_DUNGEON_EVENT, info.getData1(), iResult, false,false,false,false);
		}
		break;
/*************************************************************************************************/
/**	END                                                                                         **/
...
/**
			...
			{
				GC.getCurrentViewport().bringIntoView(pCity->getX_INLINE(), pCity->getY_INLINE());
			}
/*************************************************************************************************/
/**	END Viewport C2C			                                        						**/
/*************************************************************************************************/
...
/*************************************************************************************************/
	...
	case BUTTONPOPUP_CHOOSESPELLRESEARCH:
		if (info.getData1() == 0)
		{
			if ((GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getSpellResearchFocus() != NO_TECH) || (GC.getGameINLINE().getGameState() == GAMESTATE_OVER))
			{
				gDLL->getInterfaceIFace()->popupSetAsCancelled(pPopup);
			}
		}
...
//FfH: Added by Kael 11/04/2007
	case BUTTONPOPUP_CONFIRMCASTGLOBAL:
		bLaunched = launchConfirmCastGlobalPopup(pPopup, info);
		break;
	case BUTTONPOPUP_CONFIRMCASTWAR:
		bLaunched = launchConfirmCastWarPopup(pPopup, info);
		break;
	case BUTTONPOPUP_CONFIRMSETTLEMENT:
...
/*************************************************************************************************/
	case BUTTONPOPUP_NEW_TRAIT:
		bLaunched = launchNewTraitPopup(pPopup, info);
		break;

	case BUTTONPOPUP_SUMMON:
		bLaunched = launchSummonPopup(pPopup, info);
		break;

	case BUTTONPOPUP_TERRAFORM:
		bLaunched = launchTerraformPopup(pPopup, info);
		break;

	case BUTTONPOPUP_GLOBALENCHANTMENT:
		bLaunched = launchGlobalenchantmentPopup(pPopup, info);
		break;

	case BUTTONPOPUP_GLOBALENCHANTMENT_CANCEL:
		bLaunched = launchRemoveGlobalenchantmentPopup(pPopup, info);
		break;
	case BUTTONPOPUP_PICK_EQUIPMENT:
		bLaunched = launchPickEquipmentPopup(pPopup, info);
		break;
	case BUTTONPOPUP_CHOOSESPELLRESEARCH:
		bLaunched = launchChooseSpellResearchPopup(pPopup, info);
		break;
	case BUTTONPOPUP_PICK_SACREDKNOWLEDGE:
		bLaunched = launchChooseSacredKnowledgePopup(pPopup, info);
		break;
	case BUTTONPOPUP_CITY_INFRASTRUCTURE_INCREASE:
		bLaunched = launchCityInfrastructureIncreasePopup(pPopup, info);
		break;
	case BUTTONPOPUP_CULTURAL_ACHIEVEMENT:
		bLaunched = launchCulturalAchievementPopup(pPopup, info);
		break;
	case BUTTONPOPUP_CHOOSE_GUILD:
		bLaunched = launchChooseGuildPopup(pPopup, info);
		break;
	case BUTTONPOPUP_CHOOSE_RELIGION:
		bLaunched = launchChooseReligionPopup(pPopup, info);
		break;
	case BUTTONPOPUP_DUNGEON_EVENT:
		bLaunched = launchDungeonEventPopup(pPopup, info);
		break;
/*************************************************************************************************/
/**	END                                                                                         **/
/*************************************************************************************************/
...
//FfH: End Add
	...
	/** disable useless Recommendations
	...
	if (eProductionUnit != NO_UNIT && GC.getUnitInfo(eProductionUnit).getAdvisorType()!=NO_ADVISOR)
	{
		...
		szButton = pCity->getUnitArtStyleButton(eProductionUnit);
		gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, szBuffer, szButton, GC.getUnitInfo(eProductionUnit).getUnitClassType(), WIDGET_TRAIN, GC.getUnitInfo(eProductionUnit).getUnitClassType(), pCity->getID(), true, POPUP_LAYOUT_STRETCH, DLL_FONT_LEFT_JUSTIFY );
		iNumBuilds++;
	}
	...
	**/
	...
	for (int iPass = 0; iPass < 8; iPass++)
	{
		...
		switch(iPass)
		{
			case 0:
				szTempBuffer.clear();
				szTempBuffer.append(L" Districts - choose very careful: ");
				szLoopBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_TECH_TEXT"),szTempBuffer.c_str());
				break;
			case 1:
				szTempBuffer.clear();
				szTempBuffer.append(L" Military: ");
				szLoopBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_TECH_TEXT"),szTempBuffer.c_str());
				break;
			case 2:
				szTempBuffer.clear();
				szTempBuffer.append(L" Growth and Health: ");
				szLoopBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_TECH_TEXT"),szTempBuffer.c_str());
				break;
			case 3:
				szTempBuffer.clear();
				szTempBuffer.append(L" Gold and Resources: ");
				szLoopBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_YELLOW"),szTempBuffer.c_str());
				break;
			case 4:
				szTempBuffer.clear();
				szTempBuffer.append(L" Culture: ");
				szLoopBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_MAGENTA"),szTempBuffer.c_str());
				break;
			case 5:
				szTempBuffer.clear();
				szTempBuffer.append(L" Research and Magic: ");
				szLoopBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_ALT_HIGHLIGHT_TEXT"),szTempBuffer.c_str());
				break;
			case 6:
				szTempBuffer.clear();
				szTempBuffer.append(L" Misc: ");
				szLoopBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_CITY_GREY"),szTempBuffer.c_str());
				break;
			case 7:
				szTempBuffer.clear();
				szTempBuffer.append(L" Wonders: ");
				szLoopBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"),szTempBuffer.c_str());
				break;
			default:
				break;
		}
			...
			if (eLoopUnit != NO_UNIT)
			{
				...
				if (eLoopUnit != eProductionUnit)
				{
					if (pCity->canTrain(eLoopUnit))
					{
						if(bFirst)
						{
							gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szLoopBuffer);
							bFirst=false;
						}
						...
						szButton = pCity->getUnitArtStyleButton(eLoopUnit);
						gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, szBuffer, szButton, iI, WIDGET_TRAIN, iI, pCity->getID(), true, POPUP_LAYOUT_STRETCH, DLL_FONT_LEFT_JUSTIFY );
		...
		for (int iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
		{
			...
			{
				bool bProvideResources = (pCity->getAdditionalBaseYieldRateByBuilding(YIELD_LUMBER,eLoopBuilding) > 0
										|| pCity->getAdditionalBaseYieldRateByBuilding(YIELD_LEATHER,eLoopBuilding)> 0
										|| pCity->getAdditionalBaseYieldRateByBuilding(YIELD_METAL,eLoopBuilding)> 0
										|| pCity->getAdditionalBaseYieldRateByBuilding(YIELD_STONE,eLoopBuilding)> 0
										|| pCity->getAdditionalBaseYieldRateByBuilding(YIELD_HERB,eLoopBuilding)> 0);
				bool bWonder = (GC.getBuildingClassInfo((BuildingClassTypes)iI).getMaxGlobalInstances() != -1
								||	GC.getBuildingClassInfo((BuildingClassTypes)iI).getMaxPlayerInstances() != -1 
								||	GC.getBuildingClassInfo((BuildingClassTypes)iI).getMaxTeamInstances() != -1);
				bool bDistrict = GC.getBuildingInfo(eLoopBuilding).isDistrict();
				bool bMilitary = pCityAI->AI_buildingMilitaryValue(eLoopBuilding) > 0;
				bool bGrowth = pCityAI->AI_buildingGrowthValue(eLoopBuilding) > 0;

				bool bGold = ((pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_GOLD,eLoopBuilding) > 0)
							&& (pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_GOLD,eLoopBuilding) > pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_RESEARCH,eLoopBuilding) )
							&& (pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_GOLD,eLoopBuilding) > pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_CULTURE,eLoopBuilding) )
							&& (pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_GOLD,eLoopBuilding) > pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_MANA,eLoopBuilding) )
							&& (pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_GOLD,eLoopBuilding) > pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_ARCANE,eLoopBuilding) ));
				bool bCulture = ((pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_CULTURE,eLoopBuilding) > 0)
							&& (pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_CULTURE,eLoopBuilding) > pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_RESEARCH,eLoopBuilding) )
							&& (pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_CULTURE,eLoopBuilding) > pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_MANA,eLoopBuilding) )
							&& (pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_CULTURE,eLoopBuilding) > pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_ARCANE,eLoopBuilding) ));

				bool bResearch = ((pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_RESEARCH,eLoopBuilding) > 0) 
									|| (pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_MANA,eLoopBuilding) > 0) 
									|| (pCity->getAdditionalCommerceTimes100ByBuilding(COMMERCE_ARCANE,eLoopBuilding) > 0)); 
				if(bDistrict) {
					if(iPass != 0)
						continue;
				}
		...
		{
			//add Processes to Gold/Research
			if(!(	(iPass==3 && GC.getProcessInfo((ProcessTypes)iI).getProductionToCommerceModifier(COMMERCE_GOLD)>0)
				||	(iPass==4 && GC.getProcessInfo((ProcessTypes)iI).getProductionToCommerceModifier(COMMERCE_RESEARCH)>0)
				||	(iPass==6 && GC.getProcessInfo((ProcessTypes)iI).getProductionToCommerceModifier(COMMERCE_CULTURE)>0)
				||	(iPass==7 && GC.getProcessInfo((ProcessTypes)iI).getProductionToCommerceModifier(COMMERCE_GOLD)==0 
							  && GC.getProcessInfo((ProcessTypes)iI).getProductionToCommerceModifier(COMMERCE_RESEARCH)==0
							  && GC.getProcessInfo((ProcessTypes)iI).getProductionToCommerceModifier(COMMERCE_CULTURE)==0)))
			{
				continue;
			}
...
bool CvDLLButtonPopup::launchChooseTechPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	...
	for (int iPass=0;iPass<7;iPass++)
	{
		...
		switch(iPass)
		{
			...
			case 1:
				szTempBuffer.clear();
				szTempBuffer.append(L" Magic:");
				szLoopBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_TECH_TEXT"),szTempBuffer.c_str());
				break;
			case 2:
				szTempBuffer.clear();
				szTempBuffer.append(L" Gold:");
				szLoopBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_YELLOW"),szTempBuffer.c_str());
				break;
			case 3:
				szTempBuffer.clear();
				szTempBuffer.append(L" Research:");
				szLoopBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_ALT_HIGHLIGHT_TEXT"),szTempBuffer.c_str());
				break;
			case 4:
				szTempBuffer.clear();
				szTempBuffer.append(L" Happiness:");
				szLoopBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_MAGENTA"),szTempBuffer.c_str());
				break;
			case 5:
				szTempBuffer.clear();
				szTempBuffer.append(L" Religion:");
				szLoopBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_YIELD_PRODUCTION"),szTempBuffer.c_str());
				break;
			case 6:
				szTempBuffer.clear();
				szTempBuffer.append(L" Misc:");
		...
		for (int iI = 0; iI < GC.getNumTechInfos(); iI++)
		{
			...
			if(iPass==0 && kTech.getFlavorValue(FLAVOR_MELEE)<=0
						&&	kTech.getFlavorValue(FLAVOR_RANGED)<=0
						&&	kTech.getFlavorValue(FLAVOR_RECON)<=0
						&&	kTech.getFlavorValue(FLAVOR_MILITARY)<=0
						&&	kTech.getFlavorValue(FLAVOR_MOUNTED)<=0)
				continue;
			if(iPass==1 && kTech.getFlavorValue(FLAVOR_MAGIC)<=0)
				continue;
			if(iPass==2 && kTech.getFlavorValue(FLAVOR_GOLD)<=0)
				continue;
			if(iPass==3 && kTech.getFlavorValue(FLAVOR_SCIENCE)<=0)
				continue;
			if(iPass==4 && kTech.getFlavorValue(FLAVOR_HAPPY)<=0)
				continue;
			if(iPass==5 && kTech.getFlavorValue(FLAVOR_RELIGION)<=0)
				continue;
			if(iPass==6 && (kTech.getFlavorValue(FLAVOR_MILITARY)>0
						||	kTech.getFlavorValue(FLAVOR_MELEE)>0
						||	kTech.getFlavorValue(FLAVOR_RANGED)>0
						||	kTech.getFlavorValue(FLAVOR_RECON)>0
						||	kTech.getFlavorValue(FLAVOR_MOUNTED)>0
						||	kTech.getFlavorValue(FLAVOR_MAGIC)>0
						||	kTech.getFlavorValue(FLAVOR_GOLD)>0
						||	kTech.getFlavorValue(FLAVOR_SCIENCE)>0
						||	kTech.getFlavorValue(FLAVOR_HAPPY)>0
						||	kTech.getFlavorValue(FLAVOR_RELIGION)>0))
				continue;
...
bool CvDLLButtonPopup::launchChooseSpellResearchPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	CvPlayer& player = GET_PLAYER(GC.getGameINLINE().getActivePlayer());
	...
	CvWString szLoopBuffer;
	CvWString szTempBuffer;
	for (int iPass=0;iPass<GC.getNumBonusInfos();iPass++)
	{
		bool bFirst=true;
		szLoopBuffer.clear();
		szLoopBuffer.append(L"----------------------------------------------\n");
		szLoopBuffer.append(L"             ");
		szLoopBuffer.append(GC.getBonusInfo((BonusTypes)iPass).getDescription());
		szLoopBuffer.append(L"\n");
		szLoopBuffer.append(L"----------------------------------------------\n");
		szLoopBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_MAGENTA"),szLoopBuffer.c_str());

		for (int iI = 0; iI < GC.getNumTechInfos(); iI++)
		{
			CvTechInfo& kTech=GC.getTechInfo((TechTypes)iI);
			if(kTech.isSpellResearch() && kTech.getPrereqBonus()==iPass)
			{
				if(player.canSpellResearchNext((TechTypes)iI))
				{
					if(bFirst)
					{
 						if(iPass>0)
...
bool CvDLLButtonPopup::launchChooseGuildPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	CvPlayer& player = GET_PLAYER(GC.getGameINLINE().getActivePlayer());
	...
	if (szHeader.empty())
	{
		szHeader = gDLL->getText("TXT_KEY_POPUP_CHOOSE_GUILD").c_str();
	}
	gDLL->getInterfaceIFace()->popupSetHeaderString(pPopup, szHeader, DLL_FONT_LEFT_JUSTIFY);
	...
	{
		CvTechInfo& kTech=GC.getTechInfo((TechTypes)iI);
		if(player.canGetGuild((TechTypes)iI))
		{
			CvWString szBuffer;
			szBuffer.Format(L"%s", GC.getTechInfo((TechTypes)iI).getDescription());
...
bool CvDLLButtonPopup::launchChooseReligionPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	CvPlayer& player = GET_PLAYER(GC.getGameINLINE().getActivePlayer());
	...
	if (szHeader.empty())
	{
		szHeader = gDLL->getText("TXT_KEY_POPUP_CHOOSE_RELIGION").c_str();
	}
	gDLL->getInterfaceIFace()->popupSetHeaderString(pPopup, szHeader, DLL_FONT_LEFT_JUSTIFY);
...
bool CvDLLButtonPopup::launchDungeonEventPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	PlayerTypes ePlayer = GC.getGameINLINE().getActivePlayer();
	...
	if (szHeader.empty())
	{
		switch(pDungeon->getPopulation()) {
			case DUNGEON_POPULATION_LOW:
				szPopulation.append(L"low");
				break;
			case DUNGEON_POPULATION_AVERAGE:
				szPopulation.append(L"average");
				break;
			case DUNGEON_POPULATION_HIGH:
				szPopulation.append(L"high");
				break;
			default:
				szPopulation.append(L"very high");
				break;
		}

		switch(pDungeon->getPower() / 3) {
			case DUNGEON_POWER_WEAK:
				szPower.append(L"weak");
				break;
			case DUNGEON_POWER_AVERAGE:
				szPower.append(L"average");
				break;
			case DUNGEON_POWER_STRONG:
				szPower.append(L"strong");
				break;
			case DUNGEON_POWER_DEADLY:
			default:
				szPower.append(L"deadly");
				break;
		}

		szHeader = gDLL->getText("TXT_KEY_POPUP_DUNGEON_EVENT").c_str();
		szHeader.append(gDLL->getText("TXT_KEY_POPUP_DUNGEON_EVENT_ATTITUDE", GC.getAttitudeInfo(pDungeon->getAttitude(ePlayer)).getDescription(), szPopulation.c_str(), szPower.c_str()));
		if(pDungeon->isWealthy()) {
			szHeader.append(gDLL->getText("TXT_KEY_POPUP_DUNGEON_EVENT_WEALTHY"));
		}
		if(pDungeon->getReligionType() != NO_RELIGION) {
			szHeader.append(gDLL->getText("TXT_KEY_POPUP_DUNGEON_EVENT_RELIGION", GC.getReligionInfo(pDungeon->getReligionType()).getDescription()));
		}
	}
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szHeader, DLL_FONT_LEFT_JUSTIFY);
	...
	for (int iI = 0; iI < GC.getNumDungeonEventInfos(); iI++)
	{
		CvDungeonEventInfo& kEvent = GC.getDungeonEventInfo((DungeonEventTypes)iI);
		if(pDungeon->isDealAvailable(player.getID(), (DungeonEventTypes)iI))
		{
			CvWString szBuffer;
			szBuffer.Format(L"%s ", kEvent.getDescription());

			CvString szButton = ARTFILEMGR.getInterfaceArtInfo("INTERFACE_EVENT_UNAVAILABLE_BULLET")->getPath();

			gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, szBuffer, szButton, iI, WIDGET_GENERAL, -1, -1, true, POPUP_LAYOUT_STRETCH, DLL_FONT_LEFT_JUSTIFY);
			iNumEvents++;
		}
	}
	...
	CvPlot* pPlot = GC.getMapINLINE().plot(pDungeon->getX(), pDungeon->getY());
	if (NULL != pPlot)
	{
...
//FfH: Added by Kael 11/04/2007
bool CvDLLButtonPopup::launchConfirmCastGlobalPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	int iAction = info.getData1();
	CvWString szBuffer;
	szBuffer = gDLL->getText("TXT_KEY_POPUP_CONFIRM_CAST_GLOBAL", GC.getActionInfo(iAction).getTextKeyWide());
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_YES").c_str(), NULL, 0);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_NO").c_str());
	gDLL->getInterfaceIFace()->popupLaunch(pPopup, false, POPUPSTATE_IMMEDIATE);
...
bool CvDLLButtonPopup::launchConfirmCastWarPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	int iAction = info.getData1();
	CvWString szBuffer;
	szBuffer = gDLL->getText("TXT_KEY_POPUP_CONFIRM_CAST_WAR", GC.getActionInfo(iAction).getTextKeyWide());
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_YES").c_str(), NULL, 0);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_NO").c_str());
	gDLL->getInterfaceIFace()->popupLaunch(pPopup, false, POPUPSTATE_IMMEDIATE);
...
bool CvDLLButtonPopup::launchConfirmSettlementPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	CvWString szBuffer;
	szBuffer = gDLL->getText("TXT_KEY_POPUP_SETTLEMENT_OR_CITY");
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_CITY").c_str(), NULL, 0);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_SETTLEMENT").c_str(), NULL, 1);
	gDLL->getInterfaceIFace()->popupLaunch(pPopup, false, POPUPSTATE_IMMEDIATE);
...
/**	ADDON(Screen) Sephi					                                                        **/
/*************************************************************************************************/
bool CvDLLButtonPopup::launchNewTraitPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	CvWString szBuffer;
	szBuffer = gDLL->getText("TXT_KEY_POPUP_NEW_TRAIT", GC.getTraitInfo((TraitTypes)info.getData1()).getDescription());
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_YES").c_str(), NULL, 0);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_NO").c_str(), NULL, 1);
	gDLL->getInterfaceIFace()->popupLaunch(pPopup, false, POPUPSTATE_IMMEDIATE);

	return (true);
}

bool CvDLLButtonPopup::launchSummonPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	if(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getCurrentMagicRitual()==info.getData2())
	{
		return (false);
	}

	CvWString szBuffer;
	szBuffer = gDLL->getText("TXT_KEY_POPUP_SUMMON",GC.getUnitInfo((UnitTypes)info.getData1()).getDescription(),GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getSummonCity()->getNameKey());
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_YES").c_str(), NULL, 0);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_NO").c_str(), NULL, 1);
	gDLL->getInterfaceIFace()->popupLaunch(pPopup, false, POPUPSTATE_IMMEDIATE);
...
bool CvDLLButtonPopup::launchTerraformPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	if(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getCurrentMagicRitual()==info.getData1())
	{
		return (false);
	}

	CvProjectInfo& kProject = GC.getProjectInfo((ProjectTypes)info.getData1());
	CvPlayer& kPlayer = GET_PLAYER(GC.getGameINLINE().getActivePlayer());
	CvWString szBuffer;
	szBuffer = gDLL->getText("TXT_KEY_POPUP_TERRAFORM", kProject.getDescription());
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	if(!GC.getProjectInfo((ProjectTypes)info.getData1()).isTargetedTerraform())
	{
		gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_YES").c_str(), NULL, 0);
	}
	if(kPlayer.getMana() >= kPlayer.getSpecificMagicRitualCost((ProjectTypes)info.getData1()) && GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getNumUnits()>0)
	{			
		gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_TERRAFORM_LET_ME_SELECT").c_str(), NULL, 1);
	}
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_NO").c_str(), NULL, 2);
	gDLL->getInterfaceIFace()->popupLaunch(pPopup, false, POPUPSTATE_IMMEDIATE);

	return (true);
}

bool CvDLLButtonPopup::launchGlobalenchantmentPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	if(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getCurrentMagicRitual()==info.getData1())
	{
		return (false);
	...
	CvWString szBuffer;
	szBuffer = gDLL->getText("TXT_KEY_POPUP_GLOBALENCHANTMENT", GC.getProjectInfo((ProjectTypes)info.getData1()).getDescription());
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_YES").c_str(), NULL, 0);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_NO").c_str(), NULL, 1);
	gDLL->getInterfaceIFace()->popupLaunch(pPopup, false, POPUPSTATE_IMMEDIATE);
...
bool CvDLLButtonPopup::launchRemoveGlobalenchantmentPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	CvWString szBuffer;
	szBuffer = gDLL->getText("TXT_KEY_POPUP_GLOBALENCHANTMENT_CANCEL", GC.getProjectInfo((ProjectTypes)info.getData1()).getDescription());
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_YES").c_str(), NULL, 0);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_NO").c_str(), NULL, 1);
	gDLL->getInterfaceIFace()->popupLaunch(pPopup, false, POPUPSTATE_IMMEDIATE);
...
bool CvDLLButtonPopup::launchPickEquipmentPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	if(GC.getGameINLINE().getActivePlayer()==NO_PLAYER || gDLL->getInterfaceIFace()->getHeadSelectedUnit()==NULL)
	{
		return false;
	}
	CvWString szBuffer;
	szBuffer = gDLL->getText("TXT_KEY_POPUP_PICK_EQUIPMENT");
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	CvPlayer& kPlayer=GET_PLAYER(GC.getGameINLINE().getActivePlayer());
	...
	for(int iJ=0;iJ<GC.getNumEquipmentCategoryInfos();iJ++)
	{
		...
		for(int iI=0;iI<GC.getNumPromotionInfos();iI++)
		{
			...
			{
				if(kPlayer.canEquipUnit(pUnit,(PromotionTypes)iI))
				{
					if(bFirst)
					{
						gDLL->getInterfaceIFace()->popupAddSeparator(pPopup);
						gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, GC.getEquipmentCategoryInfo((EquipmentCategoryTypes)iJ).getDescription());
						bFirst=false;
					}
...
//					gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, GC.getPromotionInfo((PromotionTypes)iI).getDescription(), GC.getPromotionInfo((PromotionTypes)iI).getButton(), iI,WIDGET_HELP_PROMOTION,iI);
					gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, GC.getPromotionInfo((PromotionTypes)iI).getDescription(), GC.getPromotionInfo((PromotionTypes)iI).getButton(), -1, WIDGET_HELP_EQUIPMENT,iI,MAX_INT,false);
				}
	...
	gDLL->getInterfaceIFace()->popupAddSeparator(pPopup);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_NOTHING"), NULL,-1,WIDGET_GENERAL);
...
bool CvDLLButtonPopup::launchChooseSacredKnowledgePopup(CvPopup* pPopup, CvPopupInfo &info)
{
	CvWString szBuffer;
	szBuffer = gDLL->getText("TXT_KEY_POPUP_PICK_SACRED_KNOWLEDGE");
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	CvPlayer& kPlayer=GET_PLAYER(GC.getGameINLINE().getActivePlayer());

	int iDiscover=0; // TEMPFIX

	bool bFirst=true;
	for(int iI=0;iI<GC.getNumTechInfos();iI++)
	{
		if(kPlayer.canBuyHolyKnowledge((TechTypes)iI))
		{
			if(bFirst)
			{
				gDLL->getInterfaceIFace()->popupAddSeparator(pPopup);
...
bool CvDLLButtonPopup::launchCityInfrastructureIncreasePopup(CvPopup* pPopup, CvPopupInfo &info)
{
	CvWString szBuffer;
	szBuffer = gDLL->getText("TXT_KEY_POPUP_INCREASE_INFRASTRUCTURE");
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_YES").c_str(), NULL, 0);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_NO").c_str(), NULL, 1);
	gDLL->getInterfaceIFace()->popupLaunch(pPopup, false, POPUPSTATE_IMMEDIATE);
...
bool CvDLLButtonPopup::launchCulturalAchievementPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	if(!GET_PLAYER(GC.getGameINLINE().getActivePlayer()).canGetCulturalAchievement((TechTypes)info.getData1()))
	{
		return (false);
	}

	CvWString szBuffer;
	szBuffer = gDLL->getText("TXT_KEY_POPUP_CULTURAL_ACHIEVEMENT",GC.getTechInfo((TechTypes)info.getData1()).getDescription());
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_YES").c_str(), NULL, 0);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_NO").c_str(), NULL, 1);
	gDLL->getInterfaceIFace()->popupLaunch(pPopup, false, POPUPSTATE_IMMEDIATE);
...
//FfH: Modified by Kael 09/25/2007
//	gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "resource/temp/civ4_title_small.dds", 192, 48);
	if (GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getAlignment() == ALIGNMENT_GOOD)
	{
        gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/civ4_title_small_good.dds", 192, 96);
	}
	else if (GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getAlignment() == ALIGNMENT_EVIL)
	{
        gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/civ4_title_small_evil.dds", 192, 96);
	}
	else
	{
        gDLL->getInterfaceIFace()->popupAddDDS(pPopup, "Art/civ4_title_small.dds", 192, 96);
	}
//FfH: End Modify
