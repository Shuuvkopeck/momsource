...
/**
*   CAPITULATION_EDIT - Till - 20.04.2010
**/
int CvArea::getCitiesPerTeam(PlayerTypes eIndex) const
{
    FAssertMsg(eIndex >= 0, "eIndex is expected to be >= 0");
	FAssertMsg(eIndex < MAX_PLAYERS, "eIndex is expected to be < MAX_PLAYERS");
    int nCities = 0;
    for (int iI = 0; iI < MAX_PLAYERS; iI++)
    {
        ...
        {
            //Is this player in our team?
            if ( kPlayer.getTeam() == GET_PLAYER(eIndex).getTeam())
            {
                  nCities+=getCitiesPerPlayer((PlayerTypes)iI);
            }
        }
    }
	return nCities;
...
int CvArea::getPlane() const { return m_iPlane; }
void CvArea::setPlane(int Plane) { m_iPlane = Plane; }

int CvArea::getNumPortals() const { return m_iNumPortals; }
void CvArea::changeNumPortals(int iChange) { m_iNumPortals += iChange; }

bool CvArea::canAreaBeReachedThroughPortal(int iArea)
{
	return isConnectedByPortalToArea(iArea, 0);
}

bool CvArea::isConnectedByPortalToArea(int iArea, int iRecursionLevel)
{
	if(iArea == getID())
		return true;

	if(iRecursionLevel < 5) {
		for(int i = 0; i < GC.getMapINLINE().numPlots(); ++i) {
			CvPlot* pLoopPlot = GC.getMapINLINE().plotByIndex(i);
			if(pLoopPlot->getArea() == getID()) {
				if(pLoopPlot->isPortal()) {
					return isConnectedByPortalToArea(pLoopPlot->getPortalExit()->getArea(), iRecursionLevel + 1);
				}
			}
		}
	}
...
/*************************************************************************************************/
int CvArea::AI_getPotentialCounterUnits(PlayerTypes ePlayer) const
{
	...
	{
		if(pAIGroup->getGroupType()==AIGROUP_COUNTER || pAIGroup->getGroupType()==AIGROUP_DESTROY_LAIR ||pAIGroup->getGroupType()==AIGROUP_RESERVE)
		{
			if(pAIGroup->getMissionArea()==getID())
			{
				iUnits+=pAIGroup->getNumUnits();
			}
		}
	}
...
CvAIGroup* CvArea::getReserveGroup(PlayerTypes ePlayer) const
{
	CvPlayerAI& kPlayer = GET_PLAYER(ePlayer);
	bool bWater=isWater();
	int iLoop;
	...
	for(CvAIGroup* pAIGroup = kPlayer.firstAIGroup(&iLoop); pAIGroup != NULL; pAIGroup = kPlayer.nextAIGroup(&iLoop))
	{
		if((bWater && pAIGroup->getGroupType()==AIGROUP_NAVAL_RESERVE)
			||(!bWater && pAIGroup->getGroupType()==AIGROUP_RESERVE))
		{
			if(pAIGroup->getMissionArea()==getID())
			{
				return pAIGroup;
