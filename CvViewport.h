...
public:
	...
	void	resizeForMap(void);
	void	bringIntoView(int iX, int iY, CvUnit* pSelectionUnit = NULL, bool bLookAt = true, bool bForceCenter = false, bool bDisplayCityScreen = false);
	void	centerOnSelection(void);
	void	panLeft(void);
	void	panRight(void);
	void	panUp(void);
	void	panDown(void);
	...
	inline bool isSelectionInhibitted(void) const
	{
		return m_inhibitSelection;
	}

	inline ViewportGraphicalSpoofingState getSpoofHiddenGraphics(void) const
	{
