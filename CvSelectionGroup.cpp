...
/**	Alertness								11/30/08	Written: Pep		Imported: Xienwolf	**/
/**	ADDON (Alertness) merged Sephi	    														**/
/**					Ensures that best visibility range in the tile is used						**/
/*************************************************************************************************/
	int iIndex = -1;
/*************************************************************************************************/
/**	Alertness								END													**/
...
/*************************************************************************************************/
		case MISSION_SPELL_OFFENSIVE:
			if(pLoopUnit->getMissionSpell()!=NO_SPELL && pLoopUnit->canCastAt((SpellTypes)pLoopUnit->getMissionSpell(),GC.getMapINLINE().plot(iData1,iData2)))
			{
				return true;
			}
			break;
		case MISSION_TERRAFORM:
			if((ProjectTypes)GET_PLAYER(getOwnerINLINE()).getTerraformPlan() != NO_PROJECT)
			{
				if(GET_PLAYER(getOwnerINLINE()).getMana() >= GET_PLAYER(getOwnerINLINE()).getSpecificMagicRitualCost((ProjectTypes)GET_PLAYER(getOwnerINLINE()).getTerraformPlan()))
				{
					if(GC.getMapINLINE().plot(iData1,iData2)->doTerraformRitual((ProjectTypes)GET_PLAYER(getOwnerINLINE()).getTerraformPlan(),getOwnerINLINE(), iValue,true))
					{
						return true;
					}
				}
...
/*************************************************************************************************/
			...
			if(!isHuman())
			{
				if(getHeadUnit()!=NULL && getHeadUnit()->getAIGroup()!=NULL
					&& getHeadUnit()->getAIGroup()->getGroupType()==AIGROUP_CITY_INVASION)
				{
					TeamTypes eEnemyTeam=GC.getMapINLINE().plotINLINE(headMissionQueueNode()->m_data.iData1, headMissionQueueNode()->m_data.iData2)->getTeam();
					if(eEnemyTeam==getHeadUnit()->getAIGroup()->getMissionTarget()->getTeam())
					{
...
/*************************************************************************************************/
		...
		while (pUnitNode != NULL)
		{
			...
			if (pLoopUnit->canMove())
			{
				...
				default:
					//FAssert(false);	Not all Missions use this switch Sephi
					break;
				}
...
/**	ADDON(INTERFACEMODE_SPELL_OFFENSIVE) Sephi					                                **/
/*************************************************************************************************/
		case INTERFACEMODE_SPELL_OFFENSIVE:
			{
				for(int iI=0;iI<GC.getNumSpellInfos();iI++)
				{
					if(GC.getSpellInfo((SpellTypes)iI).isTargetedHostile() && pLoopUnit->canCast(iI,false))
					{
						if(!GC.getSpellInfo((SpellTypes)iI).isSpellObsoleted(pLoopUnit,iI))
						{
							return true;
						}
					}
...
/*************************************************************************************************/
		case INTERFACEMODE_SPELL_OFFENSIVE:
			if (pLoopUnit != NULL)
			{
				for(int iI=0;iI<GC.getNumSpellInfos();iI++)
				{
					if(GC.getSpellInfo((SpellTypes)iI).isTargetedHostile() && pLoopUnit->canCastAt((SpellTypes)iI,pPlot))
					{
						if(!GC.getSpellInfo((SpellTypes)iI).isSpellObsoleted(pLoopUnit,iI))
						{
							return true;
		...
		case INTERFACEMODE_TERRAFORM:
			...
			if (pLoopUnit != NULL)
			{
				if(!pPlot->isRevealed(pLoopUnit->getTeam(),false))
				{
					return false;
				}
				if(GET_PLAYER(pLoopUnit->getOwnerINLINE()).getTerraformPlan()!=NO_PROJECT)
				{
					if(GC.getProjectInfo((ProjectTypes)GET_PLAYER(pLoopUnit->getOwnerINLINE()).getTerraformPlan()).isHostileTerraform())
					{						
						if(pPlot->getTeam()==pLoopUnit->getTeam())
						{
							return false;
						}
						if(!GET_TEAM(pPlot->getTeam()).isAtWar(pLoopUnit->getTeam()))
						{
							return false;
						}
					}
					...
					if(pPlot->doTerraformRitual((ProjectTypes)GET_PLAYER(pLoopUnit->getOwnerINLINE()).getTerraformPlan(),pLoopUnit->getOwnerINLINE(), iValue,true))
					{
						return true;
					}
				}
...
bool CvSelectionGroup::groupAttack(int iX, int iY, int iFlags, bool& bFailedAlreadyFighting, bool bPortal)
{
...
/*************************************************************************************************/
    ...
    if (!GET_PLAYER(getOwnerINLINE()).isHuman() && !bPortal)
    {
        ...
        if (pDestPlot->isCity() && GET_TEAM(getTeam()).isAtWar(pDestPlot->getTeam()))
        {
            ...
            int iOurStrength=AI_GroupPower(pDestPlot,false);
            int iTheirStrength=GET_PLAYER(getOwnerINLINE()).AI_getEnemyPlotStrength(pDestPlot,0,true,false);
            if (iBombarbTurns>0 && iBombarbTurns<8 && (iOurStrength<iTheirStrength*7))
            {
                CLLNode<IDInfo>* pEntityNode = headUnitNode();
                CvUnit* pLoopUnit;
                while (pEntityNode != NULL)
...
/**	ADDON (Ranged Combat) Sephi								                     				**/
        if((this->getX()-iX)*(this->getX()-iX)+(this->getY()-iY)*(this->getY()-iY)<4)
        {
			if (getNumUnits()>1)
			{
				//never do Ranged attacks with all units in the Group
				int iMaxRangedStrikes=getNumUnits()-3;

				if (getHeadUnit()!=NULL)
				{
					if (getNumUnits()>3*pDestPlot->getNumVisibleEnemyDefenders(getHeadUnit()))
					{
						// if we greatly outnumber the enemy, we don't have to do too many RangedStrikes
						iMaxRangedStrikes=std::min(iMaxRangedStrikes,getNumUnits()/2);
					}
				}

				iMaxRangedStrikes=std::max(1,iMaxRangedStrikes);
				...
				while (pEntityNode != NULL)
				{
					...
					if (pLoopUnit!=NULL)
					{
						if (pLoopUnit->canRangeStrike())
						{
							if(pLoopUnit->canRangeStrikeAt(plot(),iX,iY))
							{
								pLoopUnit->rangeStrike(iX,iY);
								iMaxRangedStrikes--;
							}
						}
					}
					if (iMaxRangedStrikes<=0)
					{
						break;
					}
				}
...
/*************************************************************************************************/
	...
	if (getNumUnits() > 0)
	{
		if ((getDomainType() == DOMAIN_AIR) || (stepDistance(getX(), getY(), pDestPlot->getX_INLINE(), pDestPlot->getY_INLINE()) == 1) || bPortal)
		{
			if (bPortal || (iFlags & MOVE_DIRECT_ATTACK) || (getDomainType() == DOMAIN_AIR) || (iFlags & MOVE_THROUGH_ENEMY) || (generatePath(plot(), pDestPlot, iFlags) && (getPathFirstPlot() == pDestPlot)))
			{
				int iAttackOdds;
...
/*************************************************************************************************/
/**	Better AI (Spell Burning Blood) Sephi  														**/
/*************************************************************************************************/
                                if (!isHuman())
                                {
                                    int iSpell=GC.getInfoTypeForString("SPELL_BURNING_BLOOD");
                                    if (iSpell!=NO_SPELL && pBestAttackUnit->canCast(iSpell,false))
                                    {
                                        pBestAttackUnit->cast(iSpell);
                                    }
                                }
/*************************************************************************************************/
/**	Better AI								END													**/
...
/*************************************************************************************************/
						...
						if (getNumUnits() > 1)
						{
							...
							{
/*************************************************************************************************/
/**	BETTER AI (Groupattack) Sephi                                             					**/
/**	Don't care about summons																	**/
/**						                                            							**/
/*************************************************************************************************/
/** Orig
                                bFailedAlreadyFighting = true;
**/
                                if(pBestAttackUnit->getDuration()==0)
                                {
                                    bFailedAlreadyFighting = true;
                                }
...
/*************************************************************************************************/
	bool bSentryAlert;
	if (isHuman() && sentryAlert() && headMissionQueueNode() && (headMissionQueueNode()->m_data.eMissionType == MISSION_MOVE_TO))
	{
		bSentryAlert = true;
	}
	else
...
bool CvSelectionGroup::groupPathTo(int iX, int iY, int iFlags)
{
	...
	//add Sephi 
	// AI to avoid Barbarians
	if(isAutomated() || !isHuman())
	{
		if(!plot()->isWater() && !canDefend())
		{
			if (GET_PLAYER(getOwnerINLINE()).AI_isPlotThreatened(pDestPlot, 3))
			{
				if(pDestPlot->getNumDefenders(getOwnerINLINE())==0)
				{
					return false;
				}
			}
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Start("Pathfinding");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
	GC.getTimeMeasure().Stop("Pathfinding");
/*************************************************************************************************/
/**	TIME MEASURE							END													**/
/*************************************************************************************************/
...
bool CvSelectionGroup::isUpdateNecessary()
{
	if(getHeadUnit()!=NULL && getHeadUnit()->getAIGroup()==NULL)
	{
		if(readyToMove(true))
...
int CvSelectionGroup::getNumUnitsValidForEquipment(PromotionTypes eEquipment, bool bYieldCost)
{
	...
	while (pUnitNode != NULL)
	{
		...
		if (pLoopUnit != NULL)
		{
			if(GET_PLAYER(getOwnerINLINE()).canEquipUnit(pLoopUnit, eEquipment))
			{
				iCounter++;
			}
		}
