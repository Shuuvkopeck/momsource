...
int CvCityAI::AI_specialistSpecializationValue(SpecialistTypes eSpecialist)
{
	...
	if(eSpecialist == GC.getSPECIALIST_ENGINEER()) {
		if(getYieldIncome(YIELD_METAL) > 20 
		|| getYieldIncome(YIELD_LEATHER) > 20
		|| getYieldIncome(YIELD_STONE) > 20
		|| getYieldIncome(YIELD_LUMBER) > 20) {
...
int CvCityAI::AI_cityValue_Food(int iFoodAmount)
{
	...
	int iFoodPerPopulationNeeded=2;  //adjust for Sacrifice the Weak/Slavery
