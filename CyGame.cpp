...
void CyGame::addPlayerAdvanced(int eNewPlayer, int iNewTeam, int eLeader, int eCiv)
{
	if (m_pGame)
	{
		m_pGame->addPlayerAdvanced((PlayerTypes)eNewPlayer, iNewTeam, (LeaderHeadTypes)eLeader, (CivilizationTypes)eCiv);
	}
}
//FfH: End Add
...
//FfH: Added by Kael 08/10/2007
void CyGame::changeCrime(int iChange)
{
	if (m_pGame)
	{
		m_pGame->changeCrime(iChange);
	}
}

int CyGame::getCrime() const
{
	return (NULL != m_pGame ? m_pGame->getCrime() : -1);
}

void CyGame::changeCutLosersCounter(int iChange)
{
	if (m_pGame)
	{
		m_pGame->changeCutLosersCounter(iChange);
	}
}

int CyGame::getCutLosersCounter() const
{
	return (NULL != m_pGame ? m_pGame->getCutLosersCounter() : -1);
}
...
void CyGame::changeGlobalCounter(int iChange)
{
	if (m_pGame)
	{
		m_pGame->changeGlobalCounter(iChange);
	}
}

int CyGame::getGlobalCounter() const
{
	return (NULL != m_pGame ? m_pGame->getGlobalCounter() : -1);
}

void CyGame::changeGlobalCounterLimit(int iChange)
{
	if (m_pGame)
	{
		m_pGame->changeGlobalCounterLimit(iChange);
	}
}

int CyGame::getGlobalCounterLimit() const
{
	return (NULL != m_pGame ? m_pGame->getGlobalCounterLimit() : -1);
}

int CyGame::getMaxGlobalCounter() const
{
	return (NULL != m_pGame ? m_pGame->getMaxGlobalCounter() : -1);
}

void CyGame::changeScenarioCounter(int iChange)
{
	if (m_pGame)
	{
		m_pGame->changeScenarioCounter(iChange);
	}
}

int CyGame::getScenarioCounter() const
{
	return (NULL != m_pGame ? m_pGame->getScenarioCounter() : -1);
}

bool CyGame::getWBMapScript() const
{
	return GC.getInitCore().getWBMapScript();
}

void CyGame::resetGame() const
{
	GC.getInitCore().resetGame();
}

void CyGame::resetPlayers() const
{
	GC.getInitCore().resetPlayers();
}

void CyGame::incrementProjectCreatedCount(int /*ProjectTypes*/ eIndex)
{
	if (m_pGame)
	{
		m_pGame->incrementProjectCreatedCount((ProjectTypes)eIndex);
	}
}

void CyGame::incrementUnitClassCreatedCount(int /*UnitClassTypes*/ eIndex)
{
	if (m_pGame)
	{
		m_pGame->incrementUnitClassCreatedCount((UnitClassTypes)eIndex);
	}
}

void CyGame::reassignPlayerAdvanced(int /*PlayerTypes*/ eOldID, int /*PlayerTypes*/ eNewID, int iTimer)
{
	GC.getInitCore().reassignPlayerAdvanced((PlayerTypes)eOldID, (PlayerTypes)eNewID, iTimer);
}

void CyGame::setReligionSlotTaken(int /*ReligionTypes*/ iReligion, bool bNewValue)
{
	if (m_pGame)
		m_pGame->setReligionSlotTaken((ReligionTypes)iReligion, bNewValue);
}

int CyGame::getTrophyValue(const TCHAR* szName) const
{
	return (NULL != m_pGame ? m_pGame->getTrophyValue(szName) : 0);
}

bool CyGame::isHasTrophy(const TCHAR* szName) const
{
	return (NULL != m_pGame ? m_pGame->isHasTrophy(szName) : false);
}

void CyGame::setTrophyValue(const TCHAR* szName, int iNewValue)
{
	if (m_pGame)
		m_pGame->setTrophyValue(szName, iNewValue);
}

void CyGame::changeTrophyValue(const TCHAR* szName, int iChange)
{
	if (m_pGame)
		m_pGame->changeTrophyValue(szName, iChange);
}

void CyGame::testVictory()
{
   	if (m_pGame)
        m_pGame->testVictory();
}
...
bool CyGame::ctrlKey()
{
	return (NULL != m_pGame ? m_pGame->ctrlKey() : false);
}

bool CyGame::altKey()
{
	return (NULL != m_pGame ? m_pGame->altKey() : false);
}
//<<<<	BUGFfH: End Add
...
/*************************************************************************************************/
int CyGame::getMercenariesSupportTarget() const
{
    return (NULL != m_pGame ? m_pGame->getMercenariesSupportTarget() : -1);
}

void CyGame::setMercenariesSupportTarget(int iNewTarget)
{
	if (m_pGame)
		m_pGame->setMercenariesSupportTarget(iNewTarget);
}

int CyGame::getMercenariesWarTarget() const
{
    return (NULL != m_pGame ? m_pGame->getMercenariesWarTarget() : -1);
}

void CyGame::setMercenariesWarTarget(int iNewTarget)
{
	if (m_pGame)
		m_pGame->setMercenariesWarTarget(iNewTarget);
}

int CyGame::getPiecesOfBarnaxusCounter() const
{
    return (NULL != m_pGame ? m_pGame->getPiecesOfBarnaxusCounter() : -1);
}

int CyGame::getDevilGatesCreated() const
{
    return (NULL != m_pGame ? m_pGame->getDevilGatesCreated() : -1);
}

void CyGame::changeDevilGatesCreated(int iChange)
{
	if (m_pGame)
		m_pGame->changeDevilGatesCreated(iChange);
}

int CyGame::getImprovementMemory(int iImprovementType) const 
{
	return (NULL != m_pGame ? m_pGame->getImprovementMemory(iImprovementType) : false);
}

void CyGame::setImprovementMemory(int iImprovementType, int iNewValue)
{ 
	if (m_pGame)
		m_pGame->setImprovementMemory(iImprovementType, iNewValue);
}
