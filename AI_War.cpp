	...
	if(getLeaderID() != NO_PLAYER)
	{
		...
		for(int iI = 0; iI < GC.getMAX_CIV_TEAMS(); iI++)
		{
			if(iI != getID() && GET_TEAM((TeamTypes)iI).isAlive())
			{
				...
				/** DEBUG **/
				if(GC.getGameINLINE().getElapsedGameTurns() % 1 == 0)
				{
					TCHAR szOut[1024];
//					if(getLeaderID() == 0 && bFirst)
					if(bFirst)
					{
					...
					sprintf(szOut, "Leader %S vs Leader %S: WarValue -- %d\n",GET_PLAYER(getLeaderID()).getName(),GET_PLAYER(GET_TEAM((TeamTypes)iI).getLeaderID()).getName(),AI_getWarValue((TeamTypes)iI));
					gDLL->logMsg("AIWarValues.log",szOut, false, false);
				}
				/** DEBUG **/
		...
		if(AI_getWarPlanTarget() != NO_TEAM)
		{
			if(AI_getWarValue(AI_getWarPlanTarget())<-500 || isAtWar(AI_getWarPlanTarget()))
			{
				...
				sprintf(szOut, "Leader %S vs Leader %S: cancelled Warplan\n",GET_PLAYER(getLeaderID()).getName(),GET_PLAYER(GET_TEAM(AI_getWarPlanTarget()).getLeaderID()).getName());
				gDLL->logMsg("AIWarplans.log",szOut, false, false);
				/** DEBUG **/
		...
		if(AI_getWarPlanTarget()==NO_TEAM)
		{
			...
			if(eBestWarTarget!=NO_TEAM)
			{
				...
				sprintf(szOut, "Turn: %d\n",GC.getGameINLINE().getElapsedGameTurns());
				gDLL->logMsg("AIWarplans.log",szOut, false, false);
				sprintf(szOut, "Leader %S vs Leader %S: Started Warplan\n",GET_PLAYER(getLeaderID()).getName(),GET_PLAYER(GET_TEAM(eBestWarTarget).getLeaderID()).getName());
				gDLL->logMsg("AIWarplans.log",szOut, false, false);
				/** DEBUG **/
			}
		}
