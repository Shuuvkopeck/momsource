...
bool CvMapGenerator::canPlaceGoodyAt(ImprovementTypes eImprovement, int iX, int iY)
{
	...
	if(pPlot->getPlane() != NO_PLANE && !GC.getPlaneInfo(pPlot->getPlane()).isGoodies())
		return false;
...
void CvMapGenerator::addGameElements()
{
	...
	//test Sephi
	GC.getGameINLINE().assignStartingPlots();
...
/*************************************************************************************************/
/**	ADDON (new Map Elements) Sephi                               	                            **/
/*************************************************************************************************/

    if(GC.getDefineINT("PLACE_DEEP_OCEANS_ON_MAP")==1)
    {
		addDeepOcean();
	}

	addUniqueManaLocations();
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
}
	...
	for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if (!(pLoopPlot->isWater()))
		{
			if (!(pLoopPlot->isCoastalLand()))
			{
				...
				// if (!(pLoopPlot->isRiver())) Sephi modified Planes
				{
...
/**
			if(pLoopPlot->canHaveRiver())
			{
				if ((pLoopPlot->isHills() || pLoopPlot->isPeak())
					&& (pLoopPlot->area()->getNumRiverEdges() < ((pLoopPlot->area()->getNumTiles() / GC.getDefineINT("PLOTS_PER_RIVER_EDGE")) + 1)))
				{
...
**/
			if (!(pLoopPlot->isWater()) && 
				(pLoopPlot->getPlane() != NO_PLANE && !GC.getPlaneInfo(pLoopPlot->getPlane()).isNoRiver()))
			{
...
// added Sephi - Planes
//	GC.getMapINLINE().calculatePlanes();
// end added Sephi - Planes
	...
	long result = 0;
	gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "getPlaneCreationMethod", NULL, &result);
	bool bOverride = !gDLL->getPythonIFace()->pythonUsingDefaultImpl();

	if(!bOverride || result != 3) {
		convertTerrainToPlane();
	}

//	if(bOverride && result == 2) {
		GC.getMapINLINE().placePortals();
//	}
}
...
void CvMapGenerator::generatePlotTypes()
{
	...
	if (gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "generatePlotTypes", NULL, &plotTypesOut) && !gDLL->getPythonIFace()->pythonUsingDefaultImpl())
	{
		...
		for (int iI = 0; iI < iNumPlots; iI++)
		{
			...
			//Sephi - added override if no Ocean on this Plane
			PlaneTypes ePlane=(PlaneTypes)GC.getMap().plotByIndex(iI)->getPlane();
			if(ePlane!=NO_PLANE)
			{
				if(GC.getPlaneInfo(ePlane).isReplaceWaterWithPeaks())
				{
					if(paiPlotTypes[iI] == PLOT_OCEAN)
					{
						if(GC.getGame().getSorenRandNum(5, "generatePlotTypes") == 0) {
							paiPlotTypes[iI] = PLOT_PEAK;
						}
						else {
...
void CvMapGenerator::convertTerrainToPlane()
{
	CvPlot* pLoopPlot;
	PlaneTypes ePlane;
	TerrainClassTypes eTerrainClass;
	TerrainTypes eNewTerrain;
	int iNumPlots = GC.getMapINLINE().numPlotsINLINE();

	for (int iI = 0; iI < iNumPlots; iI++)
	{
		pLoopPlot = GC.getMapINLINE().plotByIndexINLINE(iI);
		ePlane = (PlaneTypes)pLoopPlot->getPlane();
		...
		eTerrainClass = (TerrainClassTypes)GC.getTerrainInfo(pLoopPlot->getTerrainType()).getTerrainClassType();
		if(eTerrainClass != NO_TERRAINCLASS)
		{
			eNewTerrain = (TerrainTypes)GC.getPlaneInfo(ePlane).getNativeTerrainType(eTerrainClass);
			if(eNewTerrain != pLoopPlot->getTerrainType()) {
				gDLL->callUpdater();
				pLoopPlot->setTerrainType(eNewTerrain, true, true);
			}
		}
	}
}
...
int CvMapGenerator::getRiverValueAtPlot(CvPlot* pPlot)
{
	...
	//added Planes - Sephi
	if(pPlot->getPlane() != NO_PLANE && GC.getPlaneInfo(pPlot->getPlane()).isNoRiver()) {
		return MAX_INT;
	}
/**
	for (iI = 0; iI < NUM_DIRECTION_TYPES; ++iI)
	{
		...
		if (pAdjacentPlot != NULL)
		{
			if(pAdjacentPlot->getPlane() != pPlot->getPlane())
			{
				return MAX_INT;
			}
		}
...
//FfH: Added by Kael 10/04/2008
void CvMapGenerator::addImprovements()
{

	if (gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "addGoodies", NULL))
	{
		if (!gDLL->getPythonIFace()->pythonUsingDefaultImpl())
		{
			return;
        ...
        if (pPlot->getBonusType(NO_TEAM) == NO_BONUS)
        {
            if (pPlot->getImprovementType() == NO_IMPROVEMENT)
            {
                ...
                if (bValid)
                {
                    ...
                    {
                        int iUnit = GC.getImprovementInfo((ImprovementTypes)iJ).getSpawnUnitType();
						if (iUnit == NO_UNIT && pPlot->canHaveImprovement((ImprovementTypes)iJ, NO_TEAM))
						{
					...
					if(pPlot->getImprovementType() == NO_IMPROVEMENT) {
						if(pPlot->isValidLairLocation()) {

							if(GC.getGameINLINE().getMapRandNum(100, "Spawn Lair") < 33) {
								pPlot->CreateLair();
							}
						}
					}
    ...
    for (int iI = 0; iI < GC.getNumImprovementInfos(); iI++)
    {
        if (GC.getImprovementInfo((ImprovementTypes)iI).isUnique())
        {
            ...
            for (int iJ = 0; iJ < GC.getMapINLINE().numPlotsINLINE(); iJ++)
            {
                ...
                if (pPlot->getFeatureType()!=NO_FEATURE || pPlot->getBonusType(NO_TEAM)!=NO_BONUS)
                {
                    bValid = false;
                }

				if(pPlot->getWilderness()<GC.getImprovementInfo((ImprovementTypes)iI).getMinWilderness())
				{
					bValid = false;
				}
				if(pPlot->getWilderness()>GC.getImprovementInfo((ImprovementTypes)iI).getMaxWilderness())
				{
					bValid = false;
				}

				if(GC.getMap().isPlanes()) {
					if(pPlot->getPlane() != GC.getImprovementInfo((ImprovementTypes)iI).getNativePlane())
					{
						bValid = false;
					}
				}
				...
				for (int iX=-iSearchRange;iX<=iSearchRange;iX++)
				{
					...
					{
						CvPlot* pAdjacentPlot=plotXY(pPlot->getX_INLINE(),pPlot->getY_INLINE(),iX,iY);
						if(pAdjacentPlot!=NULL)
						{
							if(pAdjacentPlot->getImprovementType()!=NO_IMPROVEMENT && GC.getImprovementInfo((ImprovementTypes)pAdjacentPlot->getImprovementType()).isUnique())
							{
								bValid=false;
							}
						}
...
void CvMapGenerator::addDeepOcean()
{
    int iDeepOcean = GC.getInfoTypeForString("TERRAIN_OCEAN_DEEP");
    int iOcean = GC.getInfoTypeForString("TERRAIN_OCEAN");

    if(iDeepOcean != NO_TERRAIN)
    {
        for (int iI=0;iI<GC.getMapINLINE().numPlotsINLINE();iI++)
        {
            ...
            if (pPlot->isWater())
            {
                ...
                for (int iiY=iY-isearch; iiY<=iY+isearch; iiY++)
                {
                    for (int iiX=iX-isearch; iiX<=iX+isearch; iiX++)
                    {
                        CvPlot* pPlot2 = GC.getMapINLINE().plot(iiX,iiY);
                        ...
                        if (pPlot2->isWater())
                        {
                            if (pPlot2->getTerrainType()==iDeepOcean || pPlot2->getTerrainType()==iOcean)
                            {
                                iNumOceanPlots++;
                            }
                        }
                ...
                for (int iiY=iY-1; iiY<=iY+1; iiY++)
                {
                    for (int iiX=iX-1; iiX<=iX+1; iiX++)
                    {
                        ...
                        if (pPlot2->isWater() && (pPlot2 != pPlot))
                        {
                            if (!(pPlot2->getTerrainType()==iDeepOcean || pPlot2->getTerrainType()==iOcean))
                            {
                                bValid=false;
                ...
                if(bValid)
                {
                    if (iNumOceanPlots>75)
                    {
                        pPlot->setTerrainType((TerrainTypes)iDeepOcean,false,false);
                    }
                    else if (GC.getGameINLINE().getMapRandNum(75-iNumOceanPlots, "DeepOcean")==0)
                    {
                        pPlot->setTerrainType((TerrainTypes)iDeepOcean,false,false);
                    }
                }
...
void CvMapGenerator::addUniqueManaLocation(ImprovementTypes eImprovement)
{
	...
	{
		CvPlot* pPlot = GC.getMapINLINE().plotByIndexINLINE(iI);
		if(!pPlot->isWater() && !pPlot->isPeak() && !pPlot->isStartingPlot())
		{
			if(pPlot->getBonusType()==NO_BONUS && pPlot->getImprovementType()==NO_IMPROVEMENT)
			{
				if(!GC.getMap().isPlanes() || (pPlot->getPlane() == GC.getImprovementInfo(eImprovement).getNativePlane()))
				{
					if(GC.getImprovementInfo(eImprovement).getTerrainMakesValid(pPlot->getTerrainType()))
					{
						bool bValid = true;

						for(int x = -iSearchRange; x < iSearchRange; ++x) {
							for(int y = -iSearchRange; y < iSearchRange; ++y) {
								CvPlot* pLoopPlot = plotXY(pPlot->getX(), pPlot->getY(), x, y);
								if(pLoopPlot != NULL) {
						...
						for (int iJ = 0; iJ < NUM_DIRECTION_TYPES; iJ++)
						{
							CvPlot* pAdjacentPlot = plotDirection(pPlot->getX_INLINE(), pPlot->getY_INLINE(), ((DirectionTypes)iJ));
							if(pAdjacentPlot!=NULL)
							{
								if(pAdjacentPlot->getImprovementType() != NO_IMPROVEMENT)
								{
						...
						if(bValid)
						{
							if(GC.getGameINLINE().getMapRandNum(100, "addUniqueManaLocations") < 50) {
...
void CvMapGenerator::addUniqueManaLocations()
{
	addUniqueManaLocation((ImprovementTypes)GC.getIMPROVEMENT_MANA_SHRINE());
	addUniqueManaLocation((ImprovementTypes)GC.getInfoTypeForString("IMPROVEMENT_LOST_TEMPLE"));
	addUniqueManaLocation((ImprovementTypes)GC.getInfoTypeForString("IMPROVEMENT_WITCH_HUT"));
}
