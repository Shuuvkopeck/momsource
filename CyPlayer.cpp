...
int CyPlayer::getResistHostileTerraforming()
{
	return m_pPlayer ? m_pPlayer->getResistHostileTerraforming() : -1;
}
...
{
	if (m_pPlayer)
		m_pPlayer->changeSanctuaryTimer(iChange);
}

int CyPlayer::getAlignment() const
{
	return m_pPlayer ? (int) m_pPlayer->getAlignment() : NO_ALIGNMENT;
}
...
{
	if (m_pPlayer)
		m_pPlayer->changeDisableProduction(iChange);
}

int CyPlayer::getDisableProduction() const
{
	return m_pPlayer ? m_pPlayer->getDisableProduction() : -1;
}

void CyPlayer::changeDisableResearch(int iChange)
{
	if (m_pPlayer)
...
{
	if (m_pPlayer)
		m_pPlayer->changeDisableSpellcasting(iChange);
}

int CyPlayer::getDisableSpellcasting() const
{
	return m_pPlayer ? m_pPlayer->getDisableSpellcasting() : -1;
}

int CyPlayer::getMaxCities() const
{
	return m_pPlayer ? m_pPlayer->getMaxCities() : -1;
}

void CyPlayer::changeNoDiplomacyWithEnemies(int iChange)
{
	if (m_pPlayer)
		m_pPlayer->changeNoDiplomacyWithEnemies(iChange);
}

int CyPlayer::getNumBuilding(int iBuilding) const
{
	return m_pPlayer ? m_pPlayer->getNumBuilding(iBuilding) : -1;
}

int CyPlayer::getNumSettlements() const
{
	return m_pPlayer ? m_pPlayer->getNumSettlements() : -1;
}

int CyPlayer::getPlayersKilled() const
{
...
void CyPlayer::setGreatPeopleCreated(int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setGreatPeopleCreated(iNewValue);
}

void CyPlayer::setGreatPeopleThresholdModifier(int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setGreatPeopleThresholdModifier(iNewValue);
}
...
void CyPlayer::setFavoriteReligion(ReligionTypes newvalue)
{
    if (m_pPlayer)
        m_pPlayer->setFavoriteReligion(newvalue);
}

int CyPlayer::countGroupFlagUnits(int Groupflag) const
{
...
bool CyPlayer::AI_isWillingToTalk(int ePlayer) const
{
    return m_pPlayer ? m_pPlayer->AI_isWillingToTalk((PlayerTypes)ePlayer) : -1;
}

bool CyPlayer::isIgnoreFood() const
{
    return m_pPlayer ? m_pPlayer->isIgnoreFood() : -1;
}

int CyPlayer::getPopulationLimit() const
{
    return m_pPlayer ? m_pPlayer->getPopulationLimit() : -1;
}

/** Allow Python to modify the CivilizationType of a Player **/

void CyPlayer::setCivilizationType(int newCivType)
{
    if (m_pPlayer)
        m_pPlayer->setCivilizationType((CivilizationTypes)newCivType);
}

int CyPlayer::getCityStateType() const
{
    return m_pPlayer ? m_pPlayer->getCityStateType() : -1;
}

int CyPlayer::getCityStateCounter() const
{
    return m_pPlayer ? m_pPlayer->getCityStateCounter() : -1;
}

void CyPlayer::changeCityStateCounter(int newCivType)
{
    if (m_pPlayer)
        m_pPlayer->setCivilizationType((CivilizationTypes)newCivType);
}

int CyPlayer::getPurityCounter() const
{
    return m_pPlayer ? m_pPlayer->getPurityCounter() : -1;
}

void CyPlayer::setPurityCounter(int iNewValue)
{
    if (m_pPlayer)
        m_pPlayer->setPurityCounter(iNewValue);
}

int CyPlayer::getPurityCounterCache1() const
{
    return m_pPlayer ? m_pPlayer->getPurityCounterCache1() : -1;
}

void CyPlayer::setPurityCounterCache1(int iNewValue)
{
    if (m_pPlayer)
        m_pPlayer->setPurityCounterCache1(iNewValue);
}

bool CyPlayer::canIncreaseCityInfrastructureLevel(CyCity* pCity) const
{
	return m_pPlayer ? m_pPlayer->canIncreaseCityInfrastructureLevel(pCity->getCity()) : -1;
}

bool CyPlayer::isAgnostic() const
{
    return m_pPlayer ? m_pPlayer->isAgnostic() : -1;
}

int CyPlayer::getPrestige() const
{
    return m_pPlayer ? m_pPlayer->getPrestige() : -1;
}

void CyPlayer::changePrestige(int iChange)
{
    if (m_pPlayer)
        m_pPlayer->changePrestige(iChange);
}

void CyPlayer::setCurrentMagicRitual(int iNewValue)
{
	if(m_pPlayer)
		m_pPlayer->setCurrentMagicRitual(iNewValue);
}

/** Mana **/
int CyPlayer::getMana() const
{
    return m_pPlayer ? m_pPlayer->getMana() : -1;
}

void CyPlayer::changeMana(int change)
{
    if (m_pPlayer)
        m_pPlayer->changeMana(change);
}

int CyPlayer::getFaith() const
{
    return m_pPlayer ? m_pPlayer->getFaith() : -1;
}

void CyPlayer::changeFaith(int change)
{
    if (m_pPlayer)
        m_pPlayer->changeFaith(change);
}

int CyPlayer::getArcane() const
{
    return m_pPlayer ? m_pPlayer->getArcane() : -1;
}

void CyPlayer::changeArcane(int change)
{
    if (m_pPlayer)
        m_pPlayer->changeArcane(change);
}

int CyPlayer::getArcaneNeeded(int iTech) const
{
    return m_pPlayer ? m_pPlayer->getArcaneNeeded(iTech) : -1;
}

int CyPlayer::getArcaneIncome()
{
    return m_pPlayer ? m_pPlayer->getArcaneIncome() : -1;
}

int CyPlayer::calculateArcaneByBuildings() const
{
    return m_pPlayer ? m_pPlayer->calculateArcaneByBuildings() : -1;
}

int CyPlayer::calculateArcaneByManaSchool(int iManaSchool) const
{
    return m_pPlayer ? m_pPlayer->calculateArcaneByManaSchool((ManaschoolTypes)iManaSchool) : -1;
}

void CyPlayer::setSpellResearchFocus(int iChange)
{
    if (m_pPlayer)
        m_pPlayer->setSpellResearchFocus(iChange);
}

int CyPlayer::getSpellResearchFocus() const
{
    return m_pPlayer ? m_pPlayer->getSpellResearchFocus() : -1;
}

bool CyPlayer::canSpellResearchNext(int eSpellResearch) const
{
    return m_pPlayer ? m_pPlayer->canSpellResearchNext((TechTypes)eSpellResearch) : -1;
}

void CyPlayer::changeGlobalYield(int iYield, int iChange)
{
    if (m_pPlayer)
        m_pPlayer->changeGlobalYield(iYield, iChange);
}

int CyPlayer::getGlobalYield(int iYield) const
{
    return m_pPlayer ? m_pPlayer->getGlobalYield(iYield) : -1;
}

bool CyPlayer::canDoGlobalEnchantment(int eProject) const
{
	return m_pPlayer ? m_pPlayer->canDoGlobalEnchantment((ProjectTypes)eProject) : -1;
}

bool CyPlayer::canDoTerraformRitual(int eProject) const
{
	return m_pPlayer ? m_pPlayer->canDoTerraformRitual((ProjectTypes)eProject) : -1;
}

bool CyPlayer::canCreateSummon(int eProject) const
{
...
/**						                                            							**/
/*************************************************************************************************/
int CyPlayer::getCorporationSupport(int eCorporation) const
{
	return m_pPlayer ? m_pPlayer->getCorporationSupport((CorporationTypes)eCorporation) : -1;
}

int CyPlayer::getCorporationSupportMultiplier(int eCorporation) const
{
	return m_pPlayer ? m_pPlayer->getCorporationSupportMultiplier((CorporationTypes)eCorporation) : -1;
}

bool CyPlayer::isHasCorporationSupport(int eCorporation) const
{
	return m_pPlayer ? m_pPlayer->isHasCorporationSupport((CorporationTypes)eCorporation) : -1;
}

void CyPlayer::changeCorporationSupport(int eCorporation, int change)
{
	if (m_pPlayer)
        m_pPlayer->changeCorporationSupport((CorporationTypes)eCorporation, change);
}

void CyPlayer::changeCorporationSupportMultiplier(int eCorporation, int change)
{
	if (m_pPlayer)
        m_pPlayer->changeCorporationSupportMultiplier((CorporationTypes)eCorporation, change);
}

void CyPlayer::setHasCorporationSupport(int eCorporation, bool bNewValue)
{
	if (m_pPlayer)
        m_pPlayer->setHasCorporationSupport((CorporationTypes)eCorporation, bNewValue);
}
...
/*************************************************************************************************/
bool CyPlayer::isAdventureEnabled(int iAdventure) const
{
	return m_pPlayer ? m_pPlayer->isAdventureEnabled(iAdventure) : -1;
}
bool CyPlayer::isAdventureFinished(int iAdventure) const
{
	return m_pPlayer ? m_pPlayer->isAdventureFinished(iAdventure) : -1;
}
int CyPlayer::getAdventureStatus(int iAdventure, int iStep) const
{
	return m_pPlayer ? m_pPlayer->getAdventureStatus(iAdventure, iStep) : -1;
}

int CyPlayer::getAdventureLimit(int iAdventure, int iStep) const
{
	return m_pPlayer ? m_pPlayer->getAdventureLimit(iAdventure, iStep) : -1;
}

void CyPlayer::setAdventureEnabled(int iAdventure, bool bNewValue)
{
	if (m_pPlayer)
        m_pPlayer->setAdventureEnabled(iAdventure,bNewValue);
}

void CyPlayer::setAdventureFinished(int iAdventure, bool bNewValue)
{
	if (m_pPlayer)
        m_pPlayer->setAdventureFinished(iAdventure,bNewValue);
}

void CyPlayer::changeAdventureCounter(int iAdventure, int iStep, int iChange)
{
	if (m_pPlayer)
        m_pPlayer->changeAdventureCounter(iAdventure,iStep,iChange);
}

void CyPlayer::VerifyEnabledAdventures()
{
	if (m_pPlayer)
        m_pPlayer->VerifyEnabledAdventures();
}

void CyPlayer::VerifyFinishedAdventure(int iAdventure)
{
	if (m_pPlayer)
        m_pPlayer->VerifyFinishedAdventure(iAdventure);
}

void CyPlayer::VerifyFinishedAdventures()
{
	if (m_pPlayer)
        m_pPlayer->VerifyFinishedAdventures();
}

void CyPlayer::launchPickEquipmentPopup()
{
	if (m_pPlayer)
...
{
	return m_pPlayer ? m_pPlayer->isCanEverSpellResearch(iSpellResearch) : -1;
}


int CyPlayer::CalculateCivCounter(int iStep, int iValue)
{
	return m_pPlayer ? m_pPlayer->CalculateCivCounter(iStep, iValue) : -1;
}

void CyPlayer::setTerraformPlan(int iProject)
{
	if (m_pPlayer)
		m_pPlayer->setTerraformPlan(iProject);
}

int CyPlayer::getMagicRitualTurnsNeeded(int eProject, bool bStarted)
{
	return m_pPlayer ? m_pPlayer->getMagicRitualTurnsNeeded((ProjectTypes)eProject, bStarted) : -1;
}

int CyPlayer::getCurrentMagicRitual() const
{
	return m_pPlayer ? m_pPlayer->getCurrentMagicRitual() : -1;
}

int CyPlayer::getSpecificMagicRitualCost(int eProject) const
{
	return m_pPlayer ? m_pPlayer->getSpecificMagicRitualCost((ProjectTypes)eProject) : -1;
}  

int CyPlayer::getCurrentMagicRitualManaNeeded() const
{
	return m_pPlayer ? m_pPlayer->getCurrentMagicRitualManaNeeded() : -1;
}

int CyPlayer::getManaIncome() const
{
	return m_pPlayer ? m_pPlayer->getManaIncome() : -1;
}

bool CyPlayer::canSupportMoreUnits() const
{
	return m_pPlayer ? m_pPlayer->canSupportMoreUnits() : -1;
}

void CyPlayer::changeGlobalCulture(int iChange)
{
	if (m_pPlayer)
		m_pPlayer->changeGlobalCulture(iChange);	
}
int CyPlayer::getGlobalCulture() const
{
	return m_pPlayer ? m_pPlayer->getGlobalCulture() : -1;
}

void CyPlayer::setManaBonus2(int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setManaBonus2(iNewValue);	
}

int CyPlayer::getManaBonus2() const
{
	return m_pPlayer ? m_pPlayer->getManaBonus2() : -1;
}

void CyPlayer::setManaBonus3(int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setManaBonus3(iNewValue);	
}

int CyPlayer::getManaBonus3() const
{
	return m_pPlayer ? m_pPlayer->getManaBonus3() : -1;
}

void CyPlayer::setManaBonus4(int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setManaBonus4(iNewValue);	
}

int CyPlayer::getManaBonus4() const
{
	return m_pPlayer ? m_pPlayer->getManaBonus4() : -1;
}

bool CyPlayer::canGetCulturalAchievement(int iTech) const
{
	return m_pPlayer ? m_pPlayer->canGetCulturalAchievement((TechTypes)iTech) : -1;
}

bool CyPlayer::canGetTrait(int iTrait) const
{
	return m_pPlayer ? m_pPlayer->canGetTrait((TraitTypes)iTrait) : -1;
}

int CyPlayer::getTraitCost(int iTrait) const
{
	return m_pPlayer ? m_pPlayer->getTraitCost((TraitTypes)iTrait) : -1;
}

int CyPlayer::getUnitSupportLimitTotal() const
{
...
int CyPlayer::AI_getEconomyType()
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);

	if (pPlayer)
	{
		return pPlayer->AI_getEconomyType();
	}
