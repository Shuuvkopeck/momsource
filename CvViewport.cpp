...
void	CvViewport::resizeForMap(void)
{
	m_iXSize = 400;
	m_iYSize = 400;
...
void	CvViewport::bringIntoView(int iX, int iY, CvUnit* pSelectionUnit, bool bLookAt, bool bForceCenter, bool bDisplayCityScreen)
{
	...
	if ( m_transformType != VIEWPORT_TRANSFORM_TYPE_WINDOW || !isInViewport(iX, iY, getViewportSelectionBorder()) || bForceCenter)
	{
...
void	CvViewport::centerOnSelection(void)
{
...
void CvViewport::processActionState(void)
{
	if ( m_countdown > 0 )
	{
		m_countdown--;
			...
			for(int iI = 0; iI < numPlots(); iI++)
			{
				...
				if ( pPlot != NULL )
				{
					if ( pPlot->isRiverMask() &&
						 ...
						 pPlot->isAdjacentRevealed(GC.getGame().getActiveTeam()) )
					{
	...
	case VIEWPORT_ACTION_STATE_SET_LOOKAT:
		if ( ::getUnit(m_preservedHeadSelectedUnitId) == NULL && !m_bDisplayCityScreen )
		{
			GC.getGame().cycleSelectionGroups(true, true, false);
		}
		m_preservedHeadSelectedUnitId.reset();
		if ( m_pLookatPlot != NULL && m_pLookatPlot->isInViewport() )
		{
			...
			if ( m_bDisplayCityScreen && m_pLookatPlot->getPlotCity() != NULL )
			{
...
int	CvViewport::getViewportSelectionBorder() const
{
	return m_iViewportFocusBorder;
}

int	CvViewport::getViewportCenteringBorder() const
{
