...
void CyUnit::changeBaseCombatStr(int iChange)
{
	if (m_pUnit)
	{
		m_pUnit->changeBaseCombatStr(iChange);
	}
}

void CyUnit::changeBaseCombatStrDefense(int iChange)
{
	if (m_pUnit)
	{
		m_pUnit->changeBaseCombatStrDefense(iChange);
	}
}

void CyUnit::changeBaseCombatFromPotion(int iChange)
{
	if (m_pUnit)
	{
		m_pUnit->changeBaseCombatFromPotion(iChange);
	}
}

void CyUnit::changeBaseCombatDefenseFromPotion(int iChange)
{
	if (m_pUnit)
	{
		m_pUnit->changeBaseCombatDefenseFromPotion(iChange);
	}
}

int CyUnit::getRawBaseCombatStr()
{
	return m_pUnit ? m_pUnit->getRawBaseCombatStr() : -1;
}

int CyUnit::getRawBaseCombatStrDefense()
{
	return m_pUnit ? m_pUnit->getRawBaseCombatStrDefense() : -1;
}
...
{
	if (m_pUnit)
        m_pUnit->doDamage(iDmg, iDmgLimit, pAttacker->getUnit(), iDmgType, bStartWar);
}

void CyUnit::doDamageNoCaster(int iDmg, int iDmgLimit, int iDmgType, bool bStartWar)
{
	if (m_pUnit)
        m_pUnit->doDamage(iDmg, iDmgLimit, NULL, iDmgType, bStartWar);
}

void CyUnit::doEscape()
{
	if (m_pUnit)
        return m_pUnit->doEscape();
}
...
int CyUnit::getUnitArtStyleType() const
{
	return m_pUnit ? m_pUnit->getUnitArtStyleType() : false;
}
...
void CyUnit::exploreLair()
{
    if (m_pUnit)
        m_pUnit->exploreLair();
}

void CyUnit::equip(int /* PromotionTypes */ Equipment, bool bTreasure)
{
	if(m_pUnit)
		m_pUnit->equip((PromotionTypes)Equipment, bTreasure);
}

int CyUnit::getEquipment(int eCategory) const
{
	return m_pUnit ? m_pUnit->getEquipment((EquipmentCategoryTypes)eCategory) : -1;
}

bool CyUnit::isImmuneToCapture() const
{
	return m_pUnit ? m_pUnit->isImmuneToCapture() : -1;
}

void CyUnit::changeAIControl(int iNewValue)
{
    if (m_pUnit)
        m_pUnit->changeAIControl(iNewValue);
}

void CyUnit::changeDurability(int eCategory, int iChange)
{
    if (m_pUnit)
        m_pUnit->changeDurability((EquipmentCategoryTypes)eCategory,iChange);
}

void CyUnit::changeCannotLeaveCulture(int iChange)
{
    if (m_pUnit)
        m_pUnit->changeCannotLeaveCulture(iChange);
}

void CyUnit::changeStrengthPotionsUsed(int iChange)
{
    if (m_pUnit)
        m_pUnit->changeStrengthPotionsUsed(iChange);
}

void CyUnit::displayMessagePromotionGainedInBattle(int /*PromotionTypes*/ ePromotion) const
{
    if (m_pUnit)
        m_pUnit->displayMessagePromotionGainedInBattle((PromotionTypes)ePromotion);
}

/** Better Promotions **/
int CyUnit::AI_getPromotionSpecialization()
{
	return m_pUnit ? m_pUnit->AI_getPromotionSpecialization() : -1;
}

void CyUnit::AI_setPromotionSpecialization(int newSpecial)
{
	if (m_pUnit)
		m_pUnit->AI_setPromotionSpecialization(newSpecial);
}

bool CyUnit::isHeadSelectedUnit()
{
	return m_pUnit ? gDLL->getInterfaceIFace()->getHeadSelectedUnit()==m_pUnit : -1;
}
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
