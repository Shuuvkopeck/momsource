...
public:
	...
	void calculateWilderness();
	void calculatePlanes();
	void calculatePlanes_Default();
	void calculatePlanes_Areas();
	void calculatePlanes_OnlyOne();
	void calculatePlanes_Continents();
	int getNumPlanes();
	bool isPlanes();
	bool isPortals();
	void placePortals();
	void placePortalOnArea(int iArea, int iTargetArea = -1);
	int findLargestArea();
	int findNextLargestArea(int iArea);
	void placeEpicLairs();
	void placeCityStates();
	int placeCityState(CityStateTypes eState);

	CvDungeon* getDungeon(int iID);																// Exposed to Python
	CvDungeon* addDungeon();
	void deleteDungeon(int iID);
	// iteration
	CvDungeon* firstDungeon(int *pIterIdx, bool bRev=false);								// Exposed to Python
	CvDungeon* nextDungeon(int *pIterIdx, bool bRev=false);									// Exposed to Python
...
/*************************************************************************************************/
	...
	FFreeListTrashArray<CvDungeon> m_dungeons;
