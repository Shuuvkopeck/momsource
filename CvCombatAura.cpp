...
void CvCombatAura::reset(int iID, int iOwner, int iUnit, int iCombatAuraType, bool bConstructorCall, bool bCheckExistingPromotions)
{
	...
	m_iCaptureChance = (NO_COMBATAURA != m_eCombatAuraType) ? GC.getCombatAuraInfo(m_eCombatAuraType).getCaptureChance() : 0;
	m_iEnslaveChance = (NO_COMBATAURA != m_eCombatAuraType) ? GC.getCombatAuraInfo(m_eCombatAuraType).getEnslaveChance() : 0;
	m_iChanceMiscast = (NO_COMBATAURA != m_eCombatAuraType) ? GC.getCombatAuraInfo(m_eCombatAuraType).getChanceMiscast() : 0;
	...
	if(bCheckExistingPromotions)
	{
		...
		for(int iI=0;iI<GC.getNumPromotionInfos();iI++)
		{
			if(pOwner->isHasPromotion((PromotionTypes)iI))
			{
				...
				changeCaptureChance(kPromotion.getCombatAuraCaptureChanceChange());
				changeEnslaveChance(kPromotion.getCombatAuraEnslaveChanceChange());
...
void CvCombatAura::ApplyEffectsToUnit(CvUnit* pUnit, int iChange)
{
	...
	pUnit->changeCaptureChance(iTotalChange * getCaptureChance());
	pUnit->changeEnslaveChance(iTotalChange * getEnslaveChance());
	pUnit->changeChanceMiscast(iTotalChange * getChanceMiscast());
...
void CvCombatAura::updatePromotionAdded(PromotionTypes ePromotion, int iChange)
{
	...
	changeCaptureChance(kPromotion.getCombatAuraCaptureChanceChange() * iChange);
	changeEnslaveChance(kPromotion.getCombatAuraEnslaveChanceChange() * iChange);
...
void CvCombatAura::ProcessPowerEffect(int iChange)
{
	...
	{
		changeCaptureChance(iChange * getPower() / 4);
	}

	if (getEnslaveChance() != 0)
	{
		changeEnslaveChance(iChange * getPower() / 4);
	}

	if (getChanceMiscast() != 0)
	{
		changeChanceMiscast(iChange * getPower() / 4);
	}
...
int CvCombatAura::getCaptureChance() const { return m_iCaptureChance; }
void CvCombatAura::changeCaptureChance(int iChange){ m_iCaptureChance += iChange; }
int CvCombatAura::getEnslaveChance() const { return m_iEnslaveChance; }
void CvCombatAura::changeEnslaveChance(int iChange){ m_iEnslaveChance += iChange; }
int CvCombatAura::getChanceMiscast() const { return m_iChanceMiscast; }
void CvCombatAura::changeChanceMiscast(int iChange){ m_iChanceMiscast += iChange; }
