...
void CyPlot::changePlotCounter(int iChange) const
{
	if (m_pPlot)
...
void CyPlot::setDungeonType(int iNewValue)
{
	if (m_pPlot)
		m_pPlot->setDungeonType(iNewValue);
}

int CyPlot::getDungeonType() const
{
	return m_pPlot ? m_pPlot->getDungeonType() : -1;
}
...
int CyPlot::getPlane() const
{
	return m_pPlot ? m_pPlot->getPlane() : -1;
}

void CyPlot::destroyDungeon()
{
	if(m_pPlot)
		m_pPlot->destroyDungeon();
}
