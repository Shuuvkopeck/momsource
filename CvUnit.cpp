...
/*************************************************************************************************/
	...
	for (iI = 0; iI < GC.getNumPromotionInfos(); iI++)
	{
		if (isHasPromotion((PromotionTypes)iI))
		{
			...
			//Darksavant
			//Added Magus check here since I couldn't get it to work from Python
			if (GC.getInfoTypeForString("PROMOTION_MAGUS_HERO") == iI)
			{
				setBaseCombatStr(baseCombatStrDefense());
			}
...
//FfH: End Modify
		...
		if (isDead() || pDefender->isDead())
		{
			...
			else
			{
				...
				//Darksavant
				//Letting summoned units give their summoners XP when they kill a unit
				if (getSummoner() != -1)
				{
					int iLoop;
					...
					{
						if (pLoopUnit->getID() == getSummoner())
						{
							pLoopUnit->changeExperience(iExperience, pDefender->maxXPValue(), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), !pDefender->isBarbarian());
						}
					}
				}
			}
...
int CvUnit::bombard(const CvPlot* pTarget, bool bCoundDamageOnly)
{
	...
	{
		int iMaxDamage = pBombardCity->getTotalDefense(false)*(100-bombardRate())/100;
...
/*************************************************************************************************/
        iUnitExperience = std::max(1,iUnitExperience);
...
int CvUnit::getExtraMoves() const
{
	/*if (m_iExtraMoves < 0)
		return 0;
	else if (m_iExtraMoves > 6)
		return 6;
	else	
		return m_iExtraMoves;*/

	//Added by Darksavant
	//The below is much less efficient than the original solution but fixes a pernicious bug where the m_iExtraMoves variable was getting corrupted into really large or negative values both which could
	//effectively destroy the game.  I never could figure out the root cause of the original bug but the below seems to work correctly and if it slows the game down it is hard to tell.
	int iExtraMoves = 0;
	for (int iI = 0; iI < GC.getNumPromotionInfos(); iI++)
    {
        if (isHasPromotion((PromotionTypes)iI))
        {
			iExtraMoves = GC.getPromotionInfo((PromotionTypes)iI).getMovesChange() + iExtraMoves;
        }
    }
	return iExtraMoves;
...
/*************************************************************************************************/
		...
		{
			//Darksavant
			//Commenting out check to allow a promotion with a different combatauratype to override an initial one, such as in the case of allowing an adept unit to get an offensive aura instead of a blessing
			//As far as I can tell this really shouldn't mess anything up
			/*if(getCombatAuraType()==NO_COMBATAURA)
			{
...
/*************************************************************************************************/
	...
	if(!kSpell.isTargetedHostile())
	{
		...
		{
			if(plot()->getTerrainType()!=kSpell.getTerrainOrPrereq1())
			{
				if(kSpell.getTerrainOrPrereq2()==NO_TERRAIN || kSpell.getTerrainOrPrereq2()!=plot()->getTerrainType())
				{
					return false;
				}
			}
		...
		{
			if(plot()->getFeatureType()!=kSpell.getFeatureOrPrereq1())
			{
				if(kSpell.getFeatureOrPrereq2()==NO_TERRAIN || kSpell.getFeatureOrPrereq2()!=plot()->getFeatureType())
				{
					return false;
				}
			}
...
/*************************************************************************************************/
	...
	{
		if (GET_PLAYER(getOwnerINLINE()).getFaith()<kSpell.getFaithCost())
		{
			return false;
		}
	}
...
/*************************************************************************************************/
	...
	if (GC.getSpellInfo(eSpell).getCreateBuildingType1() != NO_BUILDING)
	{
		...
		if (plot()->getPlotCity()->getNumBuilding((BuildingTypes)GC.getSpellInfo((SpellTypes)spell).getCreateBuildingType1()) > 0)
		{
			return false;
		}
	}
	...
	if (GC.getSpellInfo(eSpell).getCreateBuildingType2() != NO_BUILDING)
	{
		...
		if (plot()->getPlotCity()->getNumBuilding((BuildingTypes)GC.getSpellInfo((SpellTypes)spell).getCreateBuildingType2()) > 0)
		{
			return false;
		}
	}
	...
	if (GC.getSpellInfo(eSpell).getRemoveBuildingType() != NO_BUILDING)
	{
		...
		if (plot()->getPlotCity()->getNumBuilding((BuildingTypes)GC.getSpellInfo((SpellTypes)spell).getRemoveBuildingType()) == 0)
		{
			return false;
		}
	}
...
/*************************************************************************************************/
	...
	{
		iMiscastChance += getChanceMiscast();
	}
...
/*************************************************************************************************/
	...
	if(kSpell.isThiefMission() && isSpy()) {
		int iCoolDown = std::max(5, kSpell.getThiefDifficulty() / 5);
		...
		if(tryThiefArrested((SpellTypes)spell)) {
			...
			//Darksavant
			//Adding negative diplomatic repurcussions if your thief is captured during the attempt
			//Added new XML tag iThiefRelationsHit for spellinfo to support this
			int iRelationsHit = kSpell.getThiefRelationsHit();

			if (plot()->isCity()) {
				//GET_PLAYER(getOwnerINLINE()).displayText("Got to relation code", getX_INLINE(), getY_INLINE());
				//GET_PLAYER(plot()->getOwnerINLINE()).AI_changeMemoryCount(getOwnerINLINE(), MEMORY_SPY_CAUGHT, iRelationsHit);
				GET_PLAYER(plot()->getOwnerINLINE()).AI_changeMemoryCount(getOwnerINLINE(), MEMORY_SPY_CAUGHT, iRelationsHit);
				//GET_PLAYER(plot()->getOwnerINLINE()).AI_changeAttitudeExtra(getOwnerINLINE(), iRelationsHit);
			}
...
/*************************************************************************************************/
	...
	if(!(kSpell.isTargetedHostile() && pTargetPlot==NULL))
	{
		...
		if(pTargetPlot!=plot()) {
			...
			int iXPSoftCap = kSpell.getManaCost() * 2;
			if ((iXPSoftCap - getExperience()) <= 0)
			{
				iXPMin = iXPMin / 2;
			}
			changeExperience(iXPMin);
		}
		...
		if(kSpell.getTriggerSecondaryPlotChance()>0)
		{
			for (iI = 0; iI < NUM_DIRECTION_TYPES; ++iI)
			{
				...
				{
					castDamageSecondaryPlot(spell, pAdjacentPlot);
				}
			}
		}
	}
...
/**	ADDON (New Mana) Sephi                                          					**/
/*************************************************************************************************/
    GET_PLAYER(getOwnerINLINE()).changeFaith(-GC.getSpellInfo((SpellTypes)spell).getFaithCost());
...
int CvUnit::castDamage(int spell, CvPlot* pTargetplot, bool bCountDamage)
{
	...
	int iCritChance = (getUnitInfo().getTier()-1)*5;
	iCritChance *= (kSpell.isHighCrit()) ? 2 : 1;
	PromotionTypes iDoTPromotion = (PromotionTypes)kSpell.getDoT();
    int iDmgLimit = kSpell.getDamageLimit();
    int iDmgType = kSpell.getDamageType();
...
//						{
						...
						iDamageCount+=iMod*pBestUnit->doDamage(iDamage,iDmgLimit,this,iDmgType,false,true,true);
						iDamageCount+=castDamage_addSpecialValue(spell,pLoopUnit,iMod);
...
void CvUnit::castDamageSecondaryPlot(int spell, CvPlot* pTargetPlot)
{
	castDamage(spell, pTargetPlot);
...
int CvUnit::doDamage(int iDmg, int iDmgLimit, CvUnit* pAttacker, int iDmgType, bool bStartWar, bool bCountOnly, bool bSpell)
{
    ...
    iResist = (getUnitInfo().getTier()-1) * 10;
    iResist += getLevel();
    ...
    iDmg = (iDmg * iDamageMod) / 100;

	//Damage is Percentage of Enemy HP
	if(bSpell) {
		iDmg *= 100 - std::min(33, getDamage());
		iDmg /= 100;
	}
...
void CvUnit::combatWon(CvUnit* pLoser, bool bAttacking)
{
	...
	//Darksavant
	//Added unit-based general capture chance, in my case for a combat auram
	if (getCaptureChance() > 0)
	{
		if (GC.getGameINLINE().getSorenRandNum(100, "Combat Capture") < getCaptureChance())
		{
			iUnit = pLoser->getUnitType();
			bConvert = true;
		}
	...
	//Darksavant
	//added unit-based enslave chance, in my case to support an enslaving combat aura
    if ((m_pUnitInfo->getEnslavementChance() + getEnslaveChance() + GET_PLAYER(getOwnerINLINE()).getEnslavementChance()) > 0)
    {
...
void CvUnit::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(m_iCaptureChance);
	pStream->Write(m_iEnslaveChance);
...
int CvUnit::getEnslaveChance() const
{
	return m_iEnslaveChance;
}

void CvUnit::setEnslaveChance(int iNewValue)
{
	m_iEnslaveChance = iNewValue;
}

void CvUnit::changeEnslaveChance(int iChange)
{
	setEnslaveChance(getEnslaveChance() + iChange);
}

int CvUnit::getCaptureChance() const
{
	return m_iCaptureChance;
}

void CvUnit::setCaptureChance(int iNewValue)
{
	m_iCaptureChance = iNewValue;
}

void CvUnit::changeCaptureChance(int iChange)
{
	setCaptureChance(getCaptureChance() + iChange);
}
...
void CvUnit::equip(PromotionTypes Equipment, bool bTreasure)
{
	...
	if(!bTreasure)
	{
		...
		GET_PLAYER(getOwnerINLINE()).changeGold((-kPromotion.getGoldCost()*iMod)/100);
		GET_PLAYER(getOwnerINLINE()).changeMana((-kPromotion.getManaCost()*iMod)/100);
		GET_PLAYER(getOwnerINLINE()).changeFaith((-kPromotion.getFaithCost()*iMod)/100);
	}
...
void CvUnit::exploreLair()
{
	...
	if(iResult != NO_LAIRRESULT)
	{
		...
		if(!GC.getImprovementInfo((ImprovementTypes)plot()->getImprovementType()).isUnique())
		{
			...
			//Darksavant
			//Added below to kill off any barbarian defenders that are still left when exploring a dungeon.  This should only happen when Rantine does it using his ability that I gave him to do so (also changing reqExploreLair in python).
			CvUnit* pLoopUnit;
			CLLNode<IDInfo>* pUnitNode;
			pUnitNode = plot()->headUnitNode();
			while (pUnitNode != NULL)
...
void CvUnit::doAnimalTurnedToFood(UnitTypes eAnimal)
{
	...
	int iFood = 100 + GC.getGame().getElapsedGameTurns();
...
int CvUnit::getThiefMissionSuccessChance(SpellTypes eSpell) const
{
	...
	//increasing chance of success if team has built Nox Noctis
	if (GET_TEAM(getTeam()).getBuildingClassCount((BuildingClassTypes)GC.getInfoTypeForString("BUILDINGCLASS_NOX_NOCTIS")) == 1)
	{
		iChance += 20;
	}
