...
/*************************************************************************************************/
	gDLL->getEngineIFace()->clearAreaBorderPlots(AREA_BORDER_LAYER_SPELL_OFFENSIVE);
/*************************************************************************************************/
/**	END                                                                                         **/
/*************************************************************************************************/
...
/**	ADDON(INTERFACEMODE_SPELL_OFFENSIVE) Sephi					                                **/
/*************************************************************************************************/		
		iRange=GC.getUnitInfo(pHeadSelectedUnit->getUnitType()).getArcaneRange();
		iRange=std::max(iRange,1);
		for(int iI=0;iI<GC.getNumSpellInfos();iI++)
		{
			if(GC.getSpellInfo((SpellTypes)iI).isTargetedHostile() && pHeadSelectedUnit->canCast(iI,false))
			{
				for (iDX = -(iRange); iDX <= iRange; iDX++)
				{
					for (iDY = -(iRange); iDY <= iRange; iDY++)
					{
						...
						if (pTargetPlot != NULL && pHeadSelectedUnit->canCastAt((SpellTypes)iI,pTargetPlot))
						{
							if(pHeadSelectedUnit->canSpellTargetPlot(pTargetPlot))
							{
								NiColorA color(GC.getColorInfo((ColorTypes)GC.getInfoTypeForString("COLOR_RED")).getColor());
								color.a = 0.5f;
								gDLL->getEngineIFace()->fillAreaBorderPlot(pTargetPlot->getX_INLINE(), pTargetPlot->getY_INLINE(), color, AREA_BORDER_LAYER_SPELL_OFFENSIVE);
							}
						}
					}
				}
...
/**
	bool bSetCamera = true; //TEMPORARY
	bool bViewportSwitched = false;

	if (pNextSelectionGroup != NULL)
	{
		if ( !GC.getCurrentViewport().isInViewport(pNextSelectionGroup->getX(), pNextSelectionGroup->getY(), GC.getCurrentViewport().getViewportSelectionBorder()) )
		{
			CvUnit*	pSelectionUnit = pNextSelectionGroup->getHeadUnit();

			bool bLookatUnit = (bSetCamera && ((pCycleUnit != pSelectionUnit) || ((pCycleUnit != NULL) && pCycleUnit->getGroup()->readyToSelect())));

			bViewportSwitched = true;

			GC.getCurrentViewport().bringIntoView(pNextSelectionGroup->getX(),
												   pNextSelectionGroup->getY(),
												   pSelectionUnit,
												   bLookatUnit);
		}
	...
	if (!bViewportSwitched &&
		bSetCamera &&
		((pCycleUnit != gDLL->getInterfaceIFace()->getHeadSelectedUnit()) || ((pCycleUnit != NULL) && pCycleUnit->getGroup()->readyToSelect())))
	{
		gDLL->getInterfaceIFace()->lookAtSelectionPlot();
	}
/*************************************************************************************************/
...
//FfH: Modified by Kael 11/04/2007
//			}
//			else
//			{
                bSkip = true;
                        ...
                        for (iDY = -(iRange); iDY <= iRange; iDY++)
                        {
                            ...
                            if (pLoopPlot != NULL)
                            {
                                ...
                                while (pUnitNode != NULL)
                                {
                                    ...
                                    if (pLoopUnit!=NULL)
                                    {
                                        if (pLoopUnit->getTeam()!=pHeadSelectedUnit->getTeam())
                                        {
                                            if (pLoopUnit->getTeam()==pHeadSelectedUnit->plot()->getTeam())
                                            {
                                                if (!GET_TEAM(pLoopUnit->getTeam()).isVassal(pHeadSelectedUnit->getTeam()))
                                                {
                                                    if(!GET_TEAM(pLoopUnit->getTeam()).isAtWar(pHeadSelectedUnit->getTeam()))
                                                    {
                                                        counttargets++;
                                                    }
                                                }
...
//FfH Card Game: Added by Sto 07/28/2008
...
/*************************************************************************************************/
	...
	case CONTROL_MAGIC_SCREEN:
	case CONTROL_ARCANE_SCREEN:
/*************************************************************************************************/
/**	END                                                                                         **/
/*************************************************************************************************/
...
//FfH Card Game: Added by Sto 07/28/2008
	case CONTROL_CORPORATION_SCREEN:
		...
		{
			if (GET_PLAYER((PlayerTypes)i).isTurnActive() && GET_PLAYER((PlayerTypes)i).isHuman())
			{
				return true;
			}
		}
...
/*************************************************************************************************/
	...
	case CONTROL_WINAMP_PAUSE:
	case CONTROL_WINAMP_NEXT:
	case CONTROL_WINAMP_PREV:
		return true;
		break;
/*************************************************************************************************/
/**	ADDON (WINAMP)						END														**/
...
/**
		...
		if ( pPlot != NULL )
		{
			if ( !pPlot->isInViewport(GC.getCurrentViewport().getViewportCenteringBorder()))
			{
				GC.getCurrentViewport().bringIntoView(pPlot->getX_INLINE(), pPlot->getY_INLINE(), gDLL->getInterfaceIFace()->getHeadSelectedUnit(), true, true);
			}
			else
			{
				gDLL->getInterfaceIFace()->lookAtSelectionPlot();
...
/**
		{
			...
			{
				GC.getCurrentViewport().bringIntoView(pCity->getX_INLINE(), pCity->getY_INLINE(), NULL, true, false, gDLL->getInterfaceIFace()->isCityScreenUp());
			}
		}
/*************************************************************************************************/
/**	END Viewport C2C			                                        						**/
...
/*************************************************************************************************/
	...
	case CONTROL_CORPORATION_SCREEN:
//FfH Card Game: Modified by Sto 07/28/2008
		//gDLL->getPythonIFace()->callFunction(PYScreensModule, "showCorporationScreen");
		{
			CyArgsList argsList;
			argsList.add(getActivePlayer());
			gDLL->getPythonIFace()->callFunction(PYSomniumModule, "showSomniumIntroPopUp", argsList.makeFunctionArgs());
		}
//FfH: End Add
		break;
...
//FfH: Added by Kael 09/24/2008
	case CONTROL_TROPHY:
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "showTrophyHall");
		break;
//FfH: End Add
...
/**
	...
	{
		GC.getCurrentViewport().bringIntoView(pSelectionUnit->getX_INLINE(), pSelectionUnit->getY_INLINE(), pSelectionUnit, true);
	}
/*************************************************************************************************/
/**	END Viewport C2C			                                        						**/
/*************************************************************************************************/
...
bool CvGame::ctrlKey() const
{
	return gDLL->ctrlKey();
}

bool CvGame::altKey() const
{
	return gDLL->altKey();
}
//<<<<	BUGFfH: End Add
