...
//FfH: End Modify
	...
	iAttackerLowFS = (pDefender->immuneToFirstStrikes()) ? 0 : std::max(0,pAttacker->firstStrikes()-pDefender->getIgnoreFirstStrikes());
	iAttackerHighFS = (pDefender->immuneToFirstStrikes()) ? 0 : std::max(0,pAttacker->firstStrikes()+ pAttacker->chanceFirstStrikes()-pDefender->getIgnoreFirstStrikes()); 

	iDefenderLowFS = (pAttacker->immuneToFirstStrikes()) ? 0 : std::max(0,pDefender->firstStrikes()-pAttacker->getIgnoreFirstStrikes());
	iDefenderHighFS = (pAttacker->immuneToFirstStrikes()) ? 0 : std::max(0,pDefender->firstStrikes()+ pDefender->chanceFirstStrikes()-pAttacker->getIgnoreFirstStrikes()); 
...
//	iOdds /= (((pDefender->immuneToFirstStrikes()) ? 0 : pAttacker->chanceFirstStrikes()) + 1) * (((pAttacker->immuneToFirstStrikes()) ? 0 : pDefender->chanceFirstStrikes()) + 1);
	...
	//modified Sephi
	if(iOdds<GC.getDefineINT("THRESHOLD_COMBATODDS_ALWAYS_WIN"))
		return 0;

	if(iOdds>1000-GC.getDefineINT("THRESHOLD_COMBATODDS_ALWAYS_WIN"))
		return 1000;
	//end modified Sephi
...
//						iCost += (PATH_DEFENSE_WEIGHT * std::max(0, (200 - ((pLoopUnit->noDefensiveBonus()) ? 0 : pToPlot->defenseModifier(pLoopUnit->getTeam(), false)))));
						const int iDefenseModifier = pToPlot->defenseModifier(pLoopUnit->getTeam(), false);
						iCost += PATH_DEFENSE_WEIGHT * std::max(0, 200 - ((pLoopUnit->noDefensiveBonus() && iDefenseModifier > 0) ? 0 : iDefenseModifier));
...
//									iCost += (PATH_DEFENSE_WEIGHT * std::max(0, (200 - ((pLoopUnit->noDefensiveBonus()) ? 0 : pFromPlot->defenseModifier(pLoopUnit->getTeam(), false)))));
									const int iDefenseModifier = pFromPlot->defenseModifier(pLoopUnit->getTeam(), false);
									iCost += PATH_DEFENSE_WEIGHT * std::max(0, 200 - ((pLoopUnit->noDefensiveBonus() && iDefenseModifier > 0) ? 0 : iDefenseModifier));
//<<<<Unofficial Bug Fix: End Modify
...
/*************************************************************************************************/
	case MISSION_SPELL_OFFENSIVE: szString = L"MISSION_SPELL_OFFENSIVE"; break;
	case MISSION_TERRAFORM: szString = L"MISSION_TERRAFORM"; break;
/*************************************************************************************************/
/**	END                                                                                         **/
/*************************************************************************************************/
...
void getAI_InvasionStatusString(CvWString& szString, int Missionstatus)
{
	switch (Missionstatus)
	{
	case NO_AIGROUP_STATUS: szString = L"no MissionStatus"; break;

	case STATUS_CITY_INVASION_TAKECITY: szString = L"Missionstatus Invasion TakeCity"; break;
	case STATUS_CITY_INVASION_WAIT_MORE_UNITS: szString = L"Missionstatus Invasion Wait More Units"; break;
	case STATUS_CITY_INVASION_TRAIN_HERO: szString = L"Missionstatus Invasion Train Hero"; break;
	case STATUS_CITY_INVASION_PILLAGE_STUFF: szString = L"Missionstatus Invasion Pillage Stuff"; break;
	case STATUS_CITY_INVASION_LOOK_FOR_OTHER_TARGET: szString = L"Missionstatus Invasion Look for other Target"; break;
	case STATUS_CITY_INVASION_WAIT_FOR_PEACE: szString = L"Missionstatus Invasion Wait For Peace"; break;

	default: szString = CvWString::format(L"unknown(%d)", Missionstatus); break;
	}
}

int calculateFlankingBonus(const CvUnit* pAttacker, const CvUnit* pDefender)
{
	...
	int iFlankingBonusPerTile=GC.getDefineINT("UNIT_PER_TILE_LIMIT_FLANKING");
	int iBonus=0;

	for (int iI = 0; iI < NUM_DIRECTION_TYPES; iI++)
	{
		CvPlot* pAdjacentPlot = plotDirection(pDefender->getX_INLINE(), pDefender->getY_INLINE(), ((DirectionTypes)iI));
		if(pAdjacentPlot!=NULL)
		{
			if(pAttacker==NULL || pAdjacentPlot!=pAttacker->plot())
			{
				if(pAdjacentPlot->isVisibleEnemyDefender(pDefender))
				{
					iBonus+=iFlankingBonusPerTile;
				}
			}
		}
	}
...
bool isLimitedPerTile(UnitTypes eUnit)
{
	CvUnitInfo& kUnit=GC.getUnitInfo(eUnit);

	if(kUnit.getCombat()==0 && kUnit.getCombatDefense()==0)
	{
		return false;
	}
...
bool isTrainingBuilding(BuildingTypes eBuilding, PlayerTypes ePlayer, bool bStateReligion, bool bTech)
{
	if(eBuilding==NO_BUILDING || ePlayer==NO_PLAYER)
	{
		return false;
	}
	...
	for(int iI=0;iI<GC.getNumUnitInfos();iI++)
	{
		...
		if(kUnit.getPrereqBuilding()==eBuilding || kUnit.getPrereqBuildingClass()==eBuildingClass)
		{
			if(kUnit.getPrereqCiv()==NO_CIVILIZATION || kUnit.getPrereqCiv()==kPlayer.getCivilizationType())
			{
				if(!bStateReligion || kUnit.getPrereqReligion()==NO_RELIGION || kUnit.getPrereqReligion()==kPlayer.getStateReligion())
				{
					if(!bTech || kUnit.getPrereqAndTech()==NO_TECH || GET_TEAM(kPlayer.getTeam()).isHasTech((TechTypes)kUnit.getPrereqAndTech()))
					{
						if(!(::isWorldUnitClass((UnitClassTypes)kUnit.getUnitClassType())) && kUnit.getProductionCost()>0)
						{
							return true;
						}
					}
