		...
		for (int iI = 0; iI < getNumCityPlots(); iI++)
		{
			...
			if (pLoopPlot != NULL)
			{
				if(!(pLoopPlot->isWater() || pLoopPlot->isPeak()))
				{
					...
					{
						iTempValue+=100*GC.getTerrainInfo(eTerrainSnow).getYield((YieldTypes)iI)-GC.getTerrainInfo(pLoopPlot->getTerrainType()).getYield((YieldTypes)iI);					
					}
				}
			}
		}
...
//		if ((!kPlayer.isConquestMode()) && kPlayer.getNumCities()>2)
	...
	/** DEBUG **/
	TCHAR szOut[1024];
	sprintf(szOut, "AIBestBuildingValue %d\n",iBestBuildingValue);
