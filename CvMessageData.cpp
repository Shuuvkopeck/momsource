...
//added Sephi Spellresearch
			else if(GC.getTechInfo(m_eTech).isSpellResearch())
			{
				if(kPlayer.canSpellResearchNext(m_eTech))
				{
					kPlayer.setSpellResearchFocus(m_eTech);
				}
			}
			else if(GC.getTechInfo(m_eTech).isHolyKnowledge())
			{
				kPlayer.BuyHolyKnowledge(m_eTech);
			}
			else if(GC.getTechInfo(m_eTech).isGuild())
			{
				if(kPlayer.isAllowedMoreGuilds())
					GET_TEAM(kPlayer.getTeam()).setHasTech(m_eTech, true, m_ePlayer, true, true);
			}
			else if(GC.getTechInfo(m_eTech).isReligion())
			{
				if(kPlayer.isAllowedMoreReligions())  {
					if(!GET_TEAM(kPlayer.getTeam()).isHasTech(m_eTech))
						GET_TEAM(kPlayer.getTeam()).setHasTech(m_eTech, true, m_ePlayer, true, true);
					else {
						kPlayer.changeFaith(3000);
						kPlayer.changeReligionsSpread(1);
					}
				}
			}
//end added Sephi
