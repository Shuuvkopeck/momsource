...
CvTeamAI::CvTeamAI()
{
	...
	m_aiWarValue = new int[MAX_TEAMS];
	m_aiWarValueAccumulated = new int[MAX_TEAMS];
// Better War AI (Skyre)
	m_aiWarPlanCounter = new int[MAX_TEAMS];
// End Better War AI
...
CvTeamAI::~CvTeamAI()
{
	...
	SAFE_DELETE_ARRAY(m_aiWarValueAccumulated);
// Better War AI (Skyre)
	SAFE_DELETE_ARRAY(m_aiWarPlanCounter);
// End Better War AI
/*************************************************************************************************/
/**	SPEEDTWEAK (CAR MOD) merged Sephi                                         					**/
/**																								**/
...
void CvTeamAI::AI_reset(bool bConstructor)
{
	...
	for (int iI = 0; iI < MAX_TEAMS; iI++)
	{
		...
		m_aiWarValue[iI] = 0;
		m_aiWarValueAccumulated[iI] = 0;
// Better War AI (Skyre)
		m_aiWarPlanCounter[iI] = 0;
// End Better War AI
...
/*************************************************************************************************/
		...
		if (!bConstructor && getID() != NO_TEAM)
		{
			...
			kLoopTeam.m_aiWarValue[getID()] = 0;
			kLoopTeam.m_aiWarValueAccumulated[getID()] = 0;
/*************************************************************************************************/
/**	SPEEDTWEAK (CAR MOD) merged Sephi                                         					**/
/**																								**/
/**	                                                                 							**/
...
// Better War Diplomacy (Skyre)
			...
			*/
			if ((countNumAIUnitsByArea(pArea, UNITAI_ATTACK) +
				 ...
				 countNumAIUnitsByArea(pArea, UNITAI_COUNTER) +
				 countNumAIUnitsByArea(pArea, UNITAI_WARWIZARD) +
				 countNumAIUnitsByArea(pArea, UNITAI_HERO) +
				 countNumAIUnitsByArea(pArea, UNITAI_FEASTING) +
				 countNumAIUnitsByArea(pArea, UNITAI_ATTACK_AIR)) >
				   	(((AI_countMilitaryWeight(pArea) * 20) / 100) + 1))
...
/*************************************************************************************************/
/**	BETTER AI (Diplomacy) Sephi		                                         					**/
/**	Function AI_endWarVal Rewritten																**/
/**	Factors are only Power Difference and WarSuccess                   							**/
/*************************************************************************************************/
	iValue=-AI_getWarValue(eTeam);	//Peacevalue is negative Warvalue
	if(iValue<200)
	{
		return 0;
	}
...
/**
	...
	iValue =std::min(10000,iValue);
	int iPower = std::max(1, getPower(true));
	int iEnemyPower = std::max(1, GET_TEAM(eTeam).getDefensivePower());

	int iPowerRatio=10*iEnemyPower/std::max(iPower,1);
	...
	iPowerRatio=std::max(iPowerRatio,100);
...
**/
	int iPowerRatio=100*(iTheirPower + 10)/std::max(1, iOurPower + 10);
...
**/
        int iMod=0;
        if (GET_PLAYER(*iter).getAlignment()==GET_PLAYER(getLeaderID()).getAlignment() ||
            GET_PLAYER(*iter).getFavoriteReligion()==GET_PLAYER(getLeaderID()).getFavoriteReligion())
        {
            iMod++;
        }
		if ((eAttitude+iMod) <= GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getTechRefuseAttitudeThreshold())
/*************************************************************************************************/
/** End															    							**/
/*************************************************************************************************/
		{
...
int CvTeamAI::AI_mapTradeVal(TeamTypes eTeam) const
{
	...
	/** Easier and more expensive Calculation
	iValue /= 10;
...
**/
		...
		if (eAttitude+iMod <= GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getMapRefuseAttitudeThreshold())
/*************************************************************************************************/
/** End															    							**/
/*************************************************************************************************/
		{
...
/*************************************************************************************************/
/** CAPITULATION_EDIT   Till    27.04.2010                                                      **/
/*************************************************************************************************/
	//Are we are being badly overpowered?
	int iSurrenderPowerRatio=GC.getDefineINT("AI_SURRENDER_RATIO");
    if (
         isAtWar(eTeam) &&
         GET_TEAM(eTeam).getPower(true) > iSurrenderPowerRatio* getPower(true)
       )
    {
        return NO_DENIAL;
    }
...
**/
			...
			if (eModifiedAttitude+iMod <= GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getVassalRefuseAttitudeThreshold())
/*************************************************************************************************/
/** End															    							**/
/*************************************************************************************************/
			{
...
/** BETTER AI (Better Diplomatics) Sephi		                	    						**/
/*************************************************************************************************/
/** moved to the end of the function to make AI less predictable
...
**/
		...
		if (eAttitude + iMod <= GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getDeclareWarRefuseAttitudeThreshold())
/*************************************************************************************************/
/** End															    							**/
/*************************************************************************************************/
		{
...
**/
		...
		if (eAttitudeThem +iMod> GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getDeclareWarThemRefuseAttitudeThreshold())
/*************************************************************************************************/
/** End															    							**/
/*************************************************************************************************/
		{
...
**/
		...
		if (eAttitude+iMod <= GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getOpenBordersRefuseAttitudeThreshold())
/*************************************************************************************************/
/** End															    							**/
/*************************************************************************************************/
...
**/
		...
		if (eAttitude+iMod <= GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getDefensivePactRefuseAttitudeThreshold())
/*************************************************************************************************/
/** End															    							**/
/*************************************************************************************************/
...
**/
		...
		if (eAttitude +iMod<= GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getPermanentAllianceRefuseAttitudeThreshold())
/*************************************************************************************************/
/** End															    							**/
/*************************************************************************************************/
		{
...
TeamTypes CvTeamAI::AI_getWarPlanTarget() const
{
	return m_eWarPlanTarget;
}

void CvTeamAI::AI_setWarPlanTarget(TeamTypes eNewTeam)
{
	m_eWarPlanTarget = eNewTeam;
}
...
int CvTeamAI::AI_getWarValue(TeamTypes eIndex) const
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < MAX_TEAMS, "eIndex is expected to be within maximum bounds (invalid Index)");
	return m_aiWarValue[eIndex];
}


void CvTeamAI::AI_setWarValue(TeamTypes eIndex, int iNewValue)
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < MAX_TEAMS, "eIndex is expected to be within maximum bounds (invalid Index)");
	m_aiWarValue[eIndex] = iNewValue;
}

int CvTeamAI::AI_getWarValueAccumulated(TeamTypes eIndex) const
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < MAX_TEAMS, "eIndex is expected to be within maximum bounds (invalid Index)");
	return m_aiWarValueAccumulated[eIndex];
}


void CvTeamAI::AI_setWarValueAccumulated(TeamTypes eIndex, int iNewValue)
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < MAX_TEAMS, "eIndex is expected to be within maximum bounds (invalid Index)");
	m_aiWarValueAccumulated[eIndex] = iNewValue;
}

void CvTeamAI::AI_changeWarValueAccumulated(TeamTypes eIndex, int iChange)
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < MAX_TEAMS, "eIndex is expected to be within maximum bounds (invalid Index)");
	m_aiWarValueAccumulated[eIndex] += iChange;
}
...
void CvTeamAI::read(FDataStreamBase* pStream)
{
	...
	pStream->Read((int*)&m_eWarPlanTarget);
	pStream->Read(MAX_TEAMS, m_aiWarValue);
	pStream->Read(MAX_TEAMS, m_aiWarValueAccumulated);
/*************************************************************************************************/
/**	SPEEDTWEAK (CAR MOD) merged Sephi                                         					**/
/**																								**/
/**	                                                                 							**/
...
void CvTeamAI::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(m_eWarPlanTarget);
	pStream->Write(MAX_TEAMS, m_aiWarValue);
	pStream->Write(MAX_TEAMS, m_aiWarValueAccumulated);
/*************************************************************************************************/
/**	SPEEDTWEAK (CAR MOD) merged Sephi                                         					**/
/**																								**/
/**	                                                                 							**/
				...
				if (iI != getID())
				{
					if (!(GET_TEAM((TeamTypes)iI).isHuman()))
					{
						if (canContact((TeamTypes)iI))
						{
							...
							if (isAtWar((TeamTypes)iI))
							{
								if(AI_getWarValue((TeamTypes)iI)<-500)
								{
									if ((GET_TEAM((TeamTypes)iI).AI_getWarValue(getID())<-500)
										|| ((AI_getAtWarCounter((TeamTypes)iI) > 30) && (GET_TEAM((TeamTypes)iI).AI_getWarValue(getID())<10*AI_getAtWarCounter((TeamTypes)iI))))
									{
...
bool CvTeamAI::AI_acceptSurrender( TeamTypes eSurrenderTeam )
{
	...
	int iOurVictoryCounter = AI_getLowestVictoryCountdown();
	if( (iTheirVictoryCounter >= 0 && iOurVictoryCounter < 0) || (iTheirVictoryCounter < iOurVictoryCounter) )
	{
		return false;
	}

	// Is surrender team going for cultural victory?  Capturing and holding original capital will halt those ambitions
	for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			if( GET_PLAYER((PlayerTypes)iI).getTeam() == eSurrenderTeam )
			{
				if( GET_PLAYER((PlayerTypes)iI).AI_getCultureVictoryStage() > 2 )
				{
					return false;
				}
			}
